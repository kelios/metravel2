...
function QuestsContentPanel({
    ...
    questsAll,
    questCardWidth,
    availableSortOrders = EMPTY_SORT_ORDERS,
    ...
    // Сетка живёт не всегда (карта, отсутствие данных, пустая выборка); её высота
    // тогда протухает, и layout скроллера раскрыл бы окно по высоте ПРЕЖНЕЙ сетки.
    const questsGridMounted = viewMode !== 'map' && dataLoaded && questsAll.length > 0;
    useEffect(() => {
    ...
    // вовсе, и остаток каталога недостижим. Высоту берём с самой сетки, а НЕ из
    // `onContentSizeChange`: react-native-web вешает его на `onLayout` внутреннего
    // контейнера ScrollView, а тот из-за `flexGrow: 1` растянут ровно на вьюпорт, пока
    // контент помещается, — рост внутри вьюпорта не даёт ни одного события.
    // `styles.questsGrid` не растягивается, поэтому его `onLayout` приходит на каждое
    // раскрытие. Проверка общая и зовётся из обоих обработчиков: общий ResizeObserver
    // отдаёт вложенные узлы раньше скроллера, так что порядок событий ничего не решает.
    // Рост ограничен размером набора — цикл конечен. Сетка меряется без шапки и
    // SEO-блоков над ней, то есть «помещается» — оценка сверху: переоценка стоит одного
    // лишнего шага окна, недооценки быть не может (сетка выше вьюпорта ⟹ скролл есть).
