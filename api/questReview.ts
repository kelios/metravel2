...
//
// КОНТРАКТ ЭНДПОИНТОВ (реализованы; пробы 25.08.2026: список отзывов → `200`,
// отзыв пользователя без токена → `401`, то есть роут есть):
//
// 1) Отправка отзыва
...
//        rating: number,    // 1..5, обязательное
...
//        disliked: string
//      }
//    Пара (user, quest) уникальна: повторная отправка не создаёт второй отзыв.
//    Оценка из этой записи и есть источник агрегата `rating_avg` / `rating_count`
//    в каталоге квестов.
//
