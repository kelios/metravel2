...
- Keep the active-platform contract aligned across docs and skills: desktop web,
  mobile web, and Android through one Expo/React Native codebase;
  RU/BE/UK/PL/EN through the shared i18n layer; every task records platform and
  localization impact. Mobile web and Android validation is always paired. iOS
  scaffolding may remain documented as inactive technical inventory, but iOS is
  not a normal QA/Done-gate surface until explicitly reactivated by the user.
