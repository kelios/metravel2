...
| --- | --- | --- |
| Travel list/details/wizard/export | `components/travel/**`, `components/listTravel/**`, `app/(tabs)/travel*`, `hooks/useTravel*`, `api/travel*.ts`, `stores/*travel*` | Use `$metravel-travel-expert` before `$metravel-feature-builder`; preserve `UnifiedTravelCard`, `ImageCardMedia`, React Query for server state, Zustand for client state, travel media rules, and save != moderate. |
| Map and places | `components/MapPage/**`, `components/map/**`, `app/(tabs)/map*`, `hooks/useMap*`, `screens/tabs/PlacesScreen.tsx`, `api/places.ts` | Use `$metravel-map-expert`; add `$metravel-ui-guardrails` for visible map/popups; keep web Leaflet and native map imports separated by platform files. |
...
| Achievements and badges | `api/achievements*.ts` (`achievements.ts` is only a re-export facade; the BE contract is `achievementsTypes.ts` + `achievementsNormalizers.ts`), `hooks/useAchievementsApi.ts`, `hooks/useGamification.ts`, `components/achievements/**`, `__tests__/achievements/**` | Use `$metravel-achievements-expert`. Keep mock fallback explicit, React Query keys stable, peer-badge optimistic updates rollback-safe, and badge media via `ImageCardMedia`. |
...
| SEO/index/content | `components/seo/**`, `utils/seo.ts`, `utils/*Seo*`, `utils/seoText.*`, article/travel rich text, `docs/GROWTH_PLAN.md` | Use `$metravel-seo-index-operator` for GSC/indexing. Julia travel prose (impressions, diary, guidebook) → `metravel-travel-article` / `travel-writer`. API publish/media without prose → `$metravel-article-editor-agent`. |
