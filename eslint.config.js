...
const baseIgnores = [
  ...
  // `**/` обязателен: в `.gitignore` эти паттерны без ведущего слэша, поэтому
  // git игнорирует scratch на ЛЮБОЙ глубине. Паттерн без `**/` во flat-config
  // привязан к корню — вложенный `components/.codex-temp-probe.js` был бы
  // невидим git'у и линтовался бы, снова давая красный гейт при чистом
  // `git status` (#1846, тот же класс уровнем ниже).
  "**/.codex-temp*",
  "**/.codex-temp*/**",
  "**/.codex-debug*",
  "**/.codex-debug*/**",
  ...
  // Расширения перечислены ровно так же, как в `.gitignore`: асимметрия двух
  // контуров и есть дефект #1466 — git такой файл не пустит в коммит, а eslint
  // прочитает его локально и покрасит общий гейт (#1846).
  "__*.js",
  "__*.cjs",
  "__*.mjs",
  "__*.ts",
