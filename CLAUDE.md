...
- **Паритет mobile web ↔ устройство (правило для всех агентов):** «Мобильная версия» = mobile web (`isMobile`, ~390px) + Android + iOS ОДНОВРЕМЕННО — когда пользователь/задача говорит «мобильный/на телефоне», это всегда все три платформы сразу, и выглядеть они обязаны идентично. Мобильная web-вёрстка и native-приложение — один и тот же дизайн; **эталон — устройство** (Android/iOS оттестировано и принято), поэтому при расхождении правим mobile web под устройство, НЕ наоборот. Любая UI-правка верифицируется на ОБЕИХ платформах со скринами: web-превью 390px (`preview_resize` + `preview_screenshot`) И устройство/эмулятор (`adb exec-out screencap -p`; dev-client подключается к тому же Metro — HMR обновляет обе стороны). Запрещены web-only визуальные ветвления, протекающие в мобильный вьюпорт: serif-шрифты (Georgia/Baskerville) и hover-only элементы (кнопки, появляющиеся по hover) — только desktop web (`!isMobile`); контент-элементы (чипы, бейджи, кнопки карточек) не скрывать `Platform.OS === 'web'`-ветвлением, если на устройстве они видны. Ловушка темизации: `DESIGN_TOKENS.colors.*` на web — живые CSS-переменные (переключаются в dark), на native — статичный светлый fallback → для тематических поверхностей только `useThemedColors()`. Применено: `homeHeroStyles.ts` (sans на mobile + themed-цвета), `EmptyState.tsx`, `WelcomeBanner.tsx`, `screens/tabs/QuestCard.tsx` (play-кнопка и чип отзывов видимы на mobile web), `utils/ensureLeafletCss.ts` (инверсия тайлов в dark — только ≥768px), `TravelHeroInteractiveSlider.web.tsx` (стрелки галереи — только desktop)
...
## Контент / статьи-путеводители (load-bearing)
- **Автор всех статей — Юля (`ignatieva_julia@tut.by`, user id 1).** Любую travel-статью (путеводитель, статью-путешествие) создавать ТОЛЬКО от её имени: перед `create-*-guide.js` / любым `PUT /travels/upsert/` с `id:null` брать её токен — `METRAVEL_TOKEN=$(E2E_EMAIL=$E2E_EMAIL2 E2E_PASSWORD=$E2E_PASSWORD2 node scripts/get-quest-token.js)` (аккаунт 2 в `.env.e2e` = Julia id 1, superuser). Токен по умолчанию (`.secrets/metravel-token.json`, `E2E_EMAIL`) = Сергей (id 104) — под ним статьи создавать НЕЛЬЗЯ. Автор задаётся бэком по токену при СОЗДАНИи и не меняется при upsert/PATCH: если статья вышла под чужим автором — удалить (`DELETE /api/travels/<id>/`) и пересоздать токеном Юли (slug переиспользуется). Правка существующих статей Юли через `seo-edit --desc-file` авторство сохраняет.
- **Фото в статьях — ТОЛЬКО из собственных статей metravel.by (галереи/описания автора).** Брать URL `https://metravel.by/gallery/…`, `/travel-description-image/…`, `/address-image/…` из уже опубликованных travel того же/смежного места; каждый проверять HEAD=200. Внешние/стоковые/Wikimedia картинки в теле статьи запрещены. Нет своего фото под раздел — оставить раздел без фото, не выдумывать. (Исключение — обложки квестов: отдельный пайплайн `assets/quests/*/PROMPT.md`, там AI-иллюстрации.)
