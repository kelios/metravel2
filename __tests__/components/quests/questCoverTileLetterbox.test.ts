...
//   4) состояние долга зафиксировано: сегодня в порог ≤10% не проходит ни один
//      квест. Когда #1558 отдаст квадратные обложки, перезамер фикстуры уронит
//      именно этот пункт — его правят вместе с фиксом, а не задним числом.
//
// Фикстура — замер прода 2026-08-27 по всем 156 квестам. Square-поля уже есть
// в контракте, но пока все null; backfill отслеживает #1587. Обновляется командой:
...
describe('#1542 летербокс обложки квеста в квадратной плитке', () => {
  ...
  it('фикстура выдачи зафиксирована: сегодня порог ≤10% не берёт ни один квест', () => {
    ...
    // бэкенд отдаст квадратный вариант обложки: тогда `withinTarget` станет
    // 156, и число правится вместе с фиксом, а не задним числом.
