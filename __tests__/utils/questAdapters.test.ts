...
describe('questAdapters', () => {
  ...
  describe('fixMediaUrl', () => {
    ...
    it('fixes double-host AND strips S3 signed params in one pass', () => {
      const brokenUrl = 'https://metravel.byhttps://metravelprod.s3.amazonaws.com/quests/5/video.mp4?X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Signature=abc123&X-Amz-Credential=test&X-Amz-Date=20260224T000000Z&X-Amz-Expires=3600&X-Amz-SignedHeaders=host';
      expect(fixMediaUrl(brokenUrl)).toBe('https://metravelprod.s3.amazonaws.com/quests/5/video.mp4');
    });
  });
