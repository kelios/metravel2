...
describe('expectedTravelSlug: зеркало бэкендового slugify', () => {
  ...
  // удаление одного кодпоинта роняет две проверки (свою строку и свип ниже,
  // потому что символ уходит в `null`), удаление всех четырёх — пять. Возврат
  // `SEPARATOR` к категории они НЕ ловят и поймать не могут: `DROPPED`
  // спрашивается раньше, поэтому на сегодняшних кодпоинтах обе модели ведут
  // себя одинаково. Смысл инверсии — в символе, которого ещё нет; за ним
  // следит свип ниже.
  it.each([
    ['a\u05beb', 'ab'],
    ['a\u1806b', 'ab'],
