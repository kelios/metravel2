...
describe('пост-деплой медиа-гейт: ступени сверены с IMAGE_STORAGE_POLICY_V1', () => {
   ...
   * ни одной, любая ступень отдаёт мастер целиком (v16,
   * `v1_then_master_no_transform`), а фронт просит у него РОВНО одну ширину. Пока
   * потолок был 1600, разница не проявлялась; когда профиль дорос до 1920, гейт
   * начал бы мерить класс шириной, которой у него нет (#1373).
   */
