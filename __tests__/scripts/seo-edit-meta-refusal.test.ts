 ...
 *
 * Код возврата 2, а не 1: по общему контракту SEO-CLI (#1391) 2 значит «вызвали
 * неверно», 1 — «прогон упал по дороге». Прогон здесь не начинался, и код 1
 * отправил бы оператора искать бэкап, которого нет.
 *
...
describe('seo-edit --meta refusal', () => {
  ...
  it('отказывает до записи кодом «вызвали неверно» и называет причину', () => {
    expect(run.status).toBe(2)
    expect(run.stderr).toContain('--meta не поддерживается сервером')
    expect(run.stderr).toContain('#1737')
  })

  it('не ходит в сеть вовсе: ни чтения статьи, ни PUT, ни бэкапа', () => {
    // Отказ стоит до первого запроса, поэтому нет даже строки с названием
    // статьи, которую печатает успешное чтение.
    expect(run.stdout).not.toContain('travel #990301')
