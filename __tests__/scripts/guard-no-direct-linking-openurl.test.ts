const {
  ...
  ALLOWED_FILES,
} = require('@/scripts/guard-no-direct-linking-openurl')

describe('guard-no-direct-linking-openurl', () => {
  it('parses --json flag', () => {
  ...
  it('ignores allowed file utils/externalLinks.ts', () => {
    const violations = findViolationsInSource({
      ...
      content: 'await Linking.openURL(url)',
    })
  ...
  it('passes when there are no direct calls in scanned sources', () => {
    const result = evaluateGuard({
      sources: [
        {
          filePath: 'components/home/HomeHero.tsx',
          content: 'void openExternalUrl(url)',
        },
      ],
    })
  ...
  it('builds json contract output', () => {
    const json = buildJsonResult({
      ok: false,
      reason: 'fail reason',
      violations: [{ file: 'a.ts', line: 10, snippet: 'Linking.openURL(url)' }],
    })
