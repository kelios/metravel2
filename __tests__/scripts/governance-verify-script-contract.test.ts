...
describe('governance verify script contract', () => {
  it('keeps governance:verify wired to structural guards and governance tests', () => {
    const packageJson = JSON.parse(fs.readFileSync(packageJsonPath, 'utf8'))
    const scripts = packageJson?.scripts || {}

    const guardExternalLinks = scripts['guard:external-links']
    const guardTextRowSizing = scripts['guard:text-row-sizing']
    ...
    const governanceVerify = scripts['governance:verify']
    ...
    ensure(typeof guardTextRowSizing === 'string' && guardTextRowSizing.length > 0, 'Missing scripts.guard:text-row-sizing.')
      ...
      governanceVerify.includes('guard:text-row-sizing'),
      'scripts.governance:verify must invoke scripts.guard:text-row-sizing.',
    )
    ensure(
