...
describe('scanDictionary: условие срабатывания', () => {
  ...
  it('молчит, если зеркальную фразу уже принимает морфологический проход #1631', () => {
    // «сидить»/«сидит» — пара написаний для orthoStem, но «он сидить»
    // принимается как словоформа «он сидит». Это не дырка словаря (#1926).
    expect(gaps(['сидит', 'сидить', 'он сидит'])).toEqual([])
  })
  ...
  it('перечисляет каждую недостающую фразу отдельно', () => {
    expect(gaps(['сад', 'садъ', 'городской сад', 'им сад']))
      .toEqual(['городской садъ', 'им садъ'])
  })
