...
describe('SSG-срез маршрута квеста (#1763)', () => {
  ...
  describe('splitQuestStorySentences', () => {
    ...
    // Фразы взяты из прод-`story` (#1952): на них разбор по любой точке резал
    // мысль пополам, и на сайт уходил хвост — «Социалистическая, 56/40.»
    // вместо адреса музея. Выдуманный пример тут не годится: ловушка живёт в
    // авторской пунктуации каталога, а не в абстрактном сокращении.
    it('не режет фразу по точке после сокращения', () => {
      expect(
        splitQuestStorySentences(
          'Часы работы: вторник — воскресенье с 10:00 до 18:00, касса — до 17:30, понедельник — выходной. Адрес: ул. Социалистическая, 56/40. Если музей закрыт — нажми «Далее».',
        ),
      ).toEqual([
        'Часы работы: вторник — воскресенье с 10:00 до 18:00, касса — до 17:30, понедельник — выходной.',
        'Адрес: ул. Социалистическая, 56/40.',
        'Если музей закрыт — нажми «Далее».',
      ]);
      expect(
        splitQuestStorySentences('Открыли его около 12 года до н. э., и вмещал он тысяч пятнадцать зрителей.'),
      ).toEqual(['Открыли его около 12 года до н. э., и вмещал он тысяч пятнадцать зрителей.']);
    });

    // Карточка требует минимум «св.» — в каталоге оно встречается только с
    // заглавной («церковь Св. Иоанна»), но список сверяется по строчной форме,
    // поэтому обе формы закрыты одной строкой.
    it('не режет фразу по точке после «св.» в любом регистре', () => {
      expect(
        splitQuestStorySentences('На границе старого квартала стоит церковь Св. Иоанна — краснокирпичная готика.'),
      ).toEqual(['На границе старого квартала стоит церковь Св. Иоанна — краснокирпичная готика.']);
      expect(splitQuestStorySentences('Тут стоит церковь св. Николая, покровителя моряков.')).toEqual([
        'Тут стоит церковь св. Николая, покровителя моряков.',
      ]);
    });

    it('не режет фразу по точке между инициалами', () => {
      expect(
        splitQuestStorySentences(
          'Мемориал открыли 8 мая 1975 года, к тридцатилетию Победы. Архитекторы Т. Басенов, Р. Сейдалин и В. Ким, скульпторы В. Андрющенко и А. Артимович сделали композицию из трёх частей.',
        ),
      ).toEqual([
        'Мемориал открыли 8 мая 1975 года, к тридцатилетию Победы.',
        'Архитекторы Т. Басенов, Р. Сейдалин и В. Ким, скульпторы В. Андрющенко и А. Артимович сделали композицию из трёх частей.',
      ]);
      // Кириллица каталога шире русского алфавита: «і», «ў», «қ» и «ј» в
      // рассказах уже есть. Инициал сверяется с фамилией по блоку, иначе
      // белорусское имя снова публиковалось бы с середины — «Луцкевіч жыў…».
      expect(
        splitQuestStorySentences('Вуліцу назвалі ў гонар паэта. Тут жыў І. Луцкевіч, выдавец «Нашай Нівы».'),
      ).toEqual([
        'Вуліцу назвалі ў гонар паэта.',
        'Тут жыў І. Луцкевіч, выдавец «Нашай Нівы».',
      ]);
    });

    // Авторская пауза, а не конец мысли: «сыграл... кинотеатр» — это шутка про
    // кастинг, и без склейки на сайт уходило одинокое слово «кинотеатр.».
    it('не режет фразу по многоточию-паузе автора', () => {
      expect(
        splitQuestStorySentences(
          'Минский кинотеатр «Победа» сыграл... кинотеатр. «Победа» — сталинский послевоенный кинозал со строгим фасадом.',
        ),
      ).toEqual([
        'Минский кинотеатр «Победа» сыграл... кинотеатр.',
        '«Победа» — сталинский послевоенный кинозал со строгим фасадом.',
      ]);
    });

    // Обратная сторона правила: одинокая ЛАТИНСКАЯ заглавная в русском тексте —
    // римская цифра или форма буквы, а мера после числа («1905 г.») относится к
    // числу. Склей их — и две мысли слиплись бы в одну длинную фразу.
    it('оставляет границу там, где точка всё-таки конец предложения', () => {
      expect(
        splitQuestStorySentences(
          'Апостола казнили на кресте в форме буквы X. Этот знак город носит на гербе примерно с 1419 года.',
        ),
      ).toEqual([
        'Апостола казнили на кресте в форме буквы X.',
        'Этот знак город носит на гербе примерно с 1419 года.',
      ]);
      expect(
        splitQuestStorySentences(
          'На церемонии присутствовал сам болгарский царь Фердинанд I. Автор проекта, австрийский архитектор Фридрих Грюнангер, перестроил половину Софии.',
        ),
      ).toEqual([
        'На церемонии присутствовал сам болгарский царь Фердинанд I.',
        'Автор проекта, австрийский архитектор Фридрих Грюнангер, перестроил половину Софии.',
      ]);
      expect(splitQuestStorySentences('Собор заложили в 1905 г. Началась другая эпоха.')).toEqual([
        'Собор заложили в 1905 г.',
        'Началась другая эпоха.',
      ]);
    });

    // Склейка спрашивает про многоточие, а не про одну лишь строчную букву.
    // Правило «продолжается строчной — значит не граница» схлопнуло бы текст без
    // заглавных начал в ОДИН кусок, и город остался бы вовсе без заметок:
    // `questPointWalkSentences` публикует ХВОСТ после первых предложений, а у
    // единственного куска хвоста нет.
    it('не схлопывает в один кусок текст без заглавных начал', () => {
      expect(splitQuestStorySentences('первое предложение тут. второе предложение тут.')).toEqual([
        'первое предложение тут.',
        'второе предложение тут.',
      ]);
    });

    // Инициал — ровно одна заглавная буква. Двухбуквенное имя собственное в
    // конце фразы от сокращения не отличить, поэтому «Ош.» границей остаётся, а
    // реальные «Св.»/«Дж.»/«Шв.» перечислены в списке сокращений поимённо.
    it('не принимает короткое имя собственное за инициал', () => {
      expect(splitQuestStorySentences('Город называется Ош. Здесь красиво.')).toEqual([
        'Город называется Ош.',
        'Здесь красиво.',
      ]);
      expect(splitQuestStorySentences('Всю роскошь сделал Дж. Б. Аллио. Это лучшие ворота Вышеграда.')).toEqual([
        'Всю роскошь сделал Дж. Б. Аллио.',
        'Это лучшие ворота Вышеграда.',
      ]);
    });
  });
});
