...
//
//   1. Искать границу счёта в подсказке. Подсказка — необязательная ссылка,
//      её открывают не все (карточка лишь предлагает её после первого отказа),
//      а считать игрок начинает по заданию. Граница обязана быть в `task`.
...
describe('hasCountBoundary — что считается названной границей', () => {
  ...
  it('маркеры ищутся без учёта регистра и в неразрывном пробеле', () => {
    ...
    expect(hasCountBoundary('Считай только фигуры.')).toBe(true)
    // Неразрывный пробел внутри маркера: без нормализации «в\u00a0счёт не» не совпал бы с «в счёт не».
    expect(hasCountBoundary('Колокольня в\u00a0счёт не идёт.')).toBe(true)
  })

  it('размерные прилагательные границей не считаются (шаг 691, гейт #1718)', () => {
    expect(hasCountBoundary('Пересчитай все его купола — большие и маленькие.')).toBe(false)
    expect(BOUNDARY_MARKERS.some((m) => /больши|крупн/.test(m))).toBe(false)
  })
...
describe('inspectStep — критерий находки', () => {
  ...
  // Главный контур: подсказка — необязательная ссылка, до неё доходят не все,
  // а считать начинают по заданию. Так была устроена почти вся выборка свипа #1718.
