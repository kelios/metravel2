...
export class BlockRenderer {
  ...
  private renderImageGallery(block: ParsedContentBlock & { type: 'image-gallery' }): string {
    return renderImageGallery(
      {
        theme: this.theme,
        detectPortraitIndex: (images, fallbackIndex) => this.detectPortraitIndex(images, fallbackIndex),
        renderGalleryImageCard: (img, options) => this.renderGalleryImageCard(img, options),
      },
      block
    );
  }
