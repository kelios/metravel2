...
`SUPPORTED_BELKRAJ_COUNTRIES`. У квестов в неподдержанной стране место виджета на web
занимают `AffiliateOffers`; на native офферы показываются как и раньше. Тот же гейт
стоит на travel-секции «Экскурсии» (`ExcursionsSection`) и на ссылке в
