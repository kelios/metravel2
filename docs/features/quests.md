...
- **Подсказка выдаёт ответ** (`QUEST-HINT-LEAK-001`). Буквальный класс ловится
  ...
  находке, умеет работать по локальному data-файлу до заливки). Сканируется по
  умолчанию только `hint`; `--fields=` знает ещё `title`, `story`, `task` и
  `location` — у всех четырёх есть известные находки на проде, поэтому в
  умолчание они не входят. Семантический класс (подсказка-определение) не
