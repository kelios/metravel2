...
- Без бандлов город снова становится обёрткой вокруг карточки квеста, поэтому
  `assertQuestCityLandingsCarryWalk` останавливает сборку, называя город, а
  `verify-static-quest-seo.js` держит порог объёма: у `quest-city` больше нет
  строки в `THIN_CONTENT_EXEMPTIONS`, уровень меряется общим правилом (≥300 слов
  прозы, ≥30% собственных 5-грамм). Замер по всем 132 городам 15.09.2026: было
  118/148/182 слова и 0% своих формулировок, стало 340/694/996 слов и 38,8–79,6%
  (#1569).
