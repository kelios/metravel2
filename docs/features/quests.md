...
- **Сквозная воронка прохождения (`utils/questFunnelAnalytics.ts`).** Семь шагов,
  читаемых подряд в GA4 и в составной цели Метрики:
...
- **Как читать.** GA4 (ресурс `362328641`): Explore → Funnel exploration, семь
  ...
  своё значение, в отчётах они ушли бы в «(other)». Метрика (счётчик
  `62803912`): каждое событие приходит как `reachGoal` с тем же именем, на все
  семь имён есть JS-цели (условие «совпадает»; `quest_start` — старая цель с
  условием «содержит»). Шаги цели задаются именами, а не параметрами, поэтому у
  каждого шага воронки своё имя события. Составная цель «Воронка квеста:
  просмотр → засчитано» — пять шагов `quest_view → quest_start →
  quest_point_1_done → quest_progress_50 → quest_completion_credited`: больше
  пяти Метрика не позволяет, и шаги считаются в пределах одного визита, так что
  многодневное прохождение до конца в ней не доходит. Полная семишаговая
  воронка — в GA4.
