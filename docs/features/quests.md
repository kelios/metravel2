...
- Типы `answer_pattern` (`utils/questAdapters.ts`): `any` (всегда true, шаг-
  ...
  легаси `function` (eval, не должно быть на проде).
...
- `answer_pattern` типа `function` выполняется через `eval` как «фолбэк для
  миграции»; есть ли такие записи на проде — не проверено.
...
- [`docs/PROBLEM_MEMORY.md`](../PROBLEM_MEMORY.md) — `QUEST-ALIAS-001`,
  `QUEST-HINT-LEAK-001`, `QUEST-CONTENT-ROT-001`, `BUILD-CATALOG-001`,
  `ROUTE-BUNDLE-001`
