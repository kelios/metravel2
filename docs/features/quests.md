...
- **Как читать.** GA4: Explore → Funnel exploration, семь шагов по именам выше,
  разбивка по `device`/`auth_state`/`traffic_source` или встроенному `Country`. Метрика:
  каждое событие приходит как `reachGoal` с тем же именем, из них собирается
  составная цель — шаги по именам, а не по параметрам, поэтому у каждого шага
  своё имя события.
