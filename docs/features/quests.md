...
1. **Каталог `/quests`.** `useQuestsList` → `buildQuestCityCatalog` группирует
   ...
   персистится.
   Личные срезы «Пройденные» / «Не пройденные» (#1791, владелец —
   ...
   «Не пройденные» появляются после первого прохождения и пока срез не пуст:
   при нуле прохождений они дословно повторяют весь каталог, а на закрытом
   каталоге вели бы в пустую сетку без объяснения. «Пройденные» доступны и с
