...
1. **Каталог `/quests`.** `useQuestsList` → `buildQuestCityCatalog` группирует
   ...
   Сортировка «Популярные» (#1790) — переключатель в шапке рядом со сбросом
   фильтров, поверх уже выбранного среза: порядок даёт общее правило
   `utils/questPopularity` (прохождения → просмотры → id), то же, которым
   бэкенд отвечает на `?sort=popular` и по которому отобрано промо главной.
   По умолчанию выключен — каталог остаётся в порядке бэкенда. Переключатель
   не показывается, пока в срезе меньше двух квестов с
   `POPULAR_QUEST_MIN_COMPLETIONS` прохождениями, и не появляется там, где
   список уже упорядочен по расстоянию («Рядом» с координатой, «Искать в этой
   области»), — там смысл порядка задаёт близость. Свободный поиск перекрывает
   и «Рядом», и область карты, поэтому в его выдаче переключатель снова
   доступен. Сортировка не персистится: это разовый взгляд на каталог, а не
   сохранённый фильтр.
