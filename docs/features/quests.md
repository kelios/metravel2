...
- Детальная страница ниже порога #1930 (≥300 слов прозы, ≥30% собственных
  ...
  страницы квестов вторым проходом (`selectIndexableQuestDetailPages`, то же
...
- Без бандлов город снова становится обёрткой вокруг карточки квеста, поэтому
  ...
  у которого не пришёл ни один бандл (транспортная ошибка `by-quest-id`). Город,
  чьи бандлы есть, но истории точек короче дайджеста детальной страницы и хвоста
  ...
  собственного текста (#1929): `selectIndexableQuestCityLandings` считает отбор
  по всей модели, сборка печатает такие города предупреждением, а
  `verify-static-quest-seo.js` требует заметки только от индексируемой
  посадочной и отвергает `noindex` с меткой Helmet или при наличии заметок.
