...
`awk '/^## <название>/,/^## /' docs/features/quests.md`.
...
| `utils/questPopularity.js` | 135 | правило популярности (зеркало серверного `?sort=popular`); общий модуль для приложения и SSG |
| `utils/questRatingOrder.ts` | 89 | правило порядка по рейтингу (зеркало серверного `?ordering=-rating_avg`), порог публичности из `api/questRating` |
...
| AsyncStorage (доставка прогресса) | `quest_progress_queue_v2`, `quest_progress_deletions_v1` (`utils/questProgressQueue.ts`) | снапшоты, ждущие отправки (#1922), и намерения удалить строку сброшенного прохождения, не подтверждённые сервером (#2043). Пока удаление ждёт, писатель и чтение при открытии квеста строку не трогают, а намерения уходят раньше снапшотов |
...
- `writeCachedQuestBundle` **await-ится** в `fetchQuestByQuestId`: онлайн-ответ
  не должен проиграть гонку force-stop до коммита офлайн-снапшота. Исключение —
  читатели квеста, который посетитель не открывал: городская посадочная зовёт
  `fetchQuestByQuestId(questId, { persistOffline: false })` (#1569), иначе её
...
1. **Каталог `/quests`.** `useQuestsList` → `buildQuestCityCatalog` группирует
   ...
   `DEFAULT_NEARBY_RADIUS_KM`), `KIDS_FILTER_ID`, `BIKE_FILTER_ID`,
   ...
   персистится.
   Страны в сайдбаре и drawer (#2011, `openspec/specs/quest-catalog-sidebar/spec.md`):
   ...
   каталога. Строки видны только вошедшему: у гостя флаг всегда `false`.
   «Ещё не пройдено мной» появляется после первого прохождения и пока срез не пуст:
   при нуле прохождений они дословно повторяют весь каталог, а на закрытом
   каталоге вели бы в пустую сетку без объяснения. «Пройдено мной» доступен и с
   нулём — пустой срез объясняется отдельным состоянием с кнопкой «Выбрать
   первый квест». Выбор персистится как и городской, но сбрасывается при
   выходе из аккаунта; до `authReady` восстановленный личный срез не трогается
   (web-сессия стартует гостевой и отбирала бы фильтр на каждом заходе), а
   состав среза `authReady` не ждёт — ответ каталога уже персональный
   (`Vary: Authorization`).
   «Пройдено другими» — соседний фильтр со счётчиком квестов, где
   `completionsCount > (isCompletedByMe ? 1 : 0)`: общее число завершённых
   `QuestProgress` включает личное прохождение. Квест, пройденный только мной,
   сюда не входит; пройденный мной и другим игроком входит в оба среза.
   Фильтр виден вошедшему игроку при непустом срезе, сохраняется и сбрасывается
   по тем же правилам, что личные фильтры. Счётчики означают число квестов,
   а статистика на карточке и сортировка — общее число прохождений.
   Общий офлайн-снапшот не знает личного статуса: клиентский маркер
   `personal_status_unavailable` сохраняется при обезличивании кэша и
   превращается в `personalStatusUnavailable` в адаптере. Такие записи, как и
   записи без личного флага во время смены аккаунта, не входят в «Пройдено
   другими», чтобы собственное прохождение не приписывалось другому игроку.
   Свежий ответ API возвращает доступность фильтра.
   Порядок каталога (#1790, #1988) — группа чипов в шапке рядом со сбросом
   фильтров, поверх уже выбранного среза. Состояние одно —
   `default | popular | rating` (`QuestSortOrder` в `screens/tabs/questsShared`),
   варианты взаимно исключают друг друга, по умолчанию `default` и каталог
   остаётся в порядке бэкенда. Повторное нажатие по выбранному варианту
   возвращает порядок каталога.

   - «Популярные» — правило `utils/questPopularity` (прохождения → просмотры →
     id), то же, которым бэкенд отвечает на `?sort=popular` и по которому
     отобрано промо главной. Не показывается, пока в срезе меньше двух квестов
     с `POPULAR_QUEST_MIN_COMPLETIONS` прохождениями.
   - «По рейтингу» — правило `utils/questRatingOrder`, дословное зеркало
     серверного `?ordering=-rating_avg` (`quests/catalog.py`:
     `order_by('-qualified_rating', 'id')`): публично оценённые вперёд по
     оценке, остальные хвостом в порядке каталога по id. Ключа по числу
     отзывов НЕТ — иначе клиент разошёлся бы с сервером. Публичность берётся
     из `hasPublicQuestRating` (`api/questRating`, `QUEST_RATING_MIN_REVIEWS`),
     а не из сырого `rating_avg`: агрегат из одного отзыва — вымысел (#1486).
     Вариант не показывается, пока в срезе меньше двух публично оценённых
     квестов; на 19.09.2026 их нет ни одного из 207, поэтому чип не виден и
     появится сам, когда данные дорастут.

   Ни один вариант не появляется там, где список уже упорядочен по расстоянию
   («Рядом» с координатой, «Искать в этой области»), — там смысл порядка задаёт
   близость. Свободный поиск перекрывает и «Рядом», и область карты, поэтому в
   его выдаче порядок снова выбирается. Выбранный порядок сам возвращается к
   `default`, если перестал быть доступным после смены фильтра или города.
   Сортировка не персистится: это разовый взгляд на каталог, а не сохранённый
   фильтр.
...
9. **Квесты в профиле.** Вкладка «Обзор» держит два соседних блока поверх
   одного компактного каталога (`useQuestCityCollections`, отдельного запроса
   ни один из них не заводит): полосы коллекций по городам (#1484,
   `ProfileQuestCollections`, максимум 4 города с прохождениями) отвечают
   «сколько осталось в городе», а список «Мои квесты» (#1794,
   `ProfileCompletedQuests`) — «что я уже прошёл». Список отсортирован по
   городу и названию: времени прохождения в каталоге нет, а порядок id для
   истории игрока произволен. Показываются первые
   `PROFILE_COMPLETED_QUESTS_LIMIT`, остальные уводит «Показать все» — в тот же
   срез «Пройденные», что и сайдбар каталога. Передаётся он одноразовым ключом
   `STORAGE_PENDING_CATALOG_SELECTION`, который каталог забирает и удаляет на
   фокусе (`useQuestCatalogHandoff`): вкладка живёт всю сессию, `router.push`
   на соседнюю вкладку второго экземпляра не создаёт, а `STORAGE_SELECTED_CITY`
   экран читает только при монтировании — уже открытый каталог остался бы в
   прежнем срезе. Без прохождений оба блока не рендерятся вовсе.
...
У гвардии два режима, и они проверяют РАЗНОЕ, потому что у входов разная форма
(#1810). Прод-режим (`npm run quest:scan-point-roles`) сверяет проставленное
`point_role` с ожидаемой ролью. Режим `--source` читает авторский
`scripts/<city>-quest-data.js`, где роли ещё нет — её проставит заливщик, —
и проверяет другое: что необязательность обещана СЛОВАМИ в заголовке, а не
выведена косвенно (иконка привала плюс свободный ответ) и не объявлена одним
лишь `point_role` при молчащем заголовке. Этот режим зарегистрирован в
`check:fast` по изменённым `*-quest-data.js`, то есть новый квест проверяется до
заливки. Осознанные исключения — привалы, названные без «(по желанию)» ещё до
появления правила, — лежат в `scripts/quest-point-roles-baseline.json`
(`npm run quest:scan-point-roles:baseline`); новая такая точка валит гейт сразу.
...
Номер в кружке карточки — позиция точки в маршруте (`QuestWizard.tsx` передаёт
`index={currentIndex}`). Номер в начале заголовка — авторский текст, связи с
кодом у него нет. Вставленная в середину маршрута остановка «по желанию» сдвигает
кружок и оставляет авторскую нумерацию на месте: так в четырёх детских квестах
ребёнок видел кружок «8» и заголовок «7. Логово базилишка» (#1804). Правится
контентом, не кодом — сквозной номер честен, он считает всё, что игрок проходит.
Стережёт `npm run quest:scan-step-numbering` (exit 1 на расхождении); точки без
номера в заголовке скан пропускает — у привала его нет по замыслу.
...
- Продуктовая аналитика (`queueAnalyticsEvent`): `quest_view`, `quest_start`,
  `quest_point_1_done`, `quest_progress_25`, `quest_progress_50`,
  `quest_progress_75`, `quest_step_view`, `quest_answer_submit`, `quest_hint`,
...
- **Сквозная воронка прохождения (`utils/questFunnelAnalytics.ts`).** Семь шагов,
  читаемых подряд в GA4; в составную цель Метрики входят пять из них (см. «Как
  читать»):

  | # | событие | когда |
  | --- | --- | --- |
  | 1 | `quest_view` | карточка квеста открыта, бандл загружен |
  | 2 | `quest_start` | игрок встал на первую настоящую точку |
  | 3 | `quest_point_1_done` | закрыта первая точка |
  | 4 | `quest_progress_25` | закрыто ≥25 % точек |
  | 5 | `quest_progress_50` | закрыто ≥50 % |
  | 6 | `quest_progress_75` | закрыто ≥75 % |
  | 7 | `quest_completion_credited` | прохождение засчитано порогом (#1443) |

  Шаги 2 и 7 — существующие события, а не новые синонимы: заводить рядом
  `quest_completed` значило бы удвоить одно и то же попадание и развести два
  ответа на вопрос «сколько дошло до конца». `quest_finish` в воронку НЕ входит
  — оно стреляет и на досрочный выход (`early`), и на незачтённый порог
  (`partial`), то есть не равно «дошёл до конца»; это деталь «прохождение
  закончилось» и знаменатель для анализа досрочных выходов.
- **Общие измерения воронки** (`utils/analyticsContext.ts`) есть на каждом из
  семи событий и на `quest_finish`: `client_id` (тот же браузер между визитами —
  по нему считается unique user), `session_id` (визит), `auth_state`
  (`authenticated`/`guest`), `device` (`mobile`/`tablet`/`desktop`),
  `traffic_source`
  (`google`/`yandex`/`search`/`social`/`referral`/`campaign`/`internal`/`direct`),
  `platform`, плюс квестовые `quest_id`, `city_id`, `run_id`, `passed_count`,
  `steps_count`, `progress_pct`, `duration_sec`.
  - **Страна клиентом не считается.** И GA4, и Метрика проставляют гео по IP
    сами на каждое событие (в GA4 измерение `Country`, в Метрике срез
    «География»), точнее и без сетевого запроса из браузера. Отдельного
    параметра `country` в событиях нет и заводить его не нужно.
  - `traffic_source`, а НЕ `source`: имя `source` в проекте уже занято
    поверхностью UI (`quest_card`, `home_promo`, `quest_finale`, `registration` —
    `growthFunnelAnalytics`, `questRetentionAnalytics`). Под одним именем два
    словаря дали бы в GA4 одно измерение, непригодное ни для одного из разрезов.
    Поэтому у `quest_view` есть и `source` (поверхность входа, сейчас
    `quest_detail`), и `traffic_source` (канал визита) — это разные вопросы.
    Так же и `auth_state`: словарь взят у уже отправляемых событий
    (`GrowthAuthState`, `FavoritesProvider`), а не заведён свой.
  - `traffic_source` фиксируется ОДИН раз за сессию. Считать его в момент события
    нельзя: к пятой точке игрок давно ушёл с посадочной страницы, а после
    перезагрузки на внутреннем URL `document.referrer` показывает нас самих —
    поисковый переход превратился бы в `internal`. `utm_source` важнее реферера.
  - `platform` сегодня всегда `web`: согласие на аналитику читается только на
    web (`readConsent` на native отдаёт `null`), поэтому события воронки с
    native не уходят вовсе. Поле остаётся в общем блоке измерений на случай
    нативной аналитики, разрезом пока не является.
  - `city_id`, а не `city`: так город зовут `quest_card_*` и события петли
    возврата, и воронка переводит на это имя `quest_start`. События гостевого
    гейта (`quest_guest_gate_view`, `quest_guest_gate_login_click`,
    `quest_guest_gate_register_click`) остались на старом `city` — они в воронку
    не входят и переименовываются отдельной задачей. Данные `quest_start` до
    19.09.2026 лежат под старым именем.
- **Прохождение (`run_id`) живёт в localStorage**, ключ
  `metravel_quest_funnel_v1:{questId}`, и переживает перезагрузку. Это не
  украшение: квест идут пешком часами, с возвратами, и веха, живущая в рефе
  компонента, перевыстреливала бы на каждом заходе — середина воронки оказалась
  бы шире её старта. Каждая веха уходит один раз на прохождение; «Сбросить
  прогресс» заводит новое прохождение с новым `run_id` и пустыми вехами.
- **Восстановленный прогресс не засчитывается вехами.** Прогресс приезжает
  асинхронно (AsyncStorage, затем ответ бэкенда), и у квеста без intro визард
  встаёт на первую точку раньше них обоих. Поэтому «взял веху» отличается от
  «увидели чужие точки» не флагом загрузки (его порядок не гарантирован ни одним
  из трёх источников), а приростом: игрок закрывает точки по одной, и скачок
  больше чем на одну точку за тик — это восстановление, а не прохождение. Такие
  вехи записываются молча, а прохождение помечается `backfilled` и не сообщает
  `duration_sec` вовсе: раз точки старше записи, её время старта не настоящее, а
  неверное число хуже отсутствующего.
- `duration_sec` — секунды от `quest_start` до текущего шага; на
  `quest_completion_credited` это и есть время прохождения от старта до финиша.
  Считается по wall-clock из localStorage, поэтому переживает закрытие вкладки и
  корректен для квеста, растянутого на два дня.
- **Вся воронка требует согласия на аналитику** (`readConsent().analytics`): она
  пишет `run_id` и время старта в localStorage. Без согласия события не
  отправляются и запись не заводится; согласие, принятое уже внутри квеста,
  подхватывается на следующей закрытой точке — `trackQuestStart` возвращает
  признак «прохождение заведено», и хук не защёлкивает несостоявшийся старт. Отзыв
  согласия не просто глушит отправку, а СТИРАЕТ уже заведённые метки:
  `clearAnalyticsIdentity()` снимает `client_id`/`session_id`/источник визита,
  `clearQuestFunnelRuns()` — записи прохождений всех квестов; обе зовут
  `app/(tabs)/cookies.tsx` и «только необходимые» в `ConsentBanner`.
- **Как читать.** GA4 (ресурс `362328641`): Explore → Funnel exploration, семь
  шагов по именам выше. Без настройки разрезает по встроенным «Категория
  устройства» и `Country` (параметр `device` из события её дублирует и не
  зарегистрирован). Измерения `auth_state`, `traffic_source`, `quest_id`,
  `city_id` и метрика `duration_sec` (секунды, GA4 даёт и среднее)
  зарегистрированы пользовательскими определениями 23.09.2026: разрезы по ним
  есть только у событий с этой даты, задним числом GA4 их не восстанавливает.
  `run_id`/`client_id`/`session_id` не регистрируются — у каждого прохождения,
  браузера и визита своё значение, в отчётах они ушли бы в «(other)».
  Метрика (счётчик `62803912`): каждое событие приходит как `reachGoal` с тем
  же именем, на все семь имён есть JS-цели (условие «совпадает»; `quest_start`
  — старая цель с условием «содержит»). Шаги цели задаются именами, а не
  параметрами, поэтому у каждого шага воронки своё имя события. Составная цель
  «Воронка квеста: просмотр → засчитано» — пять шагов `quest_view →
  quest_start → quest_point_1_done → quest_progress_50 →
  quest_completion_credited`: больше пяти Метрика не позволяет, и шаги
  считаются в пределах одного визита, так что многодневное прохождение до
  конца в ней не доходит. Полная семишаговая воронка — в GA4.
...
- Детальная страница ниже порога #1930 (≥300 слов прозы, ≥30% собственных
  5-грамм) не роняет сборку и не выкладывается индексируемой: генератор пишет
  страницы квестов вторым проходом (`selectIndexableBuiltQuestPages`, то же
  правило `scripts/lib/questPageDepth`) и ставит тонкой build-owned
  `noindex, follow` (партия 19.09.2026: Суботица 297 и Ниш 299 слова —
  истории точек по одному предложению). `verify-static-quest-seo.js` держит
  обратное расхождение: детальная под `noindex`, очистившая пороги, — ошибка
  сборки; ноль индексируемых при непустом каталоге — потерянное тело шаблона,
  сборка падает. Django-sitemap такие адреса по-прежнему перечисляет —
  верификатор предупреждает; возврат в выдачу контентный: дописать истории
  точек.
- Собственный текст посадочной города — `utils/questCityWalk.js` (`places`
  «что увидите по дороге» + `routes` «как устроены маршруты»). Модель считается
  из бандлов квестов города и общая для сборки и экрана: SSG зовёт её из
  `buildQuestCityLandingModel`, приложение — через `hooks/useQuestCityWalk.ts`
  (React Query, максимум `QUEST_CITY_WALK_QUEST_LIMIT = 3` бандла, запрос после
  простоя, web-only). Публикуется ХВОСТ авторского `story` — предложения, которые
  дайджест детальной страницы (#1763) не взял, — поэтому город и его квест не
  повторяют друг друга (замер по проду 15.09.2026: пересечение 6-грамм 0%
  медиана, 1,5% максимум). Отбор предложений — общий модуль
  `utils/questStoryText.js` плюс строгий фильтр прогулки: вопросы, инструкции
  идущему, второе лицо и ответы ЛЮБОЙ точки квеста не публикуются.
- Без бандлов город снова становится обёрткой вокруг карточки квеста, поэтому
  `assertQuestCityLandingBundlesResolved` останавливает сборку, называя город,
  у которого не пришёл хотя бы один из разбираемых бандлов (транспортная ошибка
  `by-quest-id`: пустые заметки тонкость не доказывают). Город, чьи бандлы все
  на месте, но истории точек короче дайджеста детальной страницы и хвоста
  для заметок не оставляют (партия 19.09.2026: 15 городов по 1–2 предложения на
  точку), собирается под build-owned `noindex, follow` — как страна без
  собственного текста (#1929). Индексируемость города решает тот же порог
  #1930 по всему уровню (`selectIndexableBuiltQuestPages` после сборки всех
  посадочных): без заметок его не взять по построению, с одной-двумя заметками —
  тоже (партия 19.09.2026: Хаапсалу, Клайпеда, Сремски-Карловци, Сигулда,
  Тракай — 243–280 слов, 20–25% своей лексики); сборка печатает такие города с
  причиной, а `verify-static-quest-seo.js` требует заметки только от
  индексируемой посадочной, отвергает `noindex` с меткой Helmet и держит
  обратное расхождение «очистила пороги, но `noindex`».
  Возврат такого города в выдачу — контентный: дописать истории точек хотя бы до
  трёх предложений, чтобы после дайджеста оставался хвост. Порог объёма
  верификатор держит по-прежнему: у `quest-city` больше нет строки в
  `THIN_CONTENT_EXEMPTIONS`, уровень меряется общим правилом (≥300 слов прозы,
  ≥30% собственных 5-грамм), страницы под `noindex` остаются в замере и
  проверяются на обратное расхождение «очистила пороги, но `noindex`». Замер по
  всем 132 городам 15.09.2026: было 118/148/182 слова и 0% своих формулировок,
  стало 340/694/996 слов и 38,8–79,6% (#1569).
- Индексируемость странового лендинга считается ТЕМ ЖЕ правилом, а не числом
  городов: `scripts/lib/questPageDepth.js` — одна реализация на двоих, генератор
  спрашивает её до записи страницы (`selectIndexableQuestCountryLandings`), гвард
  — после сборки, по готовому HTML. Не взяла порог — `noindex, follow` при
  сохранённом теле и ссылках. Этот тег генератор пишет БЕЗ `data-rh`
  (`injectBuildOwnedRobots`): роут страны robots не объявляет, потому что вердикт
  по тексту есть только у сборки, а `meta[data-rh]`, которого нет в клиентском
  `<Head>`, Helmet снимает при гидрации — на проде живой DOM оставался без
  robots вовсе. Гвард падает на странице, чей `noindex` помечен Helmet. Цена —
  после SPA-перехода со страны тег остаётся в DOM до перезагрузки; краулер
  грузит каждый адрес заново и этого не видит. Замер по живому каталогу 15.09.2026: ни одна из 34
  стран порога не берёт (112–270 слов прозы при 300; максимум 20,7% собственных
  5-грамм при 30% — уровень остаётся одним шаблоном), поэтому индексируемых
  страновых лендингов сейчас ноль (#1929). Уровень вернётся в выдачу не когда
  одна страна располнеет, а когда страницы перестанут быть общим шаблоном — как
  это произошло с городом в #1569.
- Внутренняя навигация на страну от индексируемости НЕ зависит:
  `questCountryLandingIsLinkable` (городов ≥ 2, #1762) по-прежнему решает, попадёт
  ли страна в блок «Квесты по странам» на `/quests` и в ссылку вверх с посадочной
  города — 18 стран из 34. Страница под `noindex` остаётся узлом графа, иначе
  «снять с претензии на поиск» превратилось бы в «выкинуть из сайта».
- `sitemap.xml` фронт не генерирует: 18 страновых адресов, которые Django
  публикует по своему порогу «городов ≥ 2» (#1771), после этой правки отдают
  `noindex`. Сборка из-за этого НЕ падает — `warnQuestCountryNoindexInSitemap`
  пишет предупреждение и называет расхождение, а снять адреса из карты сайта
  может только backend-задача.
...
- **Словарь не принимает написание, которое игрок читает** (`QUEST-DICT-SPELLING-GAP-001`).
  Ловит `npm run quest:scan-compound-spelling-gap` (exit 1 на находке вне
  `scripts/quest-compound-spelling-gap-baseline.json`, умеет `--quest-id`,
  `--source=<data-файл>`, `--json`) двумя механическими классами по словарю
  `exact_any`: `compound_spelling_gap` (#1536) — фраза собрана только вокруг
  одного из двух написаний, которые словарь принимает по отдельности
  («городской сад» без «городской садъ»); `split_spelling_gap` (#1907) —
  собственный `task`/`hint`/`story` шага печатает односложную форму словаря
  (длиной ≥ 6) раздельно, а словарь знает её только слитно («от bel vedere» в
  подсказке при словаре из одного «belvedere»). Нормализация общая с рантаймом
  (`scripts/lib/questAnswerNormalize`); перевод фразы, падежи и синонимы —
  соседние классы без механического признака, скан их не ловит. Граница с
  правилом 4a: дословную строку подсказки словарь принимать не может (гейт
  утечки упадёт), такой случай записывается в baseline как осознанное
  исключение, а приём остаётся у транслитераций, которые игрок набирает сам
  («бел ведере»); на прод-прогоне baseline учитывает ключи всех файлов разом.
...
- **Строку прогресса создаёт первое действие, а не открытие экрана (#1803).**
  ...
  строку не создаёт; создание живёт в очереди писателей `withQuestProgress` и
  случается только на пути отправки снапшота и на миграции гостевого прогресса
  (#1905: чтение, слияние и PATCH идут по одному писателю на квест, иначе два
  потока после логина затирают ответы друг друга полным словарём `answers`). Гейт отправки — предикат
...
- **«Чтение идёт» и «записи нет» — РАЗНЫЕ состояния (#1803).** Роут отдаёт
  визарду `initialProgress = undefined` только пока чтение в полёте; когда оно
  закончилось без записи, он обязан отдать ПУСТОЙ снапшот
  (`normalizeQuestProgressSnapshot(null)`). На `undefined` визард засевает
  локальную копию как уже согласованную с сервером (`markSeeded=true`) и глушит
  save-эффект: квест, пройденный без сети, после этого не долился бы на сервер
  никогда — прохождение осталось бы на устройстве без «Пройден», без зачёта в
  счётчике и без первопроходца. С пустым снапшотом слияние честно поднимает
  `serverNeedsPush`, а нетронутый экран даёт `serverNeedsPush = false` и строку
  по-прежнему не создаёт. Обе половины закреплены в
  `__tests__/hooks/useQuestWizardProgress.test.tsx`.
