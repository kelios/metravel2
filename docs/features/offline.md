...
**Последняя актуализация:** 2026-09-25  
...
## Web: ложный `navigator.onLine` (#2111)

На сайте отрицательный `navigator.onLine` — только предварительный сигнал.
`utils/webNetworkStatus.ts` в этом случае один раз проверяет same-origin
`HEAD /favicon.ico` (без тела, `cache: 'no-store'`, предел 5 секунд) и отдаёт
один результат и React Query, и `hooks/useNetworkStatus.ts`. Пока проверка не
подтвердила сеть, запросы не стартуют. Настоящая сетевая ошибка оставляет
offline. Повторы идут при `online` / фокусе / видимости вкладки и с паузой
2→60 секунд, не больше 10 раз за эпизод. Native NetInfo этот модуль не заменяет.
...
5. Возврат сети обновляет активный экран в фоне, не выбрасывая уже показанный
   локальный контент.
6. Живой Android-процесс, которому система на время закрыла фоновую сеть по uid
   (netpolicy), после снятия блокировки сам выходит из «Нет сети» без
   перезапуска. Это не cold start: сценарий и ожидаемый лог `NET-DIAG` —
   `docs/MANUAL_TEST_CASES.md`, кейс AND-USB-16b. Проверка делается на сборке с
   `EXPO_PUBLIC_NETWORK_DIAGNOSTICS=1`, когда устройство свободно.
