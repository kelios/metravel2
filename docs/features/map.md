...
- Bottom-card layout (mobile web sheet ≤560, native bottom card, tablet card):
  ...
  подписью на фото поверх статического градиента. Строка ♥ + «Был / Хочу /
  Планирую» — последняя строка этой же подписи (#1779): на скриме она не
  стоит высоты карточки и сохраняет видимую подпись статуса. Под фото остаётся
  только ряд из 4 icon-действий с точкой. У точки без фото подписи нет, и там
  координаты с ♥/статусом делят ОДИН ряд, а не два полноширинных. Вся карточка
...
это отнятая у карты площадь. Бюджет режима маршрута (`MapMobileTopOverlay`):

- под тулбаром иконок живёт РОВНО один ярус маршрута высотой 44dp;
- пока заданы не оба конца, этот ярус — выбор старта
  (`map-mobile-route-start-selector`); обе его пилюли начинают маршрут заново,
  поэтому у готового маршрута это не контрол, а «уничтожить маршрут» в один тап;
- как только точек две, ярус занимает сводка (`map-mobile-route-summary`) одной
  строкой «дистанция · время» с крестиком. Подпись состояния рисуется ТОЛЬКО у
  ненормальных состояний («Маршрут обновляется», «Прямая линия»): «Маршрут
  готов» рядом с цифрами ничего не добавляет, а на 375dp усекался до
  бессмысленного «Маршрут …». Полная фраза остаётся в `accessibilityLabel`
  строки, поэтому скринридер состояние не теряет; выбор старта возвращается на
  следующем входе в режим маршрута («Очистить маршрут» в тулбаре сбрасывает
  точки и переводит карту обратно в режим мест);
- ярус не пустеет, пока роутинг считает геометрию: `addPoint` обнуляет маршрут,
  а выбор старта к этому моменту уже уступил слот, поэтому на время расчёта
  строку держит статус «Маршрут обновляется» без чипов метрик. Признак ожидания —
  отсутствие геометрии в сторе (`route?.distance ?? null`), а НЕ `isBuilding`:
  запрос стартует через дебаунс (`useRouting.ts`, `ROUTE_DEBOUNCE_MS`), и на это
  окно ярус пустел бы снова, дёргая смещение поповеров на 50dp и обратно;
- у яруса ожидания крестика нет — закрыть можно только готовую сводку: dismiss
  ожидания пришлось бы хранить отдельным ключом, и один раз закрытое ожидание
  гасило бы ярус у всех последующих построений маршрута;
- в этой строке ужимается только подпись состояния: крестик стоит вне
  ужимаемого контейнера, поэтому длинная локаль времени или крупный системный
  шрифт на узком экране обрезают метрики, а не выталкивают единственную кнопку
  «скрыть сводку» за край пилюли;
- подсказка «коснитесь карты» — часть той же фазы построения: она стоит на том
  же смещении, что и ярус, и при двух заданных точках не показывается вовсе,
  иначе легла бы поверх сводки;
- одновременно два яруса не стоят никогда: до #1699 селектор (44dp) и карточка
  сводки (68dp) складывались в 118dp сверх тулбара, и на iPhone от карты
  оставалась полоса (отчёт TestFlight 1.0.5, iPhone 16 Pro);
- смещение яруса — `MAP_ROUTE_ROW_STACK_OFFSET` из
