...
3. `usePlannedTripOriginalTrack` скачивает исходник и разбирает его тем же
   ...
   защитного потолка `ORIGINAL_TRACK_MAX_DISPLAY_POINTS = 12000`. Файл с
   несколькими `<trk>`/`<LineString>` даёт **список сегментов**, по одному на
   непрерывную линию (#1847): склейка в одну ломаную рисовала между несмежными
   треками прямую, которой в файле нет. Разрыв ищется общим для проекта
   ...
   каждому резервируются его концы; число
   самих линий ограничено `ORIGINAL_TRACK_MAX_SEGMENTS = 500` (карта встаёт на
   слоях, а не на точках), сверх потолка рисуются самые длинные из них;
...
6. «Удалить оригинал» в той же карточке (`danger-outline`, иконка `trash-2`)
   только открывает `ConfirmDialog` «Удалить оригинальный файл?»: `DELETE`
   файла (`useDeletePlannedTripRouteFile`) уходит лишь из его «Удалить» (#2054).
   Окно привязано к id файла — сменился или пропал оригинал, окно закрывается,
   ничего не удалив. Повтор подтверждения в одном тике глушит синхронный лок
   `useTripRouteFileBranch` (#1824), отказ показывает `original.removeError`, и
   повтор снова идёт через подтверждение.
...
- удаление сохранённого оригинала (`DELETE` по `routeId`) черновик точек не
  трогает и ответом его не затирает, но тот же синхронный лок
  (`routeFileDeleteLockedRef`) у него есть по той же причине: `disabled` кнопки
