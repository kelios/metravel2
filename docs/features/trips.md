...
- адресный поиск стоит в обеих формах точки (#1782). В добавлении выбор
  ...
  `Accept-Language`), и этот же язык входит в ключ `queryKeys.addressSearch`,
  чтобы кэш не отдал чужой язык;
...
- точку правят не только из списка, но и прямо на карте (#1781). Маркер
  ...
  управляет пользователь. Защёлка снимается, когда на карте не осталось ни одной
  точки, ради которой её поставили: ключ точки координатный с общим округлением
  (`routePointFitKey`, `ROUTE_POINT_COORDINATE_PRECISION`), а НЕ `id` — при
  сохранении маршрута бэкенд пересоздаёт строки точек и выдаёт им новые `id`;
