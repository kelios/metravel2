...
- порядок меняют два пути с общей арифметикой в `routePointReorder.ts`:
  ...
  разной высоты);
- локальный `route` — черновик; `routeSignature()` сравнивает его с сохранённым.
  Пока `routeMatchesSaved === false`, скрываются серверная геометрия, `routingState`,
  профиль высот, а экспорт отдаёт ровно то, что видно на карте;
...
1. **Бэкенд, поле `route_summary` в детали поездки** → `mapRouteSummary`
   ...
   `updated_at`).
2. **Клиентская оценка** `estimateRouteSummary(route, transport)`
   (`api/plannedTripsNormalizers.ts:186`): сумма haversine по точкам с
   координатами, время = расстояние / средняя скорость
   (`car 60, bike 16, foot 4.5, public 30, mixed 35` км/ч), «набор высоты»
   = `distanceKm × 8` — **синтетика, а не измерение**, `stopsCount = route.length − 1`.
   Используется как fallback в `mapTrip` (`mapRouteSummary(...) ?? estimateRouteSummary(...)`),
   в mock-мутациях и в `RouteBuilder` для несохранённого черновика.
...
- `RouteBuilder` делает **ровно один** пересчёт ORS на маршрут: ключ
  ...
  условие — владелец, маршрут совпадает с сохранённым, профиля ещё нет и
...
(car 50 / bike 15 / foot 5 км/ч) — заметьте, это **другие** скорости, чем у
