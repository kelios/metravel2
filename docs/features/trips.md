...
`awk '/^## <название>/,/^## /' docs/features/trips.md`.
...
| `components/trips/planning/useTripRouteFileBranch.ts` | 167 | файлы маршрута (ноль или несколько, #2069): чтение списка, линии всех оригинальных треков, загрузка с проверкой лимита и удаление по `routeId` плюс синхронные локи (#1824) |
| `components/trips/planning/RouteBuilderLayout.tsx` | 153 | две раскладки панели: `stack` (две колонки) и `mapFirst` (через RouteBuilderMobile); блок «Файл маршрута» — одна рамка (#1902) и стопка с полными подписями (#2053): импорт во всю ширину, GPX/KML поровну, карточки оригиналов со «Скачать»/«Удалить» (по одной на файл, #2069) |
...
| `components/trips/planning/TripRouteDownloadButtons.tsx` | 133 | пара кнопок GPX/KML, общая для двух мест; во вкладке «Экспорт» под ними карточки оригиналов на скачивание (`TripRouteStoredFiles`, #2069); в блоке «Файл маршрута» владельца — `fill` без оригиналов (#2053), у участника во вкладке «Маршрут» — прежний компактный ряд |
| `components/trips/planning/TripRouteStoredFiles.tsx` | 188 | сохранённые исходные GPX/KML списком (#2069): карточка на файл с именем, размером, «Скачать оригинал» и (во вкладке «Маршрут») «Удалить» через подтверждение #2054 |
| `components/trips/planning/useTripRouteOriginalDownload.ts` | 53 | скачивание одного исходного GPX/KML; у каждой карточки `TripRouteStoredFiles` свой экземпляр — на вкладках «Маршрут» и «Экспорт» (#2053, #2069) |
...
- `arrivalMode` (#2067, контракт бэкенда #2055): как добираются до точки от
  предыдущей — `train|flight|bus|ferry|transfer`, `null` — «как вся поездка».
  ...
  тело. Так же терялись бронь (#1843) и день похода (#1845).
...
### Исходные файлы маршрута (фаза 2 импорта, #1496; список — #2069)

У поездки хранится **ноль или несколько** исходных GPX/KML
(`api/plannedTripRoutes.ts`, backend-контракт #1493/#1840): до
`PLANNED_TRIP_ROUTE_FILES_MAX = 10` файлов (`TripRouteFile.MAX_FILES_PER_TRIP`
бэкенда), список `GET .../routes/` приходит в порядке `sort_order, id`. `POST`
всегда **создаёт** новую запись (201) в конец списка и на лимите отвечает 400 —
замены на бэкенде нет; `DELETE .../routes/{routeId}/` удаляет ровно один файл.
Хранилище доступно только владельцу поездки: участник получает 403, аноним — 401,
и `usePlannedTripRouteFiles` трактует 401/403/404/501 как «оригиналов нет»
(`[]`), а не как ошибку экрана. До #2069 фронт сводил список к `files[0]`: второй
загруженный файл не видели ни карта, ни «Файл маршрута», а «Скачать»/«Удалить»
работали с первым.
...
1. `TripRouteImportPanel` вместе с текстом файла получает от пикера сам файл
   ...
   наверх при «Заменить»/«Добавить»/«Только трек на карту»;

   Режимов применения три (`TripRouteImportMode`). «Заменить» и «Добавить»
   сводят трек к точкам маршрута и тратят общий бюджет
   `TRIP_ROUTE_IMPORT_DRAFT_MAX_POINTS = 50`; «Только трек на карту»
   (`originalOnly`, #1842) отдаёт наверх **нетронутый** черновик и грузит один
   оригинал. Ветка стоит первой в `buildImportedRouteDraft`, до всей арифметики
   бюджета, поэтому отказ `capacity` в этом режиме недостижим при любом
   состоянии маршрута. Точки не изменились — значит `hasUnsavedRouteChanges`
   остаётся прежним (он сравнивает **сигнатуры** точек, не ссылки), и
   «Сохранить маршрут» уходит одной загрузкой файла без лишнего
   `PUT .../route/`; сама кнопка при этом появляется, `routeBuilderCta`
   показывает её и по одному `hasPendingOriginal`. Отказ `capacity` в двух
   других режимах предпросмотр не закрывает, поэтому подсказка
   `error.capacityHint` и кнопка режима видны рядом с сообщением;

   **Ни один режим прежние файлы не удаляет** (#2069): файл во всех трёх
   режимах **добавляется** к сохранённым. «Заменить» заменяет точки маршрута, а
   не исходники: удалить оригинал необратимо, поэтому это делает только его
   «Удалить» с подтверждением (#2054), а не побочный эффект «Сохранить
   маршрут»; и какой из нескольких файлов «заменять», однозначно не сказать —
   у trip 47 трёхпетлевой KML и GPX по дням законно живут вместе. Пока у поездки
   есть сохранённые файлы, предпросмотр говорит это прямо
   (`original.appendHint`, testID `trip-route-import-original-append-hint`), а
   список сохранённых остаётся видим и пока новый файл ждёт сохранения;
...
2. `RouteBuilder.handleSave` сначала сохраняет точки `PUT /route/`, затем грузит
   ...
   показывается ошибка, файл остаётся выбранным, повтор — тем же «Сохранить
   маршрут». На лимите (`storedRouteFiles.length >= PLANNED_TRIP_ROUTE_FILES_MAX`)
   `useTripRouteFileBranch.uploadPendingOriginal` multipart не отправляет вовсе:
   бэкенд всё равно ответил бы 400, а общий `original.uploadError` обещал бы
   ретрай, который не пройдёт никогда. Вместо него — `original.limitError`
   «удалите лишний трек»; файл остаётся выбранным, и после удаления то же
   «Сохранить маршрут» его догрузит. Успешная загрузка дописывает файл в конец
   кэша списка (`useUploadPlannedTripRouteFile`), удаление убирает из него ровно
   свой `routeId` (`useDeletePlannedTripRouteFile`), затем список
   перечитывается;
3. `usePlannedTripOriginalTracks` (`useQueries`, по запросу на файл) скачивает
   каждый исходник и разбирает его тем же `parseRouteFilePreviews`, что и фаза
   1; `buildOriginalTrackGeometry` (`tripOriginalTrack.ts`) переносит **все**
   точки трека, обрезая только выше защитного потолка
   `ORIGINAL_TRACK_MAX_DISPLAY_POINTS = 12000` на файл. Файл с несколькими
   `<trk>`/`<LineString>` даёт **список сегментов**, по одному на непрерывную
   линию (#1847): склейка в одну ломаную рисовала между несмежными треками
   прямую, которой в файле нет. Разрыв ищется общим для проекта
   `splitRouteLineSegments` (им же режет линии карта статьи), поэтому пауза
   записи внутри одного `<trk>` рвёт линию так же, как граница треков. Потолок —
   общий бюджет файла, он делится между сегментами пропорционально их длине,
   каждому резервируются его концы; число самих линий ограничено
   `ORIGINAL_TRACK_MAX_SEGMENTS = 500` на файл (карта встаёт на слоях, а не на
   точках), сверх потолка рисуются самые длинные из них. Трек кэшируется на файл
...
4. `TripPlanRouteMap` рисует эти сегменты **отдельным слоем** поверх линии
   ...
   разных файлов не склеиваются ни между собой, ни с сегментами своего файла.
   Легенда остаётся одна на весь слой оригиналов; точки маршрута и
   `routeGeometry` не подменяются;
5. Сохранённые файлы показывает `TripRouteStoredFiles` — **карточка на файл**:
   имя (до двух строк), размер, «Скачать оригинал» (native — «Поделиться
   оригиналом») и, во вкладке «Маршрут», «Удалить оригинал». Один компонент на
   два места: блок «Файл маршрута» вкладки «Маршрут» (`TripRouteImportPanel`,
   `testIDPrefix="trip-route-import"`, #2053) и вкладка «Экспорт»
   (`TripRouteDownloadButtons`, `trip-route-export`, только скачивание, под
   рядом GPX/KML — прежняя строка «Исходный файл маршрута: …» и кнопка
   «Оригинал» в ряду ушли вместе со сводкой к одному файлу). У каждой карточки
   свой `useTripRouteOriginalDownload`, поэтому «скачивается» и ошибка
   скачивания относятся к её файлу. Байты приходят те же, что были загружены, в
   отличие от кнопок GPX/KML, которые собирают файл заново из текущих точек;
6. «Удалить оригинал» в карточке (`danger-outline`, иконка `trash-2`) только
   открывает `ConfirmDialog` «Удалить оригинальный файл?»: `DELETE` файла по его
   `routeId` (`useDeletePlannedTripRouteFile`) уходит лишь из «Удалить» окна
   (#2054). Окно одно на список и привязано к id файла: пропал этот файл из
   списка, пока окно открыто, — окно закрывается, ничего не удалив, и соседний
   файл не удаляется. Удаление одно за раз: синхронный лок `useTripRouteFileBranch`
   (#1824) глушит повтор подтверждения в одном тике, а пока идёт `DELETE` одного
   файла, «Удалить» остальных выключены и занята только его
   (`removingRouteFileId` из `variables` мутации). Отказ показывает
   `original.removeError`, и повтор снова идёт через подтверждение.
...
`usePlannedTripsApi.ts` повторяются только сетевые ошибки, HTTP 408 и 5xx:
максимум 2 повтора после первого запроса. Остальные 4xx, отмена запроса и
локальный таймаут не повторяются. На детали планируемой поездки HTTP 404
показывает «Поездка не найдена», HTTP 429 — локализованную просьбу подождать.
Политики других trips-хуков этой правкой не меняются.
...
- адресный поиск стоит в обеих формах точки (#1782). В добавлении выбор
  результата заполняет координаты и пустое название, сохраняя выбранный чип
  типа и уже набранные название с описанием. Форма остаётся открытой: название
  можно исправить, точка создаётся только кнопкой «Добавить точку», отмена
  оставляет маршрут прежним. Тип `place` при этом понижается до `custom` (#1532: `place` без
  `placeId` бэкенд отклоняет вместе со всем маршрутом), а название и описание в
  режиме «Место» с экрана уходят и потому не читаются. В правке тот же
  `AddressSearch` только подставляет координаты выбранного места
  (`handleEditAddressSelect`) и заполняет имя, если оно пустое, — имя
  принадлежит пользователю, а поля широты и долготы остаются ручным путём.
  Подсказки геокодера приходят на языке интерфейса
  (`getActiveLocaleDefinition().geocoderLanguage` в параметре и заголовке
  `Accept-Language`), и этот же язык входит в ключ
  `queryKeys.locationSearch(query, locale)`, чтобы кэш не отдал чужой язык.
  Сам запрос идёт общим `useLocationSearchQuery` (`api/geoQueries.ts`, #1819) —
  тем же слоем, что и форма точки маршрута статьи;
...
- порядок меняют три пути с общей арифметикой в `routePointReorder.ts` и общим
  ...
  разной высоты);
- дни сворачиваются (#2058, макет `trips-plan-route-tab-mock.md` §3). Больше
  15 точек с днями — все дни свёрнуты по умолчанию, строки свёрнутого дня не
  монтируются; заголовок дня — кнопка ≥ 44 px (`aria-expanded`) с датой
  (`formatTripDayMonth`), числом точек (plural) и суммой прямых отрезков дня.
  Сами раскрываются день точки, открытой в форме (из списка, попапа маркера или
  тапом по карте), день только что добавленной точки и день, куда точку
  перенесли полем дня; свернуть день может только человек. Состояние — сессия
  экрана (`routeDayCollapse.ts` + `useRouteListMapFocus`), шаблон и импорт
  возвращают свёрнутый вид, на бэкенд ничего не уходит. При ≤ 15 точках свёртки
  нет: все строки на месте, заголовок дня — подпись без кнопки, но с той же
  строкой «дата · число точек · км». Разворот дня на desktop подгоняет липкую карту под его точки
  (`focusIndices` → `fitBounds`), на mobile то же делает `[⌖]` в заголовке и
  поднимает страницу к карте (web — `scrollIntoView`, native —
  `PlannerPageScrollView`). Перетаскивание при свёрнутых днях ограничено днём
  точки (`dropScope`): замеры строк свёрнутых дней устарели, а перенос в другой
  день — полем дня в форме;
- точку правят не только из списка, но и прямо на карте (#1781). Маркер
  владельца перетаскивается (`draggable` + `dragend` на web, `L.marker` с
  `divIcon` и сообщения `ROUTE_POINT_MOVED`/`ROUTE_POINT_TAP` в native-WebView);
  дроп отдаётся наружу как намерение `RoutePointMove` — карта маршрутом не
  владеет, координаты применяет `RouteBuilder.handleMovePoint` с тем же
  округлением до шести знаков, что и ручной ввод, и с синхронизацией полей
  открытого редактора этой точки. Тап по маркеру даёт правку и удаление: на web
  кнопками в popup, в native — карточкой действий поверх карты. У гостя маркер
  не тянется и кнопок нет: обработчики уезжают в карту только владельцу.
  Первое же перетаскивание ставит защёлку авто-подгонки кадра — дальше видом
  управляет пользователь. Ту же защёлку ставит показ дня на карте (#2058/#2059:
  разворот дня на desktop или `[⌖]` на mobile): поздняя подгонка под весь
  маршрут — догрузившийся трек из файла, новая точка вне кадра, смена способа —
  кадр дня больше не перебивает. Снимает защёлку только оптовая замена маршрута —
  применение шаблона или импорт трека, — и приходит она явным сигналом:
  `RouteBuilder` считает такие замены сам и отдаёт картам счётчик
  `routeReplacementToken`. По самим точкам замену не опознать — повторное
  применение того же шаблона возвращает неперетащенные точки с прежними
  координатами, а сравнение по `id` бесполезно тем более: при сохранении
  маршрута бэкенд пересоздаёт строки точек и выдаёт им новые `id`
  (`ROUTE_POINT_COORDINATE_PRECISION` осталось общим округлением ручного ввода и
  координат дропа, ключом защёлки оно больше не служит). Новый путь оптовой
  замены обязан вызвать `markRouteReplaced()`, иначе кадр останется там, куда
  его увёл пользователь;
...
- «Сохранить маршрут» → `PUT /trips/planned/{id}/route/` с `order: i + 1`.
  Правило применения ответа то же, что у перестроения: ответ ложится в черновик,
  только если за время запроса черновик не разошёлся с тем, что ушло на сервер
  (`submittedRouteSignature`). **Разошёлся — на экране остаются правки
  пользователя, а не серверный ответ** (#1824): потерять уже введённое хуже, чем
  на минуту разойтись с сервером. Расхождение при этом не тихое —
  `hasUnsavedRouteChanges` считается уже от нового `trip.route`, поэтому кнопка
  сохранения остаётся на месте и называет своё действие; отдельного сообщения
  специально нет, чтобы не заводить текст под состояние, которое пользователь и
  так видит по кнопке. Повторное сохранение безопасно: `PUT` заменяет маршрут
  целиком, дублей точек разошедшийся черновик не создаёт. Синхронный лок
  (`routeSaveLockedRef` до `mutate`, снятие в `onSettled`) не даёт отправить
  второй `PUT` повтором нажатия в одном тике — `disabled` кнопки держится на
  `isPending`, а он появляется только со следующим рендером. Тот же тик закрыт и
  у второй ветки кнопки: когда точки не менялись и нажатие только догружает
  оригинал, повтор ловит `originalUploadLockedRef` — иначе второй multipart того
  же файла уходил бы параллельно первому, а завершение первого удаляло бы
  кэш-копию из-под второго и показывало ошибку загрузки на успешно загруженном
  файле. По отказу хранилища лок снимается вместе с успехом, поэтому обещанный
  ретрай той же кнопкой остаётся рабочим;
- удаление сохранённого оригинала (`DELETE` по `routeId` выбранного файла,
  #2069) черновик точек не трогает и ответом его не затирает, но тот же
  синхронный лок (`routeFileDeleteLockedRef`, один на все файлы) у него есть по
  той же причине: `disabled` кнопки
  держится на `isPending`, и повтор нажатия в одном тике слал второй `DELETE`
  того же файла — второй отвечал `404`, и пользователь получал «не удалось
  удалить» на уже удалённом файле. Снятие лока идёт в `onSettled`, чтобы отказ
  не запирал кнопку до перезагрузки экрана. Этим закрыты все мутации МАРШРУТА в
  `RouteBuilder`: PUT маршрута, PATCH транспорта и типа велосипеда, загрузка и
  удаление оригинала. Шестая мутация того же компонента — пересчёт высот —
  дедуплицируется не локом, а ключом запроса (`elevationRefreshKeyRef`, см.
  выше); мутации соседних панелей экрана поездки (RSVP, приглашения, отчёт,
  оценка) в этот разбор не входили;
- смена транспорта и типа велосипеда — тот же `PATCH` по поездке, поэтому у них
  ...
  разошёлся с тем, что было в момент старта мутации. Лок сохранения маршрута
  учитывается тем же гардом (`canCommitRouteRebuild`), и наоборот: две мутации
  одного маршрута не стартуют одновременно.
...
2. **Живое превью и ремонт saved healthy/null** (`#1490`, `#873`): пока
   ...
   - **#2065**: у СОХРАНЁННОГО маршрута (живое превью не владеет тройкой,
     `routingState === trip.routingState`) причина деградации в шапке карты и в
     мобильной строке итога получает свою кнопку «Повторить» —
     `components/trips/planning/useSavedRouteRetry.ts`. Она не трогает движок
     превью: по нажатию уходит существующий owner-only
     `POST /trips/{id}/route-summary/ {provider:'ors', force_refresh:true}`
     (`useRefreshTripRouteElevation`), успех инвалидирует `plannedTrip(id)` —
     причина, карта и цифры обновляются тем же путём, что и ручной пересчёт
     высот. Видна только владельцу и только при временной причине
     (`routingStateRetryable`, таблица §2
     `docs/features/trips-plan-route-tab-mock.md`); disabled на время запроса и
     ещё 10 с после ответа — у ORS 429 в этом же списке кодов;
...
До #2060 предел строк описания приходил только под компактной шапкой
(mobile); на desktop `numberOfLines` не передавался вовсе, и описание в
6 274 знака (поездка-эталон #2058) уводило `trip-plan-tabs` на 2 116 px вниз.
Теперь desktop сворачивает описание до 8 строк тем же `TripPlanCollapsibleText`
и той же кнопкой; mobile — по-прежнему 2 строки под компактной шапкой
`route`-вкладки. Вместимость строки для desktop считается не по вьюпорту
целиком, а по колонке `inner` (`plannedTripScreen.styles.ts`,
`PLANNER_INNER_MAX_WIDTH = 860`) — `app/(tabs)/trips/plan/[id].tsx` передаёт
`tripPlanTextCharsPerLine(Math.min(width, PLANNER_INNER_MAX_WIDTH))` как
`charsPerLine`, иначе на широких мониторах кнопка появлялась бы позже, чем
текст реально переполняет свёрнутую колонку. Короткое описание (≤ 8 строк) на
desktop по-прежнему рендерится целиком без кнопки.

### Разметка текста плана (#2070)

Описание поездки и описания точек хранятся обычной строкой (#1494, #1601), но
при показе разбирается лёгкая разметка — `components/trips/planning/tripPlanRichText.ts`,
один разбор на экран (`TripPlanLinkedText`) и печать (`print/tripPlanPrintHtml`):

- строка `# …` или строка ЗАГЛАВНЫМИ (≥ 3 букв, ≤ 60 знаков) — заголовок раздела;
  буква вплотную к цифре (код брони `X7K2QP`, трасса `E40`) заголовком не делает;
- строка `## …` и первая строка абзаца (после пустой строки или заголовка
  раздела), начинающаяся с даты (`27.09`, `04–11.10`; месяц двумя цифрами
  01–12, поэтому время `14.00` и дробь `5.5 км` днём не считаются), — заголовок
  дня; если и следующая строка начинается с даты, это список («26.09 · отель»),
  а не заголовок, а однострочный абзац с датой (дальше пустая строка или конец
  текста) остаётся обычной строкой — так одиночная запись под «НОЧЁВКИ» не
  становится заголовком;
- `- `, `• `, `* ` — пункт списка, `1. ` / `2) ` — нумерованный пункт;
- `**жирный**`, `*курсив*`, `_курсив_` — только по границам слов, поэтому
  `snake_case` и подчёркивания в адресах курсивом не становятся;
- подпись до «: » в начале строки («Ночь: », «Автобус 201: ») выделяется сама;
  время `08:10` и `https://` подписью не считаются.

Разметка рисуется вложенными `Text` в одном корневом `Text`: `numberOfLines`
свёрнутого описания, выделение и ссылки (`splitTripPlanLinkedText` внутри каждого
фрагмента) работают как для обычного текста; пропсы компонента не менялись.
Блок «Ссылки» (`extractTripPlanLinks`) ищет адреса в тех же фрагментах, поэтому
`**https://…**` не уводит чип на адрес с хвостом маркеров.

### Панель оформления в редакторе (#2072)

Разметку выше ставят кнопки, а не ручной набор: «Заголовок», «Список»,
«Нумерованный список», «Жирный», «Курсив» (`TripPlanFormatToolbar`, Feather
`type`/`list`/`bold`/`italic`, для нумерованного — глиф «1.», цели 44×44). Панель
стоит под полем описания поездки (обычный и полноэкранный режим
`TripPlanDescriptionEditor`) и под полем описания точки (`RoutePointEditForm`,
выделение ведёт `useTripPlanTextFormatting`) — на всех ширинах: системное меню
выделения на Android и iPhone рисуется над текстом и закрыло бы панель сверху
(`docs/RULES.md`, «Rich-text editor toolbars»). Под обычным полем и в подвале
полноэкранного — подсказка по синтаксису.

Правила кнопок — чистая функция `applyTripPlanFormat`
(`components/trips/planning/tripPlanFormatActions.ts`), формат хранения не
меняется:

- «Жирный»/«Курсив» оборачивают выделение в `**…**`/`_…_`; курсор внутри
  слова — оборачивается слово, между словами — пара маркеров с курсором внутри;
  пробелы по краям и маркер строки (`- `, `## `, `1. `) остаются снаружи
  маркеров, многострочное выделение оборачивается и снимается построчно
  (разметка действует в пределах строки);
- курсив соблюдает границы слова разборщика: слово, прилегающее к `_` или
  букве (snake_case, адрес), берётся целиком до пробелов, а текст с `_`
  оборачивается `*…*` — подчёркивания внутри слов и адресов не удаляются;
- «Заголовок»/«Список»/«Нумерованный список» ставят `## `, `- `, `1. `… всем
  непустым строкам выделения, заменяя чужой маркер; если маркер уже у всех —
  снимают его;
- после нажатия выделение остаётся на тексте, поэтому повторное нажатие снимает
  оформление; фокус возвращается в поле.

Прокрутка планировщика (`PlannerPageScrollView`) держит
`keyboardShouldPersistTaps="handled"`: на Android и iPhone кнопка под полем
срабатывает с первого касания при открытой клавиатуре.

### Печатная версия плана (#2068)

Во вкладке «Экспорт» (только web) под навигаторами стоит «Распечатать план»
(`components/trips/planning/print/TripPlanPrintButton.tsx`). Кнопка получает ту
же `displayTrip`, что и GPX/KML, — с разрешённой геометрией, иначе карты дней
рисовались бы по прямой. Устройство модуля `print/`:

- `tripPlanPrintModel.ts` — чистая раскладка без React: дни по `dayNumber`
  (дата = старт + N − 1, точки без дня — последней группой; без дней весь
  маршрут — одна группа «Маршрут»), ночёвка дня, переезды и длина дня.
  Ребро «точка k−1 → k» принадлежит дню точки назначения; ребро к точке с
  `arrivalMode` (#2067) — переезд и в длину дня не входит. Длина считается по
  сохранённой `routeGeometry`: точки привязываются к вершинам линии поиском
  только вперёд с узким запасом (`ANCHOR_TOLERANCE_KM`, 30 м сверх ближайшего),
  берётся первое прохождение и пропускается вершина предыдущей точки — иначе
  кольцо Mullerthal (Эхтернах в начале и в конце) привязывало середину поездки к
  финишу, а точки одного квартала слипались. Сумма — `haversineKm` без
  округления: `calculateDistance` округляет отрезок до 0,1 км и обнулил бы
  плотную линию. Переезд к точке без координат переходит на ребро до следующей
  точки с координатами. Без сохранённой линии расстояния считаются по прямой, и
  модель помечает их `approximate`. Карта дня — самый длинный прогон без
  переездов (≥ 2 точек); утренний путь от ночёвки открывает прогон точкой
  прошлого дня (длина дня его уже считает), кроме ребра короче 30 м. День из
  одних переездов карту не получает.
- `tripPlanPrintHtml.ts` — самодостаточный A4-документ: сводка, таблица дней,
  описание поездки, по листу на день (карта, точки с типом, переездом и бронью
  ночёвки), чеклист #1839 по категориям с квадратиками, лист заметок. Сводка
  прямой линии подписана «Дистанция по прямой», как в `RouteSummaryBar` (#2057).
  Весь текст пользователя проходит `escapeHtml`; панель с кнопкой «Печать»
  скрыта `@media print`.
- `printTripPlan.ts` — открывает окно ДО первого `await`
  (`openPendingBookPreviewWindow`, иначе блокировщик всплывающих окон; хелпер
  открывает `about:blank` без `noopener`, иначе `window.open` возвращает `null`
  и писать документ некуда — так же терялось окно печати квестов), грузит
  чеклист (`fetchTripGear` — только владельцу и участнику «еду», тот же гейт,
  что у `TripGearChecklist`; ошибка — чеклист не печатается), рисует карты дней
  `generateCanvasMapSnapshot` (тот же генератор, что у печати квестов; чанк не
  загрузился — план печатается без карт) и пишет HTML в окно. Аналитика —
  `trackRouteExported(id, 'print')`.

Подписи — ключи `trips:components.trips.planning.print.*` литералами: web-инлайнер
i18n подставляет только строковые литералы, ключ из шаблонной строки на сайте
дал бы «Перевод недоступен».
