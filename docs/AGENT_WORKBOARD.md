...
| T-068 | Validate final `TD-006/D-010` touch-target implementation in browser | Мариночка (QA) + Витаутас (DevOps) | Open | Evidence pending | Run `npx playwright test e2e/touch-targets.spec.ts --project=chromium`, capture desktop/mobile browser screenshots for hero slider + card overlay actions, and record console=0 before `TD-006` can advance |
