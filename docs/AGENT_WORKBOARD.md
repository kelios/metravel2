...
| System architecture | Майкл (System Architect) | Active today | Assigned app-wide architecture audit: what should be reworked, changed, or left as-is | Produce `T-073` architecture audit covering screens, shared modules, data/API flows, risks, and validation plan before new dev work starts |
...
| Final validation | Orchestrator | In progress | Full-page populated capture completed; priority blockers `F-004`/`D-014`/`D-006`/`TD-006`/`T-072`/`T-074` closed | Keep the board honest: only `T-039` and `T-050` remain Open in this visible priority slice |
...
| Майкл (System Architect) | App-wide architecture audit: what to rework/change/keep | `T-073` audit table: area, current problem, decision (`rework`/`change`/`keep`), affected modules, risks, validation, proposed owner/priority | Андриуш + Крина + Ромик |
...
| T-031 | Prepare implementation queue for confirmed page findings | Ромик (Dev) | In progress | Queue unblocked: `F-004`, `TD-006/D-010`, `D-006`, `T-071`, `T-072`, and `T-074` are closed with evidence | Next implementation candidate should come from remaining explicit P1/P2 backlog; confirmed findings still need acceptance criteria and repro before code |
...
| T-053 | Cross-page implementation lane | Ромик (Dev) | In progress | Unblocked 2026-06-02: `F-004`, `D-014`, `TD-006/D-010`, `D-006`, `T-072`, and `T-074` resolved; no active `Blocked` implementation lane | Pull the next scoped P1/P2 backlog item only after owner/evidence check; keep shared-component changes browser-verified |
...
| T-073 | App-wide architecture audit: what to rework, change, or keep | Майкл (System Architect) | Active today | Audit assigned. Inputs: current QA/design/perf backlog, `docs/RULES.md`, `docs/CODEX.md`, `docs/PERF_SPEEDUP_PLAN.md`, `docs/TRAVEL_PERFORMANCE_REFACTOR.md`, tech-debt backlog, and recent browser/perf evidence | Produce a concise architecture audit table for Home/Search/Travel/Map/Places/Quests/PDF-export/Profile/Auth/shared runtime: classify each area as `rework`, `change`, or `keep`; list affected modules, user impact, risk, validation commands, and recommended next owner. No code changes in this task |
...
| TD-028 | Harden e2e authenticated `storageState` and token isolation | `test-author` + Витаутас (DevOps) | P2 | Manual QA saw shared-account token invalidation during travel wizard/PDF attempts; fixture setup should avoid concurrent login races and never print secrets | Open |
...
| PERF-016 | Images | Retina card-media sizing and byte budget audit | `travel-expert` + Витаутас (DevOps) | P2 | Validate shared card image request dimensions after DPR-aware sizing: no blurry retina upscaling, no oversized small-slot transfer, no regression for own-domain resize URLs | `components/ui/UnifiedTravelCard.tsx`, `utils/imageOptimization.ts`, `e2e/pages-perf-budget.spec.ts` | Open |
| PERF-017 | Shared runtime | Production guard for `entry`/`__common` eager bytes after PERF-014 | `test-author` + `refactor-surgeon` | P2 | Convert PERF-014 eager analysis into a repeatable budget check that fails on reintroducing `react-native-gesture-handler`/`reanimated` into web eager path; thresholds documented and run on production export | `metro.config.js`, `scripts/`, `docs/PERF_014_EAGER_BUNDLE_AUDIT.md` | Open |
