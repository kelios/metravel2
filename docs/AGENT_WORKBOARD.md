...
| T-072 | Validate typo-tolerant travel slug fallback | Ромик (Dev) + Андриуш (Reviewer) | In progress | Current tree includes `api/travelDetailsQueries.ts` fuzzy/transposition fallback and `__tests__/api/travels.test.ts` coverage; targeted Jest `npm run test:run -- __tests__/api/travels.test.ts` → 70 passed; worktree clean at 18:55Z before this board note | Review fuzzy/transposition threshold for false positives, then run a browser/API smoke for the known typo slug before Done |
