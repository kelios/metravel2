...
| T-028 | Run PDF/export QA pass | Мариночка (Manual QA) | In progress | `/export` verified guest (login gate `Войти`) + authed (`sergey@lyte.com`): clean empty-state "Чтобы собрать PDF-книгу, добавьте хотя бы одно путешествие" + CTA; `console=0`, `overflowX=false`. Evidence `.codex-temp/manual/eqr/export-authed.png` | Actual PDF generation (BookSettingsModal/output) NOT reachable — test account owns 0 travels; needs an account with ≥1 travel on a non-prod backend |
