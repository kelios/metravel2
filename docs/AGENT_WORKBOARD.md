...
| T-072 | Validate typo-tolerant travel slug fallback | Ромик (Dev) + Андриуш (Reviewer) | In progress | Current tree includes `api/travelDetailsQueries.ts` fuzzy/transposition fallback and `__tests__/api/travels.test.ts` coverage; targeted Jest rerun 19:48 local `npm run test:run -- __tests__/api/travels.test.ts` → 70 passed; reviewer pass: 0 blocking code findings, threshold false-positive risk acceptable for fallback-only path | Run a browser/API smoke for the known typo slug before Done |
...
| T-074 | Add guest login CTA to `/metravel` empty-state (closes open point of `D-005`/`D-015`) | Ромик (Dev) | Open | Evidence pending. Repro from manual pass 2026-06-02: guest `/metravel` shows bare `0 путешествий` with no CTA (desktop+mobile); root cause `components/listTravel/ListTravelBase.tsx:278-285` (`isQueryEnabled` gated on `userId`) | Route via `travel-expert`. When `isMeTravel && !userId`, render the `/favorites`-style login wall ("Войдите в аккаунт" + `Войти`) instead of the generic empty count. Keep authed behaviour unchanged. Validate in browser (guest sees CTA, authed sees list), targeted Jest, `check:image-architecture`, `guard:external-links` |
