...
по IPv6 нет; маршрут IPv6 хоста статический (`accept_ra=0`). Не сделан только
последний шаг — `AAAA` для `metravel.by` и `www.metravel.by` у hoster.by: это
действие владельца, отдельная human-карточка, связанная с #1941; после него —
повторная внешняя проверка по имени хоста и `IOS-16`.
...
Ловушка домашнего NAT64-теста после `AAAA`: DNS64 по RFC 6147 не синтезирует
запись, когда реальная `AAAA` есть, и устройство пойдёт на реальный IPv6-адрес —
Mac без собственного IPv6 (как у владельца) его не маршрутизирует. Домашняя
репродукция отказа Apple в текущих условиях (#1939, шаги 1–3) делается до
публикации `AAAA`; после неё домашний NAT64-стенд сеть Apple не воспроизводит.
