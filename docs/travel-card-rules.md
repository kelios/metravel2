...
#### Настраиваемая высота контентной области (внутри блока)
- В рамках одного блока/секции (например: «Избранное», «История», «Рекомендации») контентная часть карточек должна иметь одинаковую высоту.
- Высота контента должна быть управляемой на уровне компонента карточки (например, проп `contentMinHeight`).
- Пример: для «Избранного» допускается компактный режим, где контент — это одна строка со страной (без дополнительных строк), но высота остаётся одинаковой для всех карточек этого блока.
...
- Если поле просмотров не приходит с бэкенда (например, в некоторых подборках/"Популярных") — блок просмотров не отображается
- Никогда не показываем "0" просмотров
- Иконка: `eye` (Feather)
