...
| `app/_layout.tsx` | Владеет provider order, QueryClient lifetime, theme/error boundary, fonts/splash, web hydration chrome и native footer/runtime. Один provider tree и отсутствие native black screen — invariant. | [Expo Router root layout](https://docs.expo.dev/router/basics/layout/). Менять только с provider-order contract test и runtime evidence затронутой поверхности. | `npm run typecheck`; affected layout/provider Jest; desktop/mobile web route smoke для common/shared UI; installed Android build только при Android-specific runtime impact; iPhone simulator/physical только при iOS-specific impact. |
...
| `fix-brace-expansion.js`: resolved `brace-expansion 5.0.8` | Сохраняет zero-step guard и добавляет callable CommonJS compatibility export: legacy `minimatch` ожидает функцию, тогда как v5 предоставляет `expand` named export. | [brace-expansion](https://github.com/juliangruber/brace-expansion). Удалить compatibility adapter после обновления всех legacy `minimatch` consumers; lockfile должен оставаться без уязвимых copies. | Clean install; require callable + named export; timeout-bounded zero-step unit probe; affected CLI smoke. |
| `patch-package`: React Native `0.86.0` | Для Android 15+ не вызывает устаревшие system-bar color API. Для iOS хранит `HERMES_CLI_PATH` относительно `PODS_ROOT`, чтобы generated local podspec и checksum `hermes-engine` в `Podfile.lock` не зависели от checkout. | [React Native](https://github.com/facebook/react-native). Удалять только после upgrade, в котором upstream не трогает legacy system-bar API на Android 15+ и генерирует portable Hermes CLI path; оба инварианта проверяются отдельно. | `__tests__/config/native-compat-governance.test.ts`; `__tests__/config/ios-release-config.test.ts`; `npm run ios:release:guard`; Android-specific local USB smoke при system-bar changes. |
| `patch-package`: `react-native-render-html 6.3.4` | Заменяет function `defaultProps`, удалённые React 19, на explicit defaults; native rich text/image не должен стать пустым или упасть. `patch-package 8.0.1` запускается с `--error-on-fail`; marker verification остаётся обязательным. | [react-native-render-html](https://github.com/meliorence/react-native-render-html). Удалить после upstream React 19 release и comparison всех пяти patched files на clean install. | Patch marker scan; native `StableContent` tests; article/travel rich-text smoke on installed native build. |
...
4. Для native bootstrap/resolution/rich text — device evidence только на
   затронутой native-платформе: локальная установленная Android-сборка для
   Android-specific contract, iPhone simulator/physical layer для iOS-specific
   contract. Common/shared UI отдельно проверяется в desktop/mobile web.
