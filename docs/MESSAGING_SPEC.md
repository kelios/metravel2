...
|-------|----------|------------|
| GET | `/message-threads/` | Список тредов текущего пользователя |
| GET | `/message-threads/available-users/` | Список пользователей, которым можно написать |
| GET | `/message-threads/thread-by-user/?user_id={id}` | Поиск существующего треда с пользователем |
| GET | `/messages/?thread_id={id}&page={p}&perPage={n}` | Пагинированный список сообщений **по треду** (Фаза 1.1) |
| POST | `/messages/` | Отправка сообщения (payload: `{participants, text}`) |
| DELETE | `/messages/{id}/` | Удаление сообщения |
| POST | `/message-threads/{id}/mark-read/` | Отметка треда как прочитанного (Фаза 1.3) |
| GET | `/messages/unread-count/` | Общий счётчик непрочитанных → `{ count: number }` (Фаза 1.3) |
...
|------------------|--------|----------|
| `GET /messages/?thread_id={id}` | ⏳ Ожидает | Фильтрация сообщений по треду (фронтенд готов) |
| `POST /message-threads/{id}/mark-read/` | ⏳ Ожидает | Отметка прочтения (фронтенд готов) |
| `GET /messages/unread-count/` | ⏳ Ожидает | Общий счётчик непрочитанных (фронтенд готов) |
| `unread_count` в `/message-threads/` | ⏳ Ожидает | Количество непрочитанных на тред (фронтенд готов) |
| `last_message_text` в `/message-threads/` | ⏳ Ожидает | Текст последнего сообщения (фронтенд готов) |
