...
- У экрана с несколькими наборами стилей обязан быть гейт владения ключами.
  Образец — `__tests__/components/travel/travelDetailsStyleKeyOwnership.test.ts`
  (деталь путешествия, `#1708`/`#1711`): собирает все наборы, вычитает ключи
  общего фрагмента и падает на любом имени из двух наборов сразу.
