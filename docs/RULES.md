...
- Below the fold, a page section starts loading on page load by default: it shows a skeleton placeholder until its data lands, and the page is never blocked.
  ...
  - Defer through `useProgressiveLoad` or the shared observer of `useTravelDeferredSectionsModel`, and reserve the section's height so nothing below it shifts. `disableFallbackOnWeb: true` means visibility only; otherwise the fallback timer (default 1000 ms) loads the section even if nobody scrolls to it. Without `IntersectionObserver`, `useProgressiveLoad` loads a visibility-only section at once and a timer-backed one on its timer, so it is never stranded. Timers and idle deadlines follow «Timeout policy» (≤ 1000 ms). Native loads eagerly; the travel-details native scroll staging is native-only and outside this web policy.
...
- Deferred loading registry (web). One entry per file; the allowlist of `scripts/guard-web-deferred-loading.js` mirrors it file for file.
  ...
  - `components/travel/details/hooks/useTravelDeferredSectionsModel.ts` — travel-details sections. The heavy tree mounts only after the hero (`useTravelDetailsPerformance`, see the last entry) and one idle callback (≤ 250 ms). Then the map waits for `200px`, Near/Popular sidebar and comments for `200%` (their height reserve hides the ~900 ms fetch: CLS 0.123 → 0, #1642), author and rating for `200px` or a 500 / 600 ms timer; on web the footer mounts with the tree and its `200px` flag is unused (`2fe0bd5bc`). One observer per `rootMargin` (#562). Without `IntersectionObserver` that observer never arms, so the map, sidebar and comments stay unloaded; author and rating still load on their timers (#2021).
  - `components/travel/details/hooks/useTravelDetailsContentSectionModel.ts` — the video block: `220px` or 1000 ms. The insights observer's result is never read (#2021).
  - `components/travel/details/hooks/useTravelDetailsSidebarSectionModel.ts` — the Near request: `200%` or 1000 ms (#1642). The Popular observer's result is never read (#2021).
  ...
  - `components/travel/ToggleableMapSection.tsx` — not loading: the desktop auto-open at `200px` (TD-04) never arms, because its only web consumer, `TravelRouteMapBlock`, passes `initiallyOpen` (`79d08b128`); the map mount waits in `useTravelDetailsMapSectionContentModel` instead (#2021).
  ...
  - Timer or idle only, invisible to the guard: `hooks/useTravelDetailsPerformance.ts` (hero enhancers and the post-LCP runtime flip on one idle callback ≤ 250 ms after the hero image loads, #558), `hooks/useQuestCityWalk.ts` (city-walk quest bundles on idle, #1569), `components/screens/profile/ProfileScreen.tsx` (the travels request behind the header «Маршруты» counter on idle ≤ 400 ms; an open travels or stats tab requests at once) and `hooks/useLeafletLoader.ts` (the Leaflet runtime of the map page, `components/MapPage/Map.web.tsx`, and of the travel map loads on idle behind the map frame and its skeleton). Three of them carry a 1200 ms deadline over the 1000 ms «Timeout policy» cap — the hero's LCP safety net, the city-walk idle deadline and the Leaflet idle deadline (#2020).
