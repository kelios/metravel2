...
- Always self-verify (mandatory):
  ...
  - when Android verification is relevant, assume a USB Android device may already be connected to this workstation: run `adb devices -l` before marking Android unavailable; if a device is listed with status `device`, test the needed scenario on it using `docs/MANUAL_TEST_CASES.md` `AND-USB-*` instead of asking the user to verify;
