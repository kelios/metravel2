...
| TR-LIST-02 | Канонический текстовый поиск | Desktop web и mobile web; локальный API | Поочерёдно открыть `/search?q=Минск`, `/search?search=Минск`, затем no-match текст и text+filter без совпадений; очистить поле вручную | `q` заменяется на `search` без потери других params/hash; оба URL отправляют одинаковый `query=Минск` без неявного `sort`; каждый fallback сохраняет непустой `query`; no-match показывает существующее пустое состояние, а полный каталог появляется только после явной очистки |
...
| TR-LIST-10 | Пустое состояние | — | Задать заведомо невозможные фильтры | Показан fallback (light→medium→broad) с подсказкой расширить критерии; непустой текстовый запрос на любом шаге сохраняется |
...
| TR-WIZ-20 | Мобильный wizard | Viewport 390×844 | Пройти шаги на мобильном | Видны крошки «Мои путешествия › Создание/редактирование»; милестоны скрыты, навигация работает, нет горизонтального overflow |
...
| AND-USB-11 | Quests native regressions | Установлен build с квестами, приложение в гостевом режиме (в шапке «Войти») | Пройти `e2e/maestro/quest-reviews.yaml`, `quest-intro-map-points.yaml` и `quest-offline-points.yaml` либо вручную повторить их шаги. Отзывы — поиск «Гомелю», не первая карточка каталога. Карта и GPX — поиск «Вавель» («Квест по Кракову: Вавельский дракон», 9 точек). Не сбрасывать прогресс под аккаунтом владельца | Отзывы открывают модалку отзывов; intro-карта квеста показывает «9 точек на карте»; GPX/share готовится без `expo-file-system` runtime throw |
...
| AND-USB-16b | Live process + per-uid background network block | Диагностическая release-сборка `EXPO_PUBLIC_NETWORK_DIAGNOSTICS=1` (`rm -rf "$TMPDIR"/metro-*`, затем `cd android && EXPO_PUBLIC_NETWORK_DIAGNOSTICS=1 ./gradlew :app:assembleRelease`). Гость, прод, каталог квестов уже загружен без force-stop. Устройство свободно: пересборка не должна затирать чужой QA-прогон | 1. `adb shell cmd package list packages -U by.metravel.app` — записать uid. 2. Пока процесс живой, заблокировать фоновую сеть: `adb shell cmd netpolicy add restrict-background-blacklist <UID>` и `adb shell cmd netpolicy set metered-network <SSID> true`, затем HOME и `adb shell am make-uid-idle by.metravel.app`. Если подкоманд нет на этой версии Android — сверить `adb shell cmd connectivity help` и `adb shell cmd netpolicy help` и использовать рабочий аналог (`cmd connectivity set-package-networking-enabled false by.metravel.app`). 3. Снять блокировку обратной командой и вернуть приложение на передний план: `adb shell monkey -p by.metravel.app 1`. 4. Параллельно писать `adb logcat -v time ReactNativeJS:* '*:S'` и оставить строки `NET-DIAG` | Последовательность в логе: `appstate background→active` → `refresh-result isConnected:true` → `online-manager {"online":true}`. Баннер «Нет сети» исчезает, каталог квестов снова с карточками, без перезапуска приложения. Если `refresh-result` остаётся `isConnected:false`, а `dumpsys connectivity` в тот же момент VALIDATED — это отдельный дефект нативного кэша NetInfo, не повтор этого кейса |
...
| AND-USB-27 | Places/map panel and quest map UX | Открыта карта, таб «Места», квест с картой | Нажать карточку места в нижней панели; свернуть/закрыть sheet и проверить выбранный marker popup; сравнить действия card vs marker; открыть навигацию с карточки; проверить нижний отступ; дождаться подложки и выполнить zoom-out до минимального доступного масштаба (не менее трёх шагов), затем вернуться к исходному (Android использует тот же движок, что iOS: `Map.android.tsx` реэкспортирует `Map.ios`); открыть карту квеста в fullscreen, закрыть visible `X`, открыть повторно и закрыть Android Back | Карта фокусируется на выбранной точке и показывает popup/шаблон действий; карточка и marker дают тот же набор навигационных действий; нет большого белого пустого блока снизу; на каждом масштабе, включая минимальный, подложка заполнена тайлами и не превращается в сплошное серое поле; карта квеста открывается fullscreen, показывает точки/линию и закрывается и `X`, и Android Back |
...
| AND-USB-16b | P1 | manual adb netpolicy на живом процессе; диагностика `EXPO_PUBLIC_NETWORK_DIAGNOSTICS=1`, лог `NET-DIAG`. Не заменяет cold-start из AND-USB-16 | manual, нужен свободный Android |
