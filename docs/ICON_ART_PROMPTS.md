...
Turnkey-вход для генерации **растровых иконок и арт-ассетов** (PNG) под MeTravel —
и для UI приложения, и для контента/маркетинга. Спека провайдеро-независимая: финальный
...
Готовый промпт сохраняем рядом с ассетом (в имени тикета/в `docs` или в коммит-сообщении)
для воспроизводимости и перегенерации.
...
### C. Контент / маркетинг (статьи, квесты, соцсети, hero-декор)
...
- Выход: PNG/WEBP под слот; через `ImageCardMedia` (contain+blur), фото-доминанта — см. CLAUDE.md.
...
- Иконки в фичевых компонентах — растр только через `ImageCardMedia`; одноцветный плоский
  глиф предпочтительно SVG-компонентом, а не PNG.
