...
- FRAMING: под слот. **Обложка квеста — ЛАНДШАФТ 3:2, `1536x1024`, выход
  `assets/quests/<questAssetDir>/cover.png`.** Слот обложки в карточке каталога
  `/quests` равен 1.4615 (`screens/tabs/QuestCard.tsx`) и кадрируется только
  `contain`, поэтому доля плоского поля на карточке: 3:2 — 2.6%, 4:3 — 8.8%,
  16:9 — 17.8%, квадрат 1:1 — **31.6%** (замер прода 19.09.2026, #1987).
...
## 11. Provenance текущей app-иконки

- `assets/images/icon.png` и Xcode AppIcon обновлены 2026-08-13.
- Источник формы: существующий first-party знак MeTravel
  `assets/icons/logo_yellow_512x512.png`.
- Новый raster создан OpenAI ImageGen как аккуратная перерисовка того же знака:
  оранжевая круглая птица, тёмный глаз, линия крыла, хвост и две лапки; без
  текста, сторонних логотипов, фото или заимствованных персонажей.
- Постобработка: resize до 1024×1024, непрозрачный тёплый белый фон, PNG без
  alpha; иконка не содержит встроенного rounded-corner mask.
</content>
</invoke>
