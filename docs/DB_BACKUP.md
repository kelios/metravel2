...
Почти всё уже сделано 31.08.2026 в рамках задачи борда **#1650**:
...
Ключ подменён и в `~/.bashrc` прод-хоста, и в `local_env.sh` локального
бэкенд-чекаута (в обоих местах рядом лежит резервная копия с суффиксом
`.before-key-swap-20260831`). Дев-хост живёт на другом ключе и замене не мешает.

Осталось одно: **пересоздать контейнер приложения** — до этого момента он
работает на старом root-ключе, потому что env фиксируется при создании
контейнера. Пересоздание упирается в поломку `docker-compose` v1 на хосте
(Engine 28.0.4 против compose 1.29.2, `KeyError: 'ContainerConfig'`) — это
задача **#1651**. Пока она не закрыта, root-ключ в консоли AWS не удалять.
