...
### Правило: `area=back` из фронтового workspace не реализуется

Фронтовый агент (Claude / Codex / Grok) бэкенд **только читает**. Реализацию `area=back`
пишет владелец бэка в `../metravel-backend`. Нельзя править этот checkout, пушить в
`origin/master`, заводить backend worktree или spawn с cwd в бэк-репо. Просьба «сделай
все todo», «весь борд», «спринт» бэк-карточки **не включает**: сразу после
`metravel_tasks_list` их пропускают, в отчёте — «пропущено N тикетов `area=back`».
...
| `testing` | Реализация и review готовы, diff задачи закоммичен и запушен в `main`; web-задача сразу выкатывается на прод (`frontend-deployer`, изолированный worktree, без отдельной команды владельца) и обязательная in-scope QA выполняется на проде либо назначен точный повторный замер/временное окно с параметром, порогом, текущим значением и trigger/earliest recheck. | Приёмка не заканчивается парковкой: pass → `done`; незавершённая собственная работа → `todo`/`in_progress`; отдельный подтверждённый дефект → новая/reused связанная карточка после Problem Memory. Недоступный доступ/устройство требует немедленного unblock-запроса и продолжения того же прохода. Прод-деплой — только по явной команде владельца. |
...
### Выкат и приёмка на проде — часть перехода `testing → done`

С 20.09.2026 (правило владельца: «задачу надо доводить до деплоя, потом тестинг
на проде, и если всё ок — в `done`; все задачи делаются от начала до конца»)
цепочка задачи не заканчивается на `testing`:
...
3. Приёмка на проде: `board-reviewer` сверяет `curl -s https://metravel.by/.build-source.json`
   → `sha` содержит коммит тикета (`git merge-base --is-ancestor`), затем гоняет
   Done gate по Task Contract против `https://metravel.by`.
4. Зелёный Done gate → `done` с evidence; подтверждённый дефект → `in_progress`
   тому же исполнителю, и цикл `review → testing → выкат → приёмка` повторяется.

Вне цепочки остаются: сторы (`android-publisher`, `ios-deployer` — по отдельной явной
команде на каждый stage), `area=back`, `needs_human` и мутации чужого контента.
Сборки с коммитом тикета на проде нет — это не повод «ждать деплоя» в `testing`, а
сигнал выполнить шаг 2.
