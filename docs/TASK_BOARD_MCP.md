...
- `Platform impact` — `web | Android | iOS | shared | none`, с точным перечнем
  обязательных browser/device checks; `none` должен быть обоснован.
...
- Для shared-правок `Validation` разделяет web, Android и iOS evidence;
  одна платформа не заменяет другую. Localization evidence указывает
