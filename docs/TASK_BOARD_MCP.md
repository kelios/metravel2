...
- При **создании** любого тикета (`metravel_task_create`) — всегда указывать `sprint`. Не
  ...
  нужного `area` (FE → активный FE-спринт, BE → активный BE-спринт, Android QA → активный
  Android/Release спринт). Бэклог-кандидаты без даты — только в активный backlog/migration
  спринт.
