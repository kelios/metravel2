...
- **Контроль:** web ключует защёлку `id` точек — перетаскивание id сохраняет,
  оптовая замена выдаёт новые. Координатный ключ на web неверен: Leaflet отдаёт
  сырую позицию дропа, а `RouteBuilder` кладёт в маршрут `toFixed(6)`, и такой
  ключ не совпал бы с сохранённым НИКОГДА — на маршруте из одной точки защёлка
  снималась бы собственным дропом. В native-payload id нет, поэтому там ключ
  координатный, но огрублённый тем же `toFixed(6)` с обеих сторон; ключи
  нумеруются по ПОЛНОМУ набору точек, потому что маркеры нумеруются им же, а
  нерисуемая точка индекс занимает (`''` в слоте). Regression на обеих картах:
  оптовая замена возвращает подгонку, перетаскивание единственной точки — нет,
  плюс native-тест на слоты (`tripPlanRouteMapMarkerEditing.web.test.tsx`,
