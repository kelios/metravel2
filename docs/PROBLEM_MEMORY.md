...
- **Чего guard не видит:** отложенную загрузку только на
  ...
  наблюдателе (`TRAVEL_DEFERRED_SECTION_KEYS`) и новую обёртку вокруг хука, кроме
  `DeferredSection`. Их ревью сверяет с реестром.
- **Попутные находки #2012:** #2021 — наблюдатели деталей путешествия
  ...
  без `IntersectionObserver` не грузит карту, сайдбар и комментарии; #2020 —
