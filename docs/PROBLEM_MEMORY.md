...
- **Вариант на формат записи (2026-08-11, `#1407`):** прежние случаи — про
  ...
  дефект жил в `analyze-bundle.js:114` и `analyze_bundle.py:161`, которые пишут
  git-tracked `BASELINE_METRICS.json` (лежит с завершающей строкой) — оба
  писателя того же файла её теряли, оба починены здесь же. **Урок про метод:**
...
- **Хвост того же разбора: у файла не должно быть двух писателей
  ...
  `scripts/analyze-bundle.js` (`npm run analyze:bundle`). **Control:**
  `__tests__/scripts/baseline-metrics-single-writer.test.ts` перебирает
  трекнутые файлы и падает, назвав файл поимённо, если у артефакта появится
  второй писатель; отдельным кейсом держится и то, что оставшийся писатель
  подключён к npm-скрипту, то есть не станет мёртвым сам. **Обобщение:** «оба
  писателя починены» — это не решение, а признак того, что владелец файла не
  определён; правильный вопрос не «одинаково ли они пишут», а «почему их два».
