...
- **Правило и его отложенная часть:** порог по умолчанию — 300 слов прозы
  ...
  реально держит 182 страницы квестов. Город и страна внесены в список с полом
  110 слов и владеющей карточкой (`#1569`, `#1929`): без этого гвард уронил бы
  ЛЮБУЮ прод-сборку до того, как эти две контентные карточки закроются.
  Список умеет только сокращаться — когда все страницы уровня начинают
  проходить дефолт, гвард падает на устаревшей записи и требует её удалить.
