...
- **Вариант на формат записи (2026-08-11, `#1407`):** прежние случаи — про
  ...
  проверен откатом фикса — без него падает. **Масштаб проверен отдельно от
  факта:** из 47 мест записи JSON в `scripts/` без завершающей строки пишут 39,
  но дефект ровно один — остальные пишут в бэкапы с таймстемпом, отчёты прогонов
  и lock-файлы, то есть мимо git, а соседние `*-points.json` лежат в репозитории
  сами без завершающей строки и со своими писателями согласованы. Одинаковая
  форма кода здесь не означает одинаковый дефект.
