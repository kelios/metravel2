...
- **Controls:** префиксные паттерны `.codex-temp*` / `.codex-debug*` в
  ...
  шагом в `check:fast` (то есть в `githook/pre-commit`) читает
  `git ls-files` и отклоняет любой файл в корне, названный по конвенции
  одноразового артефакта: top-level dotfile вне явного allow-list плюс
  `__*.{js,cjs,mjs,ts,tsx}` / `_tmp-*` / `*.tmp.*`, — то есть закрывает класс,
  а не имя. Последняя конвенция поймана живьём 07.09.2026 прямо во время
  этой задачи: параллельная сессия положила в корень `probe1847*.tmp.mjs`,
  и общий eslint снова покраснел у всех — третий эпизод семьи наступил до
  того, как был закрыт второй.
...
- **Решение для новой жалобы:** «общий lint красный на чужом файле» — сначала
  проверить, трекается ли файл (`git ls-files`). Трекнутый артефакт — рецидив
  этой семьи; untracked файл параллельной сессии — не дефект репозитория, его
  чинит владелец сессии.
- **Structural risk:** третье совпадение НАСТУПИЛО, и наступило прямо во время
  ...
  обязателен и ведётся в #1852: корень становится закрытым списком, а не
  растущим перечнем угаданных имён. Пока #1852 не сделана, корневой файл с
  произвольным именем (`probe.mjs`) не ловит ни git, ни eslint, ни guard.
- **Последняя проверка:** 2026-09-07 на чистом клоне `origin/main` `5d521ea3f`:
  `npm run lint` → exit 0; пять проб всех конвенций в корне и две вложенные не
  видны ни `git status`, ни eslint; `git add -f __probe.js && npm run
  check:fast` → exit 1, то есть guard предотвращает коммит, а не сообщает о нём
  постфактум.
