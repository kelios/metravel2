...
- **Controls:** `__tests__/components/travel/travelDetailsStyleKeyOwnership.test.ts`
  ...
  читатель. Собирает те же девять наборов и ищет обращение `.<ключ>` по
  `components/`, `hooks/`, `app/`, вырезав комментарии и сами модули наборов.
  ...
  вычисляемого `styles[имя]` — проверено на 2026-09-02. Появится такой путь —
  гейт соврёт в сторону «мёртв», то есть упадёт, а не промолчит. Проверен
  мутацией в обе стороны: заведомо нечитаемый ключ в реальном наборе роняет его
  с именем ключа и владельца, а известный мёртвый ключ, которому дали читателя,
  роняет проверку списка наследства.
  Наследство: на момент заведения гейта не читались 38 ключей из 118 — 18
  `decisionSummary*`, 13 `misc` (включая `travelListFallback`,
  `loadingSkeleton*`, `sectionBadge*`), 3 `neutralActionButton*`, 2
  `backToTop*` и 2 `error*`. Они перечислены в `KNOWN_UNREAD_KEYS` явно, а не
  ...
  незаметно, как отстал пришпиленный перечень в `#1711`, он не может. Слив
  наследства — отдельная карточка `#1714`.
...
- **Последняя проверка:** 2026-09-02; `#1711` и `#1713` закрыты, оба гейта —
  владения и читаемости — зелёные на текущем дереве. Открытых дублей имён нет;
  мёртвых ключей вне `KNOWN_UNREAD_KEYS` нет.
