...
- **Инвариант:** ручное перетаскивание маркера выключает авто-подгонку кадра —
  ...
  пользователем вид. Но защёлка держится ровно до тех пор, пока на карте
  остаётся хоть одна из точек, ради которых её поставили: оптовая замена
  маршрута (шаблон, импорт трека) обязана снова попасть в кадр.
- **Canonical task:** #1781. Смежное: #1303 (drag порядка точек в списке — другая
  поверхность), TRIPS-PLAN-SAVED-ROUTE-TRUTH-001 (geometry/state/summary).
- **Surface/owner:** `components/trips/planning/TripPlanRouteMap.web.tsx`
  (`FitRouteBounds`, `fitLockedKeysRef`), `components/MapPage/Map/nativeMapHtml.ts`
  (`map.__metravelRouteFitLocked` + `__metravelRouteFitLockedKeys`).
- **Причина и evidence:** первая редакция ставила защёлку навсегда до
  ...
  другого региона оставался за пределами кадра до пересоздания карты.
- **Контроль:** ключ точки — её координаты, а НЕ `id`. Идентичность точки для
  карты держится на координатах, потому что `PUT /planned-trips/<id>/route/`
  удаляет строки точек и создаёт заново: тем же точкам приезжают новые id, и
  id-ключ снимал бы защёлку на первом же «Сохранить маршрут». Округление общее
  (`routePointFitKey`, `ROUTE_POINT_COORDINATE_PRECISION` в
  `tripPlanRouteMap.types.ts`): Leaflet отдаёт сырую позицию дропа, а
  `RouteBuilder` кладёт в маршрут `toFixed(6)` — без общего шага ключ дропа не
  совпал бы с сохранённым НИКОГДА, и на маршруте из одной точки защёлка
  снималась бы собственным дропом. В native-скрипт шаг интерполируется из той
  же константы, а не зашит числом. Ключи нумеруются по ПОЛНОМУ набору точек,
  потому что маркеры нумеруются им же, а нерисуемая точка индекс занимает
  (`''` в слоте, из сверки исключается). Regression на обеих картах: оптовая
  замена возвращает подгонку, перетаскивание единственной точки и сохранение
  маршрута с новыми id — нет, плюс native-тест на слоты
  (`tripPlanRouteMapMarkerEditing.web.test.tsx`,
  `nativeMapHtml.routePoints.test.ts`). Ловушка native-скрипта: `marker.on` в
  тестовой обвязке хранит обработчики по имени события, поэтому второй
  `dragend` затирает первый — обновление ключей живёт в том же обработчике,
  что и отправка сообщения.
- **Последняя проверка:** 2026-09-06, unit и статические проверки зелёные;
  runtime QA на web/iOS/Android ещё не подтверждена.
