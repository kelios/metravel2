...
- **Controls (`#1559`):** `--section articles|travels|quests|all`, где выборка
  ...
  JSON (`byKind`: travel / quest-page / quest-city / static — это не значения
  `--section`, и поле названо так, чтобы их не путали);
...
- **Инвариант:** активная категория, созданная в админке, появляется в редакторе
  точки после входа на шаг с точками; поиск по её имени не предлагает создать
  уже существующую запись повторно.
...
- **Подтверждённая причина прежнего инцидента:** устаревший клиентский путь
  ...
  **Пробел текущего кода (2026-08-28):** `useTravelFilters.ts:419-446`
  обновляет категории только при входе в шаг; повтор того же шага блокируется
  в `:427`. Возврат из админки и открытие выбора категорий не вызывают
  обновление. Независимый source-review подтвердил отсутствие альтернативного
  обновления локального state. Это механизм, согласующийся с репортом, но
  точная причина пользовательской production-сессии без network-пробы не доказана.
- **Controls:** forced `GET /api/getFiltersTravel/` при входе на шаг 2/3,
  ...
  для полного имени — также отсутствие предложения создать дубль. План
  исправления: `openspec/changes/fix-travel-point-category-refresh/`;
  apply требует отдельного запроса пользователя.
...
- **Последняя проверка:** 2026-08-28; recurrence подтверждён двумя
  пользовательскими скриншотами (успешное создание в админке и отсутствие в
  редакторе точки). #1518 переоткрыта в `in_progress`; продуктовый код и API
  не менялись, реальное локальное воспроизведение и исправление ещё впереди.
