...
- **Controls:** `node scripts/scan-quest-hint-leak.js` — сверяет каждое
  ...
  `--fields=hint,story,title,task,location`; по умолчанию сканируется только
  `hint`, потому что это единственный контур, который правило 4a требует
  держать чистым безусловно. Остальные четыре в gate не входят: у них есть
  известные находки, и включение любого из них уронило бы каждый прогон, после
  чего скан перестал бы отличать НОВУЮ утечку от давно известной.
- **Открытый риск 1: семантический класс.** Скан закрывает ТОЛЬКО буквальный
  ...
  прогона не было.
- **Открытый риск 2: соседние поля того же шага — измерено, не исправлено.**
  Утечка не ограничена полем `hint`: игрок видит до попытки ещё четыре поля
  шага. Сплошной прогон 18.08.2026 по проду (148 квестов, 1256 шагов):
  `hint` — 0, `title` — 20, `story` — 26, `task` — 16, `location` — 17.
  `location` (#1461) разобран поштучно: реальные утечки все 17, шума нет —
  подпись места буквально называет принимаемый ответ («Ротонда Святого
  Георгия» → `ротонда`, «Арт-объект „Пуговица фонарщика“» → `пуговица`,
  «Могила Влястимила Хофмана» → `хофман`, «Указатель времени зажжения фонарей»
  → `зажжения фонарей`). Это не только печать: `location` рендерится в мастере
  строкой под заголовком шага (`questWizardStepCard.tsx:468`), то есть класс
  бьёт по основному сценарию прохождения. Списки `title`/`story`/`task`
  поштучно не разбирались. Правка контента этих 17 шагов вынесена отдельной
  задачей — механический контроль есть, редакционная работа впереди.
- **Открытый риск 3: кросс-шаговый класс в печати и офлайн-экспорте — принятое
  ...
  таблице маршрута), `questOfflineMapExport.ts:80-81` кладёт подпись в имя и
  описание точки GPX. Поэтому ответ шага N читается с листа заранее, если стоит
  в полях шага M. Замер 18.08.2026: 574 совпадения, из них 458 на границе слова
  — 204 шага в 111 квестах из 148. Гейтом это не становится: три четверти
  квестов падали бы, а выборка показывает, что подавляющая часть —
  неизбежный повтор лексики («башня», «костёл», «круг») и общеупотребительные
  ...
  сам ответ, — театр. Формат рассчитан на пару «ведущий + игрок», а не на самостоятельного
  игрока с полной распечаткой, и подпись места нужна для навигации по маршруту.
  Решение: класс признан ограничением печатного и офлайн-формата, отдельного
  режима скана не заводится. Живой пример — квест `vitebsk-kids-skazki`: ответ
  шага 1 (`пёс`) стоит в тексте шага 8, который про этого пса и есть.
- **Решение для новой жалобы:** «ответ виден прямо в подсказке» с буквальным
  ...
  текст); то же для подписи места (`--fields=location`); подсказка-определение
  ...
  не эта семья.
- **Последняя проверка:** 2026-08-18 (#1461) — скан принимает `--fields=location`
  и по всему проду (148 квестов, 1256 шагов) даёт 0 утечек по `hint` и 17 по
  `location`; кросс-шаговый класс измерен отдельной пробой и признан
...
- **Controls:** `node scripts/scan-quest-mixed-script-text.js` — по всему проду,
  ...
  Читает текст бандла через `textNodes()`: заголовок квеста, интро, каждый шаг и
  текст финала в ОБЕИХ его формах. Не читает сознательно: `finale.title` (в
  прод-сериализатор финала не попадает — там только `text`, `video_url`,
  `poster_url`, `poster_media`, игрок его не видит) и `city.name` (не
  авторский текст квеста, приходит из справочника городов; смешанных слов там
  сейчас нет — проверено по всем 145 локальным квестам). Подключён к
...
- **Грабля, на которой скан уже один раз соврал.** Интро лежит в бандле
  ...
  finale.story`. Первая редакция `scan-quest-mixed-script-text.js`
  ...
  exit 0. Ошибка повторилась и на исправлении: следующая редакция добавила финал
  одним ключом `finale.text` и осталась слепа к пяти квестам с `finale.story`.
  Обе слепые зоны поймало код-ревью #1464, ни одну — прогон. Отсюда три вывода
  на будущее: (1) любой скан контента квеста обязан собирать текст через
  ...
  (3) счётчик в отчёте должен считать то, что скан ПРОЧИТАЛ («6897 текстовых
  узлов»), а не косвенную величину вроде «1237 шагов»: косвенный счётчик и скрыл
...
- **Последняя проверка:** 2026-08-18 после правки #1464 и расширения обхода на
  ...
  148 квестам и 6897 текстовым узлам даёт ноль находок, свип по всем 142
...
- **Controls:** `npm run guard:locale-number-format` — TypeScript-AST guard по
  `app/`, `components/`, `screens/`, `hooks/`, `services/`, `utils/`. Ловит две формы:
  ...
  машинные пропсы. Известная граница: цепочка не проходит сквозь тело функции
  или `useMemo`. Каталог `services/` был вне скана до `#1465` — это была дыра, а
  не осознанная граница: PDF-экспорт печатает пользователю те же числа, что и
  экраны, и `#1465` нашёл там ровно одну находку, после которой скан расширен
  насовсем. Governance-тест
...
- **Последняя проверка:** 2026-08-18 (`#1465`); расширение скана на `services/`
  дало ровно одну находку — `${round(km)} км` в
  `services/pdf-export/.../MapPageRenderer.ts`, сведена на `formatDistance`
  (+ соседние числа профиля высоты — на `formatNumber`/`formatInteger`), после
  чего guard зелёный на 1530 файлах при пустом allowlist. Ранее 2026-08-18; первый прогон guard'а по чистому дереву
