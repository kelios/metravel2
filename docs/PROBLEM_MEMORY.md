...
| P2 | Единый контракт согласования прогресса квеста: поколение = `id` строки, копия пишется только в свою строку (OpenSpec `quest-progress-reconciliation`, #2033); остаётся сброс без сети | Семь разных root cause синхронизации прогресса (#1456 → #2033) чинились точечно: слияние аддитивное, а сброс — единственная немонотонная операция, о которой другие устройства не знали. |

## Реестр

### I18N-UK-CANCEL-NOUN-001 — существительное вместо действия на кнопке отмены

- Каноническая задача: #2064 (2026-09-23).
- Причина confirmed: generated-ресурсы переводили русскую подпись «Отмена»
  существительным: UK «Скасування», BE «Адмена», включая default ConfirmDialog.
- Инвариант: подпись действия отмены — UK «Скасувати», BE «Скасаваць»;
  существительные в поясняющих предложениях сохраняются.
- Постоянный контроль: `__tests__/i18n/resources.test.ts` проверяет переводы
  всех ключей с исходной подписью «Отмена» через `getFixedTranslator`, включая
  `shared:components.ui.ConfirmDialog.otmena_53464360`; `npm run test:i18n`.
- При повторении того же словарного перевода переоткрывать #2064; сбой выбора
  языка или загрузки каталога имеет другую причину и требует связанной задачи.
...
### QUEST-PROGRESS-RESET-LINEAGE-001 — сброс прохождения отменяется копией другого устройства

- **Ключ на борде:** `quest-progress-reset-not-propagated-crossdevice` (#2033).
- **Инвариант:** локальная копия прохождения, однажды согласованная с серверной
  строкой, пишется только в ту же строку. Поколение прохождения — `id` строки
  `quest_progress`: это BigAutoField, значения не переиспользуются, а строка,
  созданная после сброса, получает новый и больший `id`.
  - Строки больше нет или вместо неё другая — копия стирается, а не доливается.
  - Копия без поколения (офлайн-прохождение, ещё не встречавшее сервер) доливается как раньше (#1803).
  - Стирать копию можно только по подтверждённому ответу сервера. Упавшее чтение (нет сети, 5xx) подтверждением не является. 404 про неопубликованный квест (`Quest with quest_id=… not found`) — тоже: строка цела, `fetchQuestProgress` отдаёт его ошибкой, а не «строки нет».
- **Surface/owner:** клиент прохождения — фронтовая территория, API не менялся:
  - `utils/questProgressMerge.ts` (`serverId`, `isQuestProgressRunEnded`, правило слияния разных поколений);
  - `api/quests.ts` (`withQuestProgress(..., { lineageId })` до POST);
  - `utils/questProgressQueue.ts`;
  - `hooks/useQuestsApi.ts` (`progressMissing`);
  - `api/questsCatalogInvalidation.ts` (`resetQuestsCatalogCompletion` снимает только стоящую отметку);
  - `components/quests/useQuestWizardProgress.ts`.

  Контракт — OpenSpec `quest-progress-reconciliation` (change `quest-progress-reset-lineage`).
- **Цепочка:** седьмой различный root cause в синхронизации прогресса квеста:
  #1456 (владелец) → #1803/#1809 (чтение ≠ старт) → #1905 (два создателя
  строки) → #1906 (переезд между квестами) → #1451 (монотонный `completed`) →
  #1922 (очередь на диске) → #2033.
- **Подтверждённая причина:** «Сбросить» физически удаляет строку
  (`quests/views.py` `perform_destroy` в бэке) и локальную копию только ЭТОГО
  устройства. На другом устройстве копия не помнила, из какой строки она
  получена. Поэтому 404 чтения выглядел как «строки ещё нет»:
  `readOrCreateProgress` делал POST (`get_or_create`), аддитивное слияние
  добавляло `completed: true`, и сброшенное прохождение воскресало вместе с
  «Пройден» в каталоге.
- **Почему не ловилось:** тест #1803 закреплял «сервер пуст → долить локальное»
  и не различал «строки ещё не было» и «строку удалили». Сценарий из двух
  устройств с удалением между ними не покрывал ни один тест.
- **Отличие от соседних семей:**
  - #1922 закрыл воскрешение через дисковую очередь ТОГО ЖЕ устройства;
  - #1906 — чужой квест, а не чужое поколение того же квеста;
  - #1434 — реконсиляция первопроходца на сервере при удалении, клиентской копии не касается.
- **Controls:**
  - `__tests__/utils/questProgressMerge.test.ts` › «поколение прохождения (#2033)»;
  - `__tests__/api/quests.test.ts` › «поколение снапшота (#2033)» — ни POST, ни задачи на удалённой строке;
  - `__tests__/utils/questProgressQueue.test.ts` › «копия сброшенного прохождения не воскрешает его»;
  - `__tests__/hooks/useQuestProgressSync.reset.test.ts`;
  - `__tests__/hooks/useQuestWizardProgress.test.tsx` › «поколение прохождения (#2033)»;
  - `__tests__/routes/quest-screen-progress-key.test.tsx` › «состояние сервера для визарда»;
  - `__tests__/hooks/useQuestProgressSync.catalog.test.ts` › «confirmed missing progress drops a stale completion mark» — «Пройден» в кэше не переживает стёртую копию;
...
### QUEST-PROGRESS-RESET-GUEST-LOCAL-001 — гостевой «Сбросить» не стирает гостевую копию

- **Ключ на борде:** #2047; под-ключ семьи QUEST-PROGRESS-RESET-LINEAGE-001.
- **Инвариант:** «Сбросить» у гостя стирает обе локальные копии прохождения:
  копию визарда (`guest_<questId>`) и гостевую `guestQuestProgress:v1:<questId>`.
  Гостевая копия — источник `initialProgress` при повторном открытии и вход
  миграции после логина, поэтому пережившие сброс ответы возвращаются в обоих
  местах.
- **Подтверждённая причина:** гостевая ветка экрана не передавала визарду
  `onProgressReset`, а `resetProgress` засевает пустое состояние с
  `markSeeded=true`, и оно не уходит в `persistGuestProgress`. Слияние при
  повторном открытии аддитивно (`mergeQuestProgress`), так что пустая копия
  визарда старые ответы гостевой копии не перевешивает.
- **Surface/owner:** `components/quests/useGuestQuestFlow.ts` →
  `resetGuestProgress` (стирает гостевую копию и `guestInitial`);
  `app/(tabs)/quests/[city]/[questId].tsx` — гостевые ветки web и native
  передают его как `onProgressReset`. Нового хранилища нет.
- **Controls:**
  - `__tests__/components/quests/guestQuestFlow.test.tsx` › «useGuestQuestFlow reset (#2047)»;
  - `__tests__/routes/quest-screen-progress-key.test.tsx` › «гостевой «Сбросить» стирает и гостевую копию прохождения (#2047)».

### QUEST-PROGRESS-GUEST-MIGRATION-RETRY-001 — миграция гостя после входа — одна попытка на открытие экрана

- **Ключ на борде:** #2048; под-ключ семьи QUEST-PROGRESS-RESET-LINEAGE-001.
- **Инвариант:** упавшая миграция гостевого прохождения повторяется без
  повторного открытия квеста, теми же будильниками, что и обычная отправка
  прогресса (#1922): старт, возврат в приложение, появление сети, вход, бэкофф.
  Ответы при этом живут ровно в одном месте.
- **Подтверждённая причина:** миграция в `useGuestQuestFlow` — эффект экрана с
  одной попыткой. При сбое она сбрасывала `migratedRef` и ждала следующего
  маунта; в `useQuestProgressQueueRuntime` её не было.
- **Surface/owner:** `components/quests/useGuestQuestFlow.ts` — при сбое
  гостевой снапшот уходит в `enqueueQuestProgress` с id вошедшего, гостевая
  копия стирается, очередь будится. Доставку делает `pushQuestProgressSnapshot`
  (`utils/questProgressQueue.ts`): то же слияние через `withQuestProgress` и
  то же ожидание удаления строки (#2043). Нового механизма нет. Без `userId`
  (сессия ещё не подтянулась) копия остаётся гостевой, как раньше.
- **Запись очереди — не подмножество снапшота экрана:** экран вошедшего
  стартует от сервера и гостевых ответов не знает. Поэтому после его успешной
  отправки запись квеста снимается только если ответ сервера её покрывает
  (`dequeueDeliveredQuestProgress` + `isQuestProgressCoveredBy`: ответы,
  попытки, подсказки, отметки; курсор и карта не в счёт), а непокрытую очередь
  дожимает сразу. Безусловный `dequeueQuestProgress` остаётся только для сброса
  и чужого поколения (находка код-ревью #2048).
- **Почему копия стирается:** две копии одних ответов — гостевая и в очереди —
  после доставки и последующего «Сбросить» в аккаунте залили бы старые ответы
  в новое прохождение при следующем открытии квеста.
- **Controls:** `__tests__/components/quests/guestQuestFlow.test.tsx` ›
  «упавшая миграция доезжает будильником очереди прогресса без повторного
  открытия квеста» и «не сливает гостевые ответы в строку, удаление которой ещё
  ждёт сети» (ответы ждут в очереди), «отправка экрана вошедшего не снимает с
  очереди непокрытые гостевые ответы»; `__tests__/utils/questProgressQueue.test.ts`
  › «доставка экраном снимает запись, покрытую ответом сервера, и оставляет
  непокрытую».
...
- **Controls:**
  ...
  4. `#2086` (замечание ревью `#2084`, ранее тот же пробел для `services`
     в `#1552`): `SOURCE_DIRS` в `bundle-composition.test.ts` охватывает также
     `api`, `context`, `i18n` целиком, `ui`, `config`, `styles`, `types` и
     `metro-stubs`. Контроль проверяет присутствие реальных файлов из этих
     каталогов, включая вложенный модуль локали, в результате обхода.
     Прежний guard пропускал запрещённый импорт из `api`, хотя такой же импорт
     из `utils` находил: причина — неполный список входных каталогов, а не
     детектор импортов. Контроль приёмки — временный импорт `leaflet` в `api`
     должен падать с путём нарушителя; чистое дерево должно проходить.
...
- **Корректирующий слой (#2036, 23.09.2026):** 48 мест в 26 файлах (40 `':hover'`,
  3 `':active'`, 4 `':focus'`, 1 `':focus-visible'`). Интерактивному элементу отклик
  возвращён состоянием `hovered` у `Pressable` (`IconButton`, `Chip`, кнопки действий
  карточки автора, стрелки и удаление кадра галереи, чипы, стрелки навигации и др.;
  `':active'` — `pressed` под курсором мыши) или правилом `app/global.css` на маркере
  `dataSet` под `(hover: hover) and (pointer: fine)` (строка и иконки списка точек,
  зона загрузки галереи, «Отменить» прогресса). Неинтерактивный блок и ключ, равный
  базовому виду, сняты; `':focus'` полей ввода снят — фокус держит
  `input/textarea:focus-visible` в `app/global.css`, кольцо skip-ссылки перенесено на
  её состояние фокуса. Захардкоженные цвета задуманных откликов заменены токенами или
  не перенесены.
- **Regression control:** правило `pseudo-class-style-key` в `guard:web-style-channels`
  (`lint`, `lint:ci`, `check:fast`) — красное на ключе `:x`/`::x`/`&:x` в любом объектном
  литерале и поле типа, долг `KNOWN_PSEUDO_CLASS_STYLE_KEY_DEBT` пуст; фикстуры в
  `__tests__/scripts/guard-web-style-channels.test.ts`. Настоящий RNW DOM —
  `__tests__/components/ui/hoverResponse.dom.web.test.tsx`: `IconButton`, `Chip`, кнопки
  карточки автора и кадра галереи меняют computed style под мышью и не меняют от
