...
- **Цепочка evidence:** `#1702` (избыточные ссылки на перекрытые стили у
  ...
  против `radii.xl` без них). Четыре карточки за один день, и каждый раз аудит
  предыдущей находил «ещё один» дубль вне своего scope.
...
- **Controls:** `__tests__/components/travel/travelDetailsStyleKeyOwnership.test.ts`
  ...
  переобъявления. Проверен на искусственно
  заведённом дубле. Первый же прогон нашёл два семейства сверх `#1708`: семь
...
- **Пробел:** причина не снята, снят только её сигнал. Пока у экрана три набора,
  ...
  тогда дублировать становится негде. Открытый долг, зафиксированный ревью
  `#1708` и пришпиленный в гейте как `KNOWN_OPEN_DUPLICATES`: восемнадцать имён
  shell-набора продублированы в агрегате (тринадцать в
  `styles/travelDetailsLayoutStyles.ts` — `wrapper`, `safeArea`,
  `mainContainer(Mobile)`, `sideMenu*`, `scrollView`, `scrollContent`,
  `contentOuter`, `contentWrapper`, `sectionTabsContainer`; пять в
  `styles/travelDetailsMiscStyles.ts` — `error*`). Копии уже разошлись
  (`sideMenuBase`: shell `surfaceMuted`/`dashed` против layout
  `surface`/`solid`; `scrollContent` на web: shell
  `calc(max(var(--mt-dock-h)…))` против layout `spacing.lg`), а читает эти
  имена только shell-набор — контейнер, скролл-рантайм, `useTravelDetailsLayout`
  и экраны ошибок. То есть копии в агрегате мёртвые, как и снятые в `#1708`;
  снимать их — отдельная карточка, файлы вне scope `#1708`.
  Отдельно: `heroOverlay`/`heroTitle`/`heroMeta` в
  `styles/travelDetailsHeroMediaStyles.ts` после `#1708` остались в одном
  экземпляре, но не читаются ни одним потребителем — дубля больше нет, мёртвый
  код есть. Гейт ловит дубли имён, а не мёртвые ключи: у него нет обзора
  потребителей, принимающих `styles: any`.
...
- **Последняя проверка:** 2026-09-02; `#1708` закрыт, гейт зелёный на текущем
  дереве.
...
- **Controls:** деление `count` на фактическую длину первой страницы (образцы —
  `stores/travelStatusStore.ts` `fetchAuthoredTravelStatusEntries`,
  `api/quests.ts` «размер страницы берём фактический, а не запрошенный»);
  фолбэк на кэш вместо пустого списка при отказе одной из страниц.
- **Пробел:** логика докачки продублирована уже трижды. Общий FE-хелпер «собрать
  все страницы по `count`» не выделен — это отдельная задача, иначе четвёртый
  потребитель повторит ошибку.
