...
- **Цепочка:** `#134`, `#152` (done, /places), `#1487` (route-карточки),
  `#1541` (hero-слайдер), `#1542` (квест-плитки).
...
- **Треугольник несовместимости:** «поле ≤10% на всех обложках» + «contain» +
  ...
  curated-набора (#1541).
- **Controls:** `cardMediaLetterbox.test.ts` (единый слот + потолок поля),
...
- **Пробел:** нет guard'а, заставляющего новую `contain`-поверхность объявить
  контракт пропорций (слот от контента / фиксированный кадр с тестом на
  реальном распределении); hero и квест-плитки не покрыты авто-тестом на поле.
- **Решение для новой жалобы:** полоса на route-карточках → `reuse #1487`;
  hero → `#1541`; квест-плитки → `#1542`; новая поверхность → create-linked
  с собственной подтверждённой причиной, точечный фикс без записи сюда запрещён.
...
- **Цепочка:** #1190 (правило введено, целевого прогона по `hint` не делала) →
  ...
  измерен, 18 утечек переписаны, остаток закрыт baseline).
...
- **Controls:** `node scripts/scan-quest-hint-leak.js` — сверяет каждое
  ...
  известной. Набор поверхностей — `--scopes=step,intro,finale`, по умолчанию
  (#1488) `step` и `intro`; остаток по интро удерживает
  `--baseline=scripts/quest-hint-leak-baseline.json`.
...
- **Вторая поверхность: текст УРОВНЯ КВЕСТА — измерен и взят под гейт (#1488).**
  ...
  - **Финал в умолчание НЕ входит, и это не долг.** Замер 23.08.2026: 106 находок
    ...
    `--scopes=finale`.
- **Открытый риск 4: третья поверхность — `quest.title`, скан её не читает.**
  Название квеста — самостоятельный ключ бандла, НЕ `intro.title`:
  `loadFromApi` (`scan-quest-hint-leak.js:374-383`) собирает
  `{id, quest_id, intro, finale, steps}` и `title` туда не копирует, поэтому
  максимальный прогон `--fields=hint,story,title,task,location
  --scopes=step,intro,finale` по такому квесту молчит. Найдено ревью #1488:
  `minsk-cmok` называется «Квест по центру Минска: Свислочский цмок» при ответе
  шага `2-sun-memory` «свислочь», `batumi-golden-fleece` — «по следам золотого
  руна» при ответе шага `3-medea` «руно». Разовая проба тем же matching-кодом по
  152 квестам прода: 14 буквальных совпадений; `batumi` в них НЕ попал —
  словоформа «руно» → «руна» короче порога `answerStem`, значит масштаб ≥14.
  Поверхность шире визарда: тот же текст уходит в `<title>`/`og:title` через
  `buildQuestSeoMetadata` (`utils/questSeo.js:81-117`), то есть в выдачу и в
  превью репостов, и гейтом `isMobile` не закрыт — в отличие от шапки визарда
  (`questWizardShell.tsx:505-514`). Отдельная карточка.
  - **Структурный вывод.** Это пятое точечное расширение одного скана в
    ...
    код-ревью СЛЕДУЮЩЕЙ карточки, а не прогон. Причина повторяемости —
...
- **Механически ловимый срез семантического класса (2026-08-23, #1536).** До
  ...
  2026-08-23 — 9 и 12. Команда: `.quest-audit/scan-compound-spelling-gap.py`
  (разовый; оформление постоянным `scripts/scan-quest-compound-spelling-gap.js`
  — в #1536).
