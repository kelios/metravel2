...
- **Граница класса:** общее правило `docs/RULES.md` → «Web loading and
  hydration policy» — секции грузятся сразу, без гейта по видимости. Контроль 2
  живёт по узкому исключению оттуда же (решение владельца 21.09.2026): ждать
  видимости могут только данные ПОСЛЕДНЕЙ секции страницы, под которой нечего
  сдвигать. Поэтому вкладка «Уровень» профиля (`useQuestCityCollections`),
...
- **Повторение:** тот же fan-out на детали переоткрывает `#1992`; новый
  ...
  видимости может только последняя секция страницы по исключению «Web loading
  and hydration policy», остальные грузятся сразу (решение владельца
  21.09.2026, `#2010`; расхождение правила с главной и деталями путешествия —
  `#2012`).
