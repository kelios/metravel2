...
- **Открытый риск.** Скан закрывает ТОЛЬКО буквальный класс. Семантический
  ...
  же семейства через соседнее поле — правка hint её не снимает.
...
- **Последняя проверка:** 2026-08-18 — скан по всему проду (142 квеста, 649
  шагов `exact_any`, 5484 варианта) даёт 6 blocker'ов класса `mixed_script`
  (шаги 130, 147, 520, 647, 1313, 1379; правка — #1455), ноль непроходимых шагов
  и 5 `case_form_gap` (шаги 119, 371, 416, 930, 1212). Контроль на здоровой
  позиции: из 21 шага с вопросом «какого … цвета» 15 родительный падеж уже
  держат, починенный шаг 791 `yelnya-bog-bells` находок не даёт, а дефисные
  формы вида «SPA-центр» подменой не считаются — `normalize()` дефис срезает,
  игрок нет. Находка шага 130
  подтверждена рантаймом: `buildAnswerChecker` на словаре из
  `GET /api/quest-steps/130/` отвергает белорусское «гадзіннік» (U+0456) и
  принимает только «гадзiннiк» с латинской `i` (U+0069).
...
- **Последняя проверка:** 2026-08-18 — механизм установлен и записан в #1453,
  правка не применялась: выбор между расширением словаря до семейства и
  переписыванием `task`/`hint` — решение владельца.
