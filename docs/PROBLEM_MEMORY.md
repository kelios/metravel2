...
- **Контроль:** ключ точки — её координаты, id у карты нет. Перетаскивание
  меняет ровно одну точку, поэтому пересечение с прежним набором не пустеет;
  для единственной точки маршрута ключи защёлки перезаписываются уже местом
  дропа, иначе она сняла бы себя сама. Regression: оптовая замена возвращает
  подгонку, перетаскивание единственной точки — нет, на обеих картах
