...
- **Цепочка:** #1190 (правило введено, целевого прогона по `hint` не делала) →
  ...
  формата) → #1467 (правка контента 17 шагов с утечкой в подписи).
...
- **Controls:** `node scripts/scan-quest-hint-leak.js` — сверяет каждое
  ...
  находка в них означает НОВУЮ утечку. `title`/`story`/`task` в gate не входят:
...
- **Открытый риск 2: соседние поля того же шага — `location` закрыт, три поля
  ...
  разбор.
- **Граница нулевого свипа: интро квеста скан не смотрит.** «0 находок» по проду
  относится к полям ШАГОВ: `scanQuests` обходит `quest.steps`
  (`scan-quest-hint-leak.js:117-121`), а интро приходит отдельным ключом бандла
  (`scripts/lib/questBundles.js:78`). Найдено ревью #1467: интро
  `sofia-serdica-underfoot` называет «ротонду» (ответ шага 677), интро
  `brest-lantern` — «гигантскую пуговицу фонарщика» (ответ шага 538), то есть
  путь к ответу до попытки остаётся открытым мимо вычищенной подписи. Отдельная
  карточка.
