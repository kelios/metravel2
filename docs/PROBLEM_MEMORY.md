...
- **Recurrence Log — 2026-09-20 (`#1569`, реопен — впервые сработал сам
  ...
  20.09.2026: гвард разделён на два случая — сборка падает только когда у города
  не пришёл ни один бандл (`assertQuestCityLandingBundlesResolved`,
  `questCityWalkBundleCount`), а город с бандлами без собственных заметок
  собирается под build-owned `noindex, follow` (`selectIndexableQuestCityLandings`,
  зеркало правила страны #1929); лог сборки называет такие города поимённо,
  ...
  (`selectIndexableQuestDetailPages`, то же правило `scripts/lib/questPageDepth`),
