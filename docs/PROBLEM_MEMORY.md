...
- **Решение:** create-linked #1808; реализация ещё не выполнена. Норма UI —
  `docs/design/contact-feedback-success.md`. Требуемый постоянный контроль:
  оба success shape на пяти локалях с реальным i18n, unknown passthrough,
  смена локали и отсутствие ложного успеха на 451; оба потребителя показывают
  нормализованный текст. Повторное production-письмо требует новой команды:
