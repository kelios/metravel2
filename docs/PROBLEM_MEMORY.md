...
- **Решение для новой волны 404:** мёртвые адреса найдены отчётом и это только
  доливка алиасов — `create-linked` к `#1249`; отчёт молчит, а адрес мёртв —
  чинить сам отчёт, это регресс control'а, а не новая волна.
...
- **Подтверждённая причина:** body-санитайзер SSG держит узкий allowlist тегов и
  ...
  попадали в 160-символьное окно.
...
- **Решение для новой жалобы:** пропала разметка из тела статьи — `reopen #1138`;
  ...
  затем `create-linked` к `#1139`; другой тип страницы — `create-linked`.
...
- **Последняя проверка:** `#1138`/`#1139`/`#1140` — 2026-07-29.
