...
- **Structural risk:** второе совпадение из трёх. Эскалация налицо: локальный
  красный lint → артефакт на `origin/main`. Третий эпизод обязывает к
  структурному заходу — общей политике «что вообще может лежать в корне».
- **Последняя проверка:** 2026-09-07; файл удалён из индекса, guard зелёный,
  проба нового `.codex-temp*` в корне не видна ни `git status`, ни eslint.
