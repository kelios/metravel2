...
После клонирования на macOS, Windows или Linux достаточно скопировать эти файлы
в те же пути и запустить `npm run android:release:doctor`. Keychain на новом
компьютере не нужен. Четыре `METRAVEL_ANDROID_KEYSTORE_*` variables остаются
разрешённым override для CI. Старые Keychain services
`metravel-android-upload-store-password` и
`metravel-android-upload-key-password` используются только как legacy source при
создании bundle на основном Mac.
...
После копирования portable `.secrets` bundle:
...
Обновление closed/internal testing выполняется только по отдельной явной команде
владельца и отдельным инструментом; production-only script для этого не
расширяется:

```bash
npm run android:submit:testing:latest
npm run android:submit:testing
```

Первый вызов загружает AAB, обновляет внутри временного edit ровно `alpha` и
`internal`, проверяет неизменность `production`/`beta`, валидирует и удаляет edit.
Второй выполняется только после зелёного dry-run и commit'ит ту же allowlisted
пару. Инструмент не вызывает endpoints тестировщиков или стран.
...
Для build-only операции на любом компьютере используй
`$metravel-android-portable-builder`; для Play submit/track —
`$metravel-google-play-operator`. Общие gates находятся в `docs/RELEASE.md` и
