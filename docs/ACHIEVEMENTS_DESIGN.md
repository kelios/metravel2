...
- Визуал значков: **векторные гравюрно-книжные эмблемы** во фронте
  ...
  _(Прежнее направление «премиум объёмные металлик-медали» отменено владельцем 2026-06;_
  _AI-промпт-спека `ACHIEVEMENTS_BADGE_PROMPTS.md` оставлена лишь как опциональный фоллбэк_
  _на случай, если бэк начнёт отдавать `image_url`.)_
...
### Share-карточки достижений (Sprint C, РЕАЛИЗОВАНО на origin/master)

Серверная генерация share-карточки медали (PNG + OG-страница) и публичный URL достижения.
...
Дефолтный визуал значка теперь **векторная гравюрная эмблема** во фронте
(`BadgeEmblem`), а не AI-картинка: `image_url=null` — это нормальное штатное состояние,
а не временный фолбэк. AI-генерация металлик-медалей отменена как дефолт (см. §устар.
`ACHIEVEMENTS_BADGE_PROMPTS.md`); если бэк всё же отдаст `image_url`, `BadgeMedal`
отрисует картинку поверх. UI не ломается ни при `null`, ни при `pending`/`failed`.
...
- `api/achievements.ts` — типы `Badge`, `UserBadge`, `UserRank`, `BadgeProgress`,
  `AchievementsResponse`. `UserBadge` несёт `id` (PK записи разблокировки) — он, а не
  `badge.id`, передаётся в `achievement_id` share-карточки (см. ⚠️ в §5).
