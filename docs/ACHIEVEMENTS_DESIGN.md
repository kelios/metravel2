# MeTravel Achievements & Ranks — текущий контракт
...
Статус: frontend adapters, React Query hooks, achievements/rank/peer/rare UI,
profile/public-profile/AuthorCard integrations и tests реализованы. Production
readiness остаётся backend-dependent и подтверждается реальными API
payloads/mutations; development mocks включаются только явно.

Решения (зафиксированы с владельцем 2026-06-20):
- Механика: **значки + ранг (XP)**.
- Визуал значков: **векторные гравюрно-книжные эмблемы** во фронте
  (`components/achievements/BadgeEmblem.tsx`, `react-native-svg`) — состаренная бумага,
  тонкая линия, лента-баннер, рамка тира (bronze→legendary кодируется сложностью рамки).
  Optional backend `image_url` поддерживается как media override, но не является
  обязательным условием отображения значка.
...
## 1. Frontend DTO и ожидаемый backend contract

Backend находится в отдельном репозитории и в этом workspace не редактируется.
Схема ниже объясняет DTO фронта; production contract сверяется read-only probes
и `api/achievements*.ts`.
...
- category id/slug/name/icon, nullable;
- `tier`: `none|bronze|silver|gold|platinum|legendary`;
- optional `image_url` и `image_status`; если URL отсутствует или невалиден,
  frontend показывает `BadgeEmblem`, а не fake media URL;
- `award_type`, `target`, `points`, `is_secret`, `order`;
...
- `criteria_params`: backend-side параметры порога/привязки, напр. `{"threshold": 5}` или
...
## 5. API (DRF, роутер `achievements`)

Frontend adapter contract реализован. Наличие кода в backend branch не заменяет
...
- `GET /api/achievements/me/` (Token) →
  ...
  - `rank` = `{level, title, total_points, badges_count, current_level_min_points,
    next_level_min_points, next_level_title, is_max_level, progress_ratio,
    remaining_points, recomputed_at}` — **#721: rank-progress summary отдаётся готовым
    (кэш)**, FE читает пороги/прогресс напрямую, ничего не считает.
  - `rank_levels` = `[{level, title, min_points}]` — оставлен для совместимости; FE
    использует его только как legacy fallback, если в rank нет summary-полей.
...
- `GET /api/achievements/user/{user_id}/` — публично:
  `{ rank, rank_levels, earned_badges, peer_received }`. **#721: rank теперь содержит
  тот же rank-progress summary** → публичный RankBar показывает XP-полосу (раньше был
  без порогов).
- `GET /api/user/{pk}/profile/` может отдавать optional `rank_summary`; frontend
  нормализует его через общий `mapRank` для первого paint и сохраняет fallback на
  achievements-query, если поле отсутствует.

### Share-карточки достижений

Frontend adapter и share UI реализованы. Перечисленные ниже server endpoints —
ожидаемый backend contract; production readiness требует runtime evidence.

- `POST /api/achievements/share-cards/` (Token) — создать/переиспользовать карточку.
  Тело `{ achievement_id, template?: 'default'|'rare', utm?: {source, medium, campaign} }`
  → `201 { share_token, image_url, public_url, expires_at }`.
- `GET /api/achievements/share-cards/{share_token}/` — публично: `{ share_token,
  achievement, image_url, public_url, og: {title, description, image} }`.
- `GET /api/achievements/share-cards/{share_token}.png` — публично: `200 image/png`
  с `Cache-Control`/`ETag`.
- `GET /api/achievements/public/{public_slug}/` — публично (JSON): `{ public_slug,
  achievement, owner_profile, share_card_image_url, canonical_url, visibility }`.
- `GET /achievements/{public_slug}/` — публичная HTML/OG-страница, рендерит Django
  (не SPA — маршрут заведён в nginx до фронтового фолбэка). Содержит achievement-specific
  `og:title/description/image` + canonical.
- Ошибки: `400` неверный template/UTM, `403` достижение не шарится текущим юзером,
  `404` приватное/не найдено (анонимам), `410` истёкший/отозванный токен.

> ⚠️ **Контракт `achievement_id` (источник реального бага FE):** это PK **записи о
> разблокировке**, а НЕ id определения значка. Для `template='default'` →
> `UserBadge.id` (поле `id` элемента `earned_badges[]` из `GET /api/achievements/me/`,
> НЕ `earned_badges[].badge.id`). Для `template='rare'` → `RareAwardGrant.id`. Передача
> каталожного `Badge.id` даёт `403 «achievement not shareable»`. Поэтому FE-тип
> `UserBadge` обязан нести `id` записи (см. §8), и именно он уходит в `achievement_id`.

---

## 6. Backend admin expectations

- `BadgeAdmin`: `criteria_type`+`criteria_params`, `points`, `tier`, `category`,
  `is_active`, `order`, optional image status/preview. Конкретные admin actions и
  generation pipeline не считаются frontend contract и проверяются в backend.
- `BadgeCategoryAdmin`, `RankLevelAdmin`.
- `UserBadge` — readonly inline/list для аудита выдач.

## 7. Badge catalog and media

Дефолтный визуал — векторная гравюрная эмблема `BadgeEmblem`: `image_url=null`
является штатным состоянием. Если backend отдаёт валидный `image_url`,
`BadgeMedal` показывает media через shared image layer; `pending`/`failed` не
ломают UI и не заменяются fake URL.

Новый raster art создаётся только отдельной asset-задачей через
`$metravel-visual-asset-designer` с воспроизводимым prompt contract и явным
backend upload/persistence scope. Отменённая metallic-medal prompt-спека удалена.

Стартовый catalog включает:
...
- `api/achievementsTypes.ts`, `api/achievementsNormalizers.ts`,
  `api/achievementsRequests.ts` — разделённые contracts. `UserBadge` несёт `id`
  записи разблокировки: он, а не `badge.id`, передаётся в `achievement_id`
  share-карточки (см. ⚠️ в §5).
- query keys централизованы в `api/queryKeys.ts` (`achievementsBadges`,
  `achievementsMe`, `achievementsUser` и связанные rare/peer keys).
- `api/achievementsShare.ts` — `createShareCard({ achievementId, template?, utm? })` →
  `{ shareToken, imageUrl, publicUrl, expiresAt }` (DEV mock-folбэк на 404/501).
- `utils/achievementShare.ts` — `buildShareLink` (навешивает `utm_source/medium/campaign`
  на `public_url`, без дублей, hash сохраняется) и `buildShareUtm` (UTM в тело create).
- `utils/gamificationAnalytics.ts` — события `trackBadgeShareOpened` (`badge_share_opened`),
  `trackBadgeShared` (`badge_shared`), `trackShareCardClick` (`share_card_click`); payload
  без PII (achievement id/slug/channel/template/is_rare/context).

### Компоненты `components/achievements/`
- `BadgeMedal.tsx` / `BadgeEmblem.tsx` — default vector emblem, optional media
  override, tier frame и earned/locked state. Media рендерится через shared image
  layer; прямой feature-level `expo-image` запрещён.
...
- `AchievementsGalleryModal` — полная галерея с группировкой по категориям и прогрессом;
  для earned-значка кладёт `userBadgeId` (`UserBadge.id`) в `BadgeDetail`.
- `BadgeUnlockToast.tsx` — «Новый значок!» после действия (свой профиль).
- `BadgeDetailSheet.tsx` — карточка значка + кнопка «Поделиться» (видна только при
  `earned && ownerName && userBadgeId != null`); `subject.achievementId = userBadgeId`.
- `ShareBadgeSheet.tsx` — лист шаринга: каналы copy/native share/TG/FB/«сохранить картинку»,
  все внешние ссылки только через `@/utils/externalLinks`; зовёт трекеры аналитики.
- `ShareCardPreview.tsx` — превью карточки: `RegularFrame` vs `PremiumFrame` (редкая
  награда: tier-градиент + лента «Редкая награда»), переключение по `subject.isRare`.
...
- Frontend implementation не подтверждает deployment backend endpoint.
- Mock используется только с явным development flag.
- Missing fields/endpoints становятся `area=back` task с Task Contract; frontend
  не добавляет permissive fake-success fallback.
- Production acceptance включает profile, public profile, AuthorCard,
  peer/rare mutations, empty/loading/error states и network evidence.
...
Визуал по умолчанию — `BadgeEmblem`; optional backend media подчиняется контракту
раздела 7.
