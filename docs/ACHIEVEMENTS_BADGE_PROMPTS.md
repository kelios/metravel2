...
> **АРТ-НАПРАВЛЕНИЕ СМЕНЕНО (2026-06, решение владельца).** Дефолт значка теперь —
> **векторная гравюрно-книжная эмблема, отрисованная во фронте** (`components/achievements/BadgeEmblem.tsx`,
> `react-native-svg`): состаренная бумага + тонкая линия (woodcut/engraving) + лента-баннер +
> рамка-индикатор тира (bronze→legendary кодируется сложностью рамки, не «блеском»). Мотив —
> иллюстрированная сцена под значок (гора+компас, волна+якорь, глобус+маршрут, …), резолвится
> `badgeMotif(category, slug)`. Спека «premium 3D glossy metal medal» ниже **больше не
> актуальна как дефолт** и оставлена только как опциональный AI-фоллбэк, если бэк когда-нибудь
> начнёт отдавать `image_url` — `BadgeMedal` отрисует картинку вместо векторной эмблемы.
...
и шлёт в выбранную image-модель.
