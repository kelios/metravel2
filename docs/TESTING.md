...
- `guard:themed-colors` enforces the theming rule from `docs/RULES.md` → «UI rules»: a themed surface reads its color through `useThemedColors()` (`hooks/useTheme.ts:199`), not directly from `DESIGN_TOKENS.colors.*`. `constants/designSystem.ts` builds a token as `Platform.OS === 'web' ? var(--color-…, fallback) : fallback`, so on web it stays a live CSS variable that `html[data-theme="dark"]` overrides, while on native the light literal is frozen into a module-level `StyleSheet.create` that a runtime theme switch never recomputes — the screen silently stays light on Android and iPhone. The scan is AST-based over `app/`, `components/`, `hooks/` and `utils/` — `utils/passwordStrength.ts` hands its color straight to the RN password indicator, so the defect does not stop at `components/`. Comments and strings do not count, while dynamic access (`DESIGN_TOKENS.colors[key]`), an aliased import (`DESIGN_TOKENS as DT`), a namespace import, a local re-alias of the root (`const alias = DESIGN_TOKENS`) and destructuring the `colors` sub-object (`const { colors } = DESIGN_TOKENS`) all do. An alias is followed only while its name is declared once in the file: a file that both aliases the tokens and shadows the name with its own `const colors = useThemedColors()` is charged for the alias declaration alone, never for the read that already follows the rule. A read is a violation only when it is fixable: a themed counterpart exists in `getThemedColors()` and its light value differs from its dark one. The counterpart is matched by palette entry rather than by name — `error` and `card` are built from the same `MODERN_MATTE_PALETTE` entries as the themed `danger` and `surface`, so a name-only lookup would have excused them — and the failure output names the replacement (`-> useThemedColors().danger`). Tokens without a themed counterpart (`travelPoint`, `mapPin`, `bookPage*`) and theme-invariant ones (`textOnDark`) are exempt, and `*.web.*` files are excluded because the token there is always the live variable. Both exemptions are derived from the design-system source rather than an allowlist, so the guard tightens itself: the moment a token gains a diverging themed counterpart, its existing direct reads become violations over the baseline. Legacy debt lives in `scripts/themed-colors-baseline.json` per file; the baseline is never rewritten outside `guard:themed-colors:update`, which is reserved for a reviewed reduction, not for absorbing a new violation;
...
- `guard:web-deferred-loading` enforces `RULES-WEB-LOADING-POLICY-DRIFT-001` (#2012): on web a page section may wait for scroll visibility, a timer or idle only in a file named by the registry in `docs/RULES.md` → «Web loading and hydration policy». It reads web app sources (not tests, not `*.native|ios|android.*`) through the shared `scripts/lib/maskSource.js` scanner, so comments and string contents do not count, and fails on: a file with an `IntersectionObserver` (construction, alias, type or `typeof` check), a `useProgressiveLoad` call or any other reference to the hook (an import under another name, an alias) or a `<DeferredSection>` that is missing from `APPROVED_DEFERRED_LOADING`; a changed number of deferral sites in an approved file; an approved file with no site left; any difference between that allowlist and the registry entries; an empty scan. It runs in `lint`, `lint:ci` and unconditionally in `check:fast`, because a new deferred section arrives in a new file (#1475, #818, #2010). It cannot see a deferral built only on `setTimeout`/`requestIdleCallback`, a new key inside an already approved shared observer, or a new wrapper component around the hook; review checks those against the registry;
- `guard:web-style-channels` enforces `RNW-RAW-DATA-ATTRIBUTE-DROPPED-001` (#2032): web styling reaches the page through two channels only. A DOM attribute on a react-native-web component goes through `dataSet` (`dataSet={{ fooBar: 'x' }}` → `data-foo-bar="x"`); RNW forwards an allowlist of props and drops a raw `data-*`, while a react-test-renderer test still sees it. Web CSS goes into `app/global.css`: the web export emits only the stylesheet imported by `app/_layout.tsx`, and a `.css` required next to a lazily loaded component never reaches `dist`. The guard parses web app sources (not tests, not `*.native|ios|android.*`) with the TypeScript AST and fails on: a literal `data-…` JSX attribute or object key that reaches anything but a DOM element (a lowercase tag, `createElement('div', …)` or a lowercase tag variable) — an object whose way to an element it cannot follow (a constant, a return value, `Platform.select`) counts as well; a `data-…` key inside `dataSet`; an `import`, `require` or `import()` of a `.css` anywhere but `app/_layout.tsx` → `./global.css`, or that import missing; an empty scan. It also fails on a pseudo-class style key (`':hover'`, `':focus'`, `'::before'`, `'&:hover'`) in any object literal or type field (rule `pseudo-class-style-key`, #2036): RNW compiles such a key into an invalid rule, so hover goes through `Pressable`'s `hovered` or a `dataSet`-marked rule in `app/global.css` under `@media (hover: hover) and (pointer: fine)`. Sites from before a rule live in `KNOWN_RAW_DATA_ATTRIBUTE_DEBT` / `KNOWN_PSEUDO_CLASS_STYLE_KEY_DEBT` with exact per-file counts — both are empty since #2035/#2036: a new site in a listed file fails like one in a new file, a fixed site must lower or drop the entry. It runs in `lint`, `lint:ci` and unconditionally in `check:fast`. It does not see a computed key (`` [`data-${x}`] ``) or a marker passed under another prop name to a wrapper that spreads it; for those the proof is a rendered-DOM test (`__tests__/components/travel/CompactSideBarTravel.dom.web.test.tsx`, `__tests__/components/ui/hoverResponse.dom.web.test.tsx`);
- `guard:cli-contract` enforces `SEO-OPS-001`: every `scripts/seo-*.js`, `scripts/indexnow-*.js`, `scripts/index-status.js`, `scripts/test-seo-prod.js`, `scripts/post-deploy-seo-check.js`, `scripts/scan-quest-*.js`, `scripts/sync-quest-*.js`, `scripts/apply-quest-patches.js`, `scripts/migrate-quest-from-file.js` and `scripts/quest-answer-insights.js` parses its arguments through `scripts/lib/cli-contract.js`, runs through `runCli`, holds no hand-rolled flag lookup (`argv.includes('--all')`, `args.indexOf('--limit')`, `arg === '--json'`, `case '--json'`, `argv.find(a => a.startsWith('--source='))`, a locally aliased `argv.slice(2)`) and never exits zero explicitly (`process.exit(0)`, `process.exit()`, `process.exit(failed ? 1 : 0)`, `process.exitCode = 0`). Those shapes answer a typo with the widest default — that is how `#1389` submitted 544 URLs, how `#1325` reported a green "0 checked", and how `test-seo-prod.js --ur https://dev.metravel.by` used to check production instead. The covered set is walked from the filesystem under `scripts/` at any depth except `scripts/lib/`, so a new script of either family — or one moved into a subfolder — is guarded without editing an allowlist. `#1934` added the quest half after the same invariant broke there, in scripts that write to production: `sync-quest-to-prod.js --dryrun` was a live edit of quest content, because the misspelt flag was dropped and `--dry-run` defaulted to off. The quest half is matched by the two prefixes `scan-quest-`/`sync-quest-` plus three named tools, deliberately not by `quest` anywhere in the name — `scripts/` also holds ~60 one-off `migrate-<city>-quest.js` scripts and ~175 `*-quest-data.js` data modules, which are not CLIs. Since `#1934` the three production writers (`sync-quest-to-prod.js`, `migrate-quest-from-file.js`, `apply-quest-patches.js`) declare `modes: ['dry-run', 'apply']`, so neither spelling has a default: a run without an explicit mode is refused rather than guessed in either direction. The parser reads both `--limit 5` and `--limit=5`, so the flag NAME is checked in either spelling; `--dry-run=false` is refused outright, and free operands exist only where a script declares `positionals` (only `apply-quest-patches.js` does, for its `patches-*.json` glob). There is deliberately no skip-when-unclear path: an empty or unreadable covered file fails, and a scan that matched zero files fails too, because a clean report over nothing is the very defect this guard exists for. Since `#1398` a covered script also declares in its CLI spec what it works on — `selection: 'articles'` for a run over a list, `selection: 'none'` for one that takes a single named target such as `seo-edit --id 641`. A missing or empty declaration fails; a declared selection must call `requireNonEmptySelection()`, because rule four only catches the loud spelling of the green report and a script that simply returns from `main()` over an empty list still exits zero; and a `'none'` declaration next to a flag that means many targets (`--all`, `--ids`, `--limit`, `--map-file`, `--urls-file`) fails as a stale declaration. Since `#1655`, every declaration other than `selection: 'none'` must also make a real `requireNoBatchFailures(failed, { total, ... })` call after its final report or artifact flush. A call with a hard-coded zero does not satisfy the guard because it proves nothing about the processed batch. The helper returns only for valid non-negative integer counts with `failed <= total` and `failed === 0`; positive failures become `ExpectedFailureError`, while malformed counts fail closed as programmer errors. This closes the other green-report path: a loop catches per-item errors, prints `failed: N`, but exits 0. Every rule reads the file through one scanner that blanks what is not executable code, character for character, so line numbers still point at the source. Comments are blanked for every rule — whole-line, trailing or inline block — so a comment neither satisfies a requirement nor triggers a ban. String, template and regex contents are blanked for the call-presence rules — `contract-module`, `strict-parse`, `exit-contract`, `empty-selection-guard` and `batch-failure-guard`: a helper named in USAGE prose is not a call, while the same call inside a `${…}` interpolation still counts. `#1442` moved the first three off literal needles; forbidden-pattern rules still read literals because a real `argv.includes('--all')` is itself a string in code. The declaration is read only from a top-level `CLI_SPEC`: a `selection` nested in a flag config, a local `CLI_SPEC` inside a function, or one quoted in a string does not answer for the script;
