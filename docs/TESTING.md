...
- `guard:locale-number-format` enforces `LOCALE-NUMBER-FORMAT-001`: a displayed number goes through `i18n/format.ts` (`formatNumber`, `formatInteger`, `formatCompactNumber`, `formatCurrency`) or a domain wrapper over it (`utils/distanceCalculator.ts` for distance, `utils/ratingHelpers.ts` for a rating, `utils/fileSize.ts` for a file size), and its unit comes from a translation key. Two shapes fail: a `.toFixed(0..2)` result that reaches a display position — JSX text, a display prop, a translation argument, or through variables and formatter functions of the same file, followed to a fixpoint across plain expressions — and a computed number glued to a hardcoded unit or compact suffix (`K`, `M`, `тыс.`, `км`, `KB`), whether it came from `.toFixed()`, `Math.round()` or arithmetic; that suffix is user-visible text wherever it is written. Coordinates (`lat`/`lng`/`lon` receivers), precision above two digits, ISO-8601 durations (`PT45M`) and machine props (`points`, `style`, `testID`, `href`, `data-*`, event handlers) are excluded structurally rather than by allowlist, because a locale decimal comma would itself be the defect there. Known limit: the alias fixpoint does not cross a function or `useMemo` body — following those would call half a file displayed. `services/` joined the scanned roots in `#1465`: PDF export prints the same user-visible numbers as the screens do, and the extension surfaced exactly one call site. Four tickets in a row — `#1433`, `#1440`, `#1449`, `#1457` — fixed this same root in one more domain each; the guard exists so the fifth domain fails at development time. The governance suite keeps the allowlist empty and rejects a vacuous scan;
