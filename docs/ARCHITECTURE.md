...
Документ описывает фактическую архитектуру frontend/app из текущего `main` и
карту реализованных пользовательских областей. Он не является отчётом о
конкретном тестовом прогоне и не дублирует backlog: validation evidence и статусы
работ хранятся в MCP task board.
...
Текущий registry содержит RU/BE/UK/PL/EN. Web compile-time transform подставляет
компактные значения только для реально используемых ключей, а лёгкие
`i18n/*.web.ts(x)` сохраняют interpolation, plural rules и locale lifecycle без
доставки полных каталогов на каждую страницу. Native и Jest инициализируют полный
i18next resource contract. Canonical URL пока не имеет языкового префикса;
locale-specific URL/hreflang требует отдельного SEO-контракта.
...
| iOS/Android shell | implemented | readiness требует свежей installed-build проверки | Android USB и отдельный iOS simulator/device pass |
...
Проверка выбирается по изменённой области:

| Scope | Минимальное evidence |
| --- | --- |
...
Канонические правила закреплены в `docs/RULES.md`, тестах и governance scripts:

- работать на `main`, если пользователь явно не указал иначе;
- сначала переиспользовать существующие components, hooks, services и utilities;
- держать route files тонкими, а feature logic — в feature folders/hooks;
- использовать `DESIGN_TOKENS` и `app/global.css`, без feature-level hex;
- использовать Feather icons и нейтральные media placeholders;
- открывать внешние URL только через `utils/externalLinks.ts`;
- не возвращать service-worker runtime/static caching и UX «clear cache»;
- не маскировать backend/runtime gaps mock-success fallback;
- не выводить секреты из env, EAS, SSH или server configs.

## Карта документации

- `docs/README.md` — quick start и API family map;
- `docs/INDEX.md` — полный актуальный каталог документов;
- `docs/RULES.md` — обязательные правила;
- `docs/CODEX.md` — workflow, skills, triage и validation;
- `docs/DEVELOPMENT.md` — local setup;
- `docs/TESTING.md` — tests, governance и quality locks;
- `docs/RELEASE.md` и `docs/PRODUCTION_CHECKLIST.md` — release flow;
- `docs/features/*.md` — feature ownership maps;
- `docs/adr/*.md` — архитектурные решения.

Открытые работы и Done evidence находятся только на MCP task board.
