...
- Корень репозитория — закрытый список: корневой файл либо объявлен в
  `ALLOWED_ROOT_FILES` (`scripts/guard-root-scratch-artifacts.js`), либо
  подпадает под единственную конвенцию `.env*.example`. Гейт читает не только
  индекс, но и untracked-срез корня, поэтому разовая проба краснеет ещё до
  коммита; уносить её в `.codex-temp/` или игнорировать, а не дописывать в
  список. Штатный артефакт инструмента (`package-lock.json`, симлинк
  `node_modules`) закрывается `.gitignore`, тоже не списком. «Игнорировать»
