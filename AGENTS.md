...
3. После каждого логического шага запускай проверки по scope изменений:
   - для точечных изменений: только релевантные тесты/чеки, которые покрывают затронутую область;
   - для крупных изменений (сквозная логика, много модулей, инфраструктурные правки): полный прогон
     - `npm run lint`
     - `npm run test:run`
4. По завершению задачи:
   - для точечных изменений: обязательно запусти релевантные тесты/чеки по затронутому scope;
   - для крупных изменений: обязательно запусти полный прогон
     - `npm run lint`
     - `npm run test:run`

### 2.1 E2E окружение и доступы
...
- Запущены проверки по масштабу задачи:
  - точечные изменения: пройдены релевантные тесты/чеки в зоне правок;
  - крупные изменения: пройдены `npm run lint` и `npm run test:run`.
