...
3. Проверь текущую ветку и `git status --short`; если ветка не `main`, остановись и уточни дальнейшие действия.
4. Внеси минимально достаточные изменения.
5. Временную отладочную информацию складывай только в ignored-папки (`.codex-temp/`, `.codex-debug/`) и перед завершением удаляй всё ненужное:
...
6. Чини все реальные проблемы, найденные в ходе задачи:
...
7. После каждого логического шага запускай проверки по scope изменений:
