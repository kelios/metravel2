...
test.describe('Галерея на странице путешествия — мобильный touch @smoke', () => {
  test('свайп пальцем листает слайды, вертикальный жест скроллит страницу', async ({ page, context }) => {
  ...
  test('тап по фото открывает fullscreen-галерею, свайп листает, крестик закрывает', async ({ page, context }) => {
    await preacceptCookies(page);
    await mockFallbackTravelDetails(page);
    await gotoWithRetry(page, `/travels/${FALLBACK_TRAVEL_SLUG}`);
