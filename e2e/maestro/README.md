...
- Шапка деталки квеста — кнопки по `accessibilityLabel`. С #1669 на телефоне в самом ряду
  остались только масштаб шрифта и офлайн-загрузка, а сброс, GPX и «Открыть точки квеста в
  приложении карт» переехали в лист «Действия с квестом» (кнопка `more-horizontal`); печать
  в листе есть только на web.
  Флоу открывает лист перед каждым таким действием: он закрывается сам при выборе строки.
  Если добавите явные `testID` — заменить на `id:`.
