    ...
    .poll(
      async () => {
        const signals = [
          ...
          page.locator('[data-testid="travel-card-skeleton"], [testID="travel-card-skeleton"]').first(),
          page.getByText('Пока нет путешествий').first(),
          page.getByText('Ничего не найдено').first(),
          ...
          page.getByText('Результаты').first(),
        ];
