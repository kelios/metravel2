...
// Production-build baseline refreshed on 2026-08-24 after the intentional
// lazy/post-LCP route split: 96 budget-scoped requests (97 total, 88 scripts),
// 1049 KB JS and 1202 KB total. The previous be65c498 build measured on the
// same harness was 89 scoped (92 total, 82 scripts), 996 KB JS and 1149 KB
// total. Keep this request ceiling exact while the independent byte and Web
// Vitals budgets continue to catch payload or main-thread regressions.
const MAX_REQUESTS = envNum('PERF_MAX_REQUESTS', 96);
 ...
 *     старт. Прогретые проходы дешевле в 2–4 раза (замер 23.08 на одной
 *     сборке: 146–278 мс прогретых против 410–498 мс холодных в своём
 *     контексте; первый проход в ОБЩЕМ контексте доходил до 493–1175 мс),
 *     потому что переживают V8 compilation cache. Регрессия, добавившая парс/компиляцию в критический
...
test.describe('@perf Travel Details — Main-thread blocking (prod build, mobile throttled)', () => {
  // Блок идёт последовательно сам по себе; изолировать его от соседних блоков
  // файла нельзя — `mode: 'serial'` на уровне файла превращает падение любого
  // теста в скип остальных (проверено 2026-08-23: провал бюджета запросов
  // обрывал прогон до замера). Поэтому порог ниже калиброван при ПОЛНОМ
  // прогоне файла на `--project=chromium`, то есть под той же параллельной
  // нагрузкой, что даёт `npm run e2e:perf-budget`. `verify:slider-perf` в этот
  // режим не входит: он добавляет ещё два спека и второй проект, то есть грузит
  // машину сильнее, и его числа отдельно не снимались.
  test.describe.configure({ mode: 'serial' });

  // Ratchet по измеренному, а не назначенное число. Прод-сборка, стенд
  // `scripts/serve-web-build.js` (gzip, как в CI), CPU ×4, Slow-4G, медиана
  // пяти ХОЛОДНЫХ проходов. Замеры 2026-08-23:
  //   - изолированный прогон (`-g "Main-thread blocking"`): 416 / 417 / 449 мс;
  //   - ПОЛНЫЙ прогон файла, то есть штатный режим гейта, где соседние блоки
  //     занимают тот же CPU: 465 мс (проходы 489 / 461 / 443 / 465 / 526);
  //   - под посторонней нагрузкой (параллельная prod-сборка): 561 мс.
  // Потолок 650 мс держит запас над штатным режимом и загруженной машиной и
  // всё ещё ловит полуторакратный рост блокировки.
  //
  ...
  // Нижняя граница выборки — 5, как и в контракте замера (§7.1 доки): порог
  // откалиброван медианой пяти проходов, и медиана трёх с тем же потолком
  // сравнима с ним не была бы. Ужесточить (взять больше проходов) можно.
  const RUNS = Math.max(5, envNum('PERF_MOBILE_THROTTLED_RUNS', 5));
