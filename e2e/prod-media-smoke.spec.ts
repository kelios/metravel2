...
test.describe('#1263 search list scroll reveal', () => {
  test.describe('desktop 1280x900', () => {
    ...
    test('обложки готовы при въезде в кадр и не грузятся заново', async ({ page }) => {
      const profile: TraceProfile = {
        ...
        minVisibleCovers: 6,
        maxRequests: 20,
        // Перебазировано под квадратный слот #1487 (2026-08-24): каталожная
        // обложка рисуется 386×386 вместо прежней ландшафтной полосы, ступень
        // выросла до w=720 по решению владельца «резкость сетки за байты», и
        // прежний бюджет 1 450 000 (замер 2026-08-10 на старой поверхности:
        // 1 404 556 B + 3%) стал бюджетом ДРУГОЙ поверхности. Свежий замер
        // прода после деплоя 56ab51b6: afterReturn 1 581 633 B, детерминированно
        // на прогоне и retry. Методика прежняя — замер + 3% на дрейф контента.
        // Гейт не ослеп: регрессия на ступень 960w даёт ~2 900 000 B (замер
        // свипа 2026-08-23) — кратно выше границы.
        maxBytes: 1_630_000,
      };
  ...
  test.describe('mobile 390x844', () => {
    ...
    test('обложки готовы при въезде в кадр и не грузятся заново', async ({ page }) => {
      const profile: TraceProfile = {
        cpuThrottle: 4,
        // #1487 (2026-08-24): карточка мобильного каталога стала квадратной
        // (~455 px строка вместо 340), и в вьюпорт 844 px помещаются ДВЕ
        // карточки, а не три — прежний минимум 3 был геометрией старой
        // поверхности и стал недостижим независимо от перформанса. Регрессию
        // «первый экран залит доминантным цветом» ловит и порог 2: fill-only
        // карточки в счётчик готовых не входят.
        minVisibleCovers: 2,
        maxRequests: 12,
        maxBytes: 700_000,
      };
