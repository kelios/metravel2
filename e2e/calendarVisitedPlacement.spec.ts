...
test.describe('Calendar — visited placement (#878/#880) @smoke', () => {
  test('dateless visit (year+month, no exact date) renders as a card and gets a calendar marker', async ({ page }) => {
    await seedVisited(page, [
      makeItem(701, 'Озеро Гарда', { visited_date: null, travel: { ...makeItem(701, 'Озеро Гарда').travel, year: '2024', monthName: 'Июль' } }),
    ]);
