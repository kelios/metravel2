...
export default function Root({ children }: { children: React.ReactNode }) {
  ...
  return (
    ...
    <head>
      ...
      <script
        dangerouslySetInnerHTML={{ __html: String.raw`(function(){if(typeof window==='undefined')return;var KEY='__metravel_chunk_reload';var COUNT_KEY='__metravel_chunk_reload_count';var COOLDOWN=30000;var MAX_RETRIES=2;function isStale(m){return/requiring unknown module|cannot find module|loading chunk|failed to fetch dynamically imported module|loading module.*failed|is not a function|is not iterable|iterable.*null|spread|Minified React error #130|Minified React error #423|Element type is invalid/i.test(m)}function doReload(){try{sessionStorage.setItem(KEY,Date.now().toString());var c=parseInt(sessionStorage.getItem(COUNT_KEY)||'0',10);sessionStorage.setItem(COUNT_KEY,String(c+1))}catch(_){}try{window.location.reload()}catch(_){}}function fullCleanup(){var p1=Promise.resolve();try{if('caches' in window){p1=caches.keys().then(function(ns){return Promise.all(ns.map(function(n){return caches.delete(n)}))})}}catch(_){}return p1.catch(function(){})}function handler(e){var m=String((e&&e.message)||(e&&e.reason&&e.reason.message)||'');if(!isStale(m))return;try{var count=parseInt(sessionStorage.getItem(COUNT_KEY)||'0',10);if(count>=MAX_RETRIES){console.warn('[Recovery] Max retries reached ('+MAX_RETRIES+'), not reloading');return}var last=sessionStorage.getItem(KEY);if(last){var elapsed=Date.now()-parseInt(last,10);if(elapsed<COOLDOWN){console.warn('[Recovery] Stale chunk detected but cooldown active, skipping reload');return}}}catch(_){}e.preventDefault&&e.preventDefault();fullCleanup().then(doReload).catch(doReload)}window.addEventListener('error',handler);window.addEventListener('unhandledrejection',function(e){var r=e&&e.reason;var m=String((r&&r.message)||r||'');if(!isStale(m))return;handler({message:m,preventDefault:function(){e.preventDefault&&e.preventDefault()}})});try{var last=sessionStorage.getItem(KEY);if(last){var elapsed=Date.now()-parseInt(last,10);if(elapsed>COOLDOWN){sessionStorage.removeItem(KEY);sessionStorage.removeItem(COUNT_KEY)}}}catch(_){}var h=window.location&&window.location.hostname;if(h==='metravel.by'||h==='www.metravel.by'||h==='localhost'||h==='127.0.0.1'){var re=/Indexed property setter is not supported|shadow\*.*deprecated|textShadow\*.*deprecated|pointerEvents is deprecated|useNativeDriver.*native animated module|useLayoutEffect does nothing on the server/;var oe=console.error,ow=console.warn;console.error=function(){var m=arguments[0];if(typeof m==='string'&&re.test(m))return;oe.apply(console,arguments)};console.warn=function(){var m=arguments[0];if(typeof m==='string'&&re.test(m))return;ow.apply(console,arguments)}}})();` }}
      />
    </head>
