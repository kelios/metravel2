...
(function(){
  try {
    ...
    // #1185: literal empty markers are not slugs. Redirecting them built a
    // travel URL out of the literal word and landed on a 404 instead of the
    // home page. #1438: those example URLs are deliberately not spelled out
    // here — this comment ships inside the served HTML, and URL-regex
    // scrapers crawl anything that looks like a path.
...
(function(){
  try {
    ...
    function getRouteMeta(path){
      ...
      var lastSegment=humanizeSegment(getLastSegment(normalized));
      if(normalized==='/'||normalized==='/index')return{title:${JSON.stringify(HOME_TITLE)},description:${JSON.stringify(HOME_DESCRIPTION)},ogType:'website'};
      if(normalized==='/search')return{title:${JSON.stringify(SEARCH_TITLE)},description:${JSON.stringify(SEARCH_DESCRIPTION)},ogType:'website'};
      if(normalized==='/map')return{title:${JSON.stringify(MAP_TITLE)},description:${JSON.stringify(MAP_DESCRIPTION)},ogType:'website'};
      if(normalized==='/articles')return{title:${JSON.stringify(ARTICLES_TITLE)},description:${JSON.stringify(ARTICLES_DESCRIPTION)},ogType:'website'};
      if(normalized==='/about')return{title:${JSON.stringify(ABOUT_TITLE)},description:${JSON.stringify(ABOUT_DESCRIPTION)},ogType:'website'};
      if(normalized==='/contact')return{title:${JSON.stringify(CONTACT_TITLE)},description:${JSON.stringify(CONTACT_DESCRIPTION)},ogType:'website'};
    ...
    var routePath=window.location&&window.location.pathname||'/';
    var routeMeta=getRouteMeta(routePath);
    var titleEl=document.querySelector('head title[data-rh="true"]')||document.querySelector('head title');
    if(titleEl){
      var currentTitle=titleEl.textContent||'';
      if(isGenericTitle(currentTitle))titleEl.textContent=routeMeta.title;
    }else if(isGenericTitle(document.title)){
      document.title=routeMeta.title;
    }
    if(isGenericTitle(document.title))document.title=routeMeta.title;
    var descriptionMeta=document.querySelector('meta[name="description"]');
    if(descriptionMeta){
      var currentDescription=descriptionMeta.getAttribute('content')||'';
      if(isGenericDescription(currentDescription))descriptionMeta.setAttribute('content',routeMeta.description);
    }else{
    ...
    if(routeMeta.robots||shouldNoindexPath(routePath))upsertMeta('meta[name="robots"]',{name:'robots'},routeMeta.robots||'noindex, nofollow');
    ...
    upsertMeta('meta[property="og:type"]',{property:'og:type'},routeMeta.ogType||'website');
  } catch(_){}
  try {
    var s=null;try{s=window.localStorage.getItem('theme')}catch(_){}
    var th=(s==='light'||s==='dark'||s==='auto')?s:'auto';
    var d=false;
    if(th==='dark')d=true;else if(th!=='light')d=window.matchMedia&&window.matchMedia('(prefers-color-scheme:dark)').matches;
  ...
  try {
    ...
    if(/^\/quests\/[^/]+\/[^/]+\/?$/.test(p2))return;
    if(p2.length>1)p2=p2.replace(/\/+$/,'');
    var o='https://metravel.by';
    var correctUrl=o+p2;
    var cl=document.querySelector('link[rel="canonical"]');
    if(cl){
      var h2=cl.getAttribute('href')||'';
      if(h2!==correctUrl){cl.setAttribute('href',correctUrl)}
    }else{
    ...
    if(ou){
      var oc2=ou.getAttribute('content')||'';
      if(/\[|\%5B/.test(oc2)||oc2!==correctUrl){ou.setAttribute('content',correctUrl)}
    }else{
      ou=document.createElement('meta');
      ou.setAttribute('property','og:url');
      ou.setAttribute('content',correctUrl);
      document.head.appendChild(ou);
    }
    try {
      if(typeof MutationObserver!=='undefined'){
        var _canUrl=correctUrl;
        var _canObs=new MutationObserver(function(){
          var all=document.querySelectorAll('link[rel="canonical"]');
          if(all.length>1){
            for(var i=all.length-1;i>=1;i--){try{all[i].parentNode&&all[i].parentNode.removeChild(all[i])}catch(e){}}
          }
          if(all[0]&&all[0].getAttribute('href')!==_canUrl){all[0].setAttribute('href',_canUrl)}
        });
        _canObs.observe(document.head,{childList:true});
        setTimeout(function(){try{_canObs.disconnect()}catch(e){}},1000);
      }
    } catch(_2){}
  } catch(_){}
})();
