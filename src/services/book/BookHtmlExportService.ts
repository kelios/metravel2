...
export class BookHtmlExportService {
  ...
  private enhanceHtmlForPrintPreview(html: string): string {
    ...
    const toolbarHtml = `\n<div class="print-toolbar no-print" style="position:sticky;top:0;z-index:50;width:100%;background:rgba(15,23,42,0.96);color:rgb(229,231,235);padding:12px 24px;box-sizing:border-box;display:flex;align-items:center;justify-content:space-between;gap:16px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',system-ui,sans-serif;font-size:14px;">\n  <div style="display:flex;flex-direction:column;gap:6px;max-width:70%;">\n    <div>\n      Для сохранения книги в PDF используйте меню печати браузера (Ctrl/Cmd+P) и выберите «Сохранить как PDF».\n    </div>\n    <div id="print-status" style="font-size:12px;color:rgba(229,231,235,0.8);">\n      Изображения: подготовка…\n    </div>\n  </div>\n  <button id="print-btn" onclick="window.__metravelPrint && window.__metravelPrint()" style="background:rgb(249,115,22);border:none;border-radius:999px;color:rgb(255,255,255);padding:8px 16px;font-weight:600;cursor:pointer;box-shadow:0 4px 10px rgba(0,0,0,0.3);">\n    Печать / Сохранить в PDF\n  </button>\n</div>\n`;
