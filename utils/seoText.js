...
// Координатное значение в двух формах записи: десятичной дробью (53.6006) и в
// градусах-минутах-секундах (52°07′44.8″). Целое без дроби и без знака градуса
// значением не считается: иначе правило съело бы «100 метров от дороги».
const COORDINATE_VALUE = String.raw`-?\d{1,3}(?:[.,]\d+|\s*°(?:\s*\d{1,2}\s*[′'])?(?:\s*\d{1,2}(?:[.,]\d+)?\s*[″"])?)`;
// Вторая координата отделяется запятой или просто пробелом: в градусах пары
// пишут без запятой — «52°07′44.8″ 25°00′9.5″».
// Форм записи градусов больше, чем описано выше: Google Maps копирует шапку с
// полушарием («52°07'44.8"N 25°00'09.5"E»), а редактор подменяет штрихи на
// типографские кавычки («52°07’44.8”»). Узнав такую шапку наполовину, срез
// оборвался бы ВНУТРИ координаты и открыл сниппет обрубком «N 25°00'09.5"E…» —
// это хуже неснятой шапки. Поэтому за координатой не должно оставаться ни
// буквы/цифры вплотную, ни продолжения координаты через пробел: не узнали
// форму целиком — не режем вовсе.
const COORDINATE_BLOCK_END = String.raw`(?![\p{L}\p{N}°′″'"’”]|\s+[\p{N}°′″'"’”]|\s*[свюз]\s*\.\s*[шд])`;
const COORDINATE_PAIR = String.raw`${COORDINATE_VALUE}(?:(?:\s*,\s*|\s+)${COORDINATE_VALUE})?${COORDINATE_BLOCK_END}`;
const LEADING_GEO_BLOCK_RE = new RegExp(
  String.raw`^(?:\s*(?:координаты(?:\s+gps)?|gps|широта|долгота)\s*:?\s*${COORDINATE_PAIR}\s*\.?|\s*расстояние\s+от\s+\p{L}+\s*:?\s*\d+[\s\u00A0]*(?:км|km)(?!\p{L})\s*\.?)+`,
  'iu',
);
...
const LEADING_GREETING_RE =
  /^(?:всем\s+)?(?:привет|здравствуйте|здравствуй|добрый\s+день|друзья)(?:\s+всем)?\s*[!.…]+\s*/iu;
...
function findQuoteEnd(text, [open, close]) {
  ...
  let depth = 0;
  // Баланс сходится не всегда: внешнюю кавычку эпиграфа авторы забывают закрыть,
  // и «…Табе пачуецца: «Спыніся!…стаіш!» Антон Буболо остаётся с depth = 1.
  // Возврат -1 отдавал такой эпиграф в сниппет целиком, поэтому откатываемся на
  // ПЕРВУЮ закрывающую — она же единственный кандидат на границу: глубина растёт
  // только на открывающих, и первое её падение приходится ровно сюда. Последняя
  // закрывающая границей быть не может: у /travels/svyato-uspenskaya-cerkov это
  // «совсем» на 5140-м символе тела статьи, за тысячи знаков от эпиграфа.
  // Границу это только предполагает — эпиграфом отрезанное признают проверки
  // stripLeadingEpigraph, прежде всего «фраза продолжается после кавычки».
  ...
  for (let i = 0; i < text.length; i += 1) {
    if (text.startsWith(open, i)) depth += 1;
      ...
      depth -= 1;
      if (depth === 0) return i;
      if (firstCloseIndex < 0) firstCloseIndex = i;
    }
  }
