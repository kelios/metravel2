...
//
// #1113: прокси поддерживает НЕ произвольную ширину. Неподдержанное значение он не
// снэпит и не отвергает — МОЛЧА отдаёт исходный файл целиком, поэтому вместо превью
// в единицы КБ прилетают сотни. Замер прода 2026-07-28 на `travel-image/682/...webp`
// (исходник 1024×576, 132 344 B):
//   работают:     32 → 322 B · 96 → 1 650 B · 160 · 320 → 11 562 B · 480 · 640 ·
//                 720 → 45 390 B · 800 → 53 934 B · 960 → 73 460 B · 1280 · 1600 · 1920
//   НЕ работают:  47/48/49 → 132 344 B (реальные пиксели 1024×576 — ОРИГИНАЛ),
//                 16, 24, 239/240/241, 1024, 1440, 2048, 2500 → тот же оригинал
// Подтверждено на /gallery/, /travel-image/, /address-image/, /avatar/, /quest-cover/.
//
// Лестница ниже — подмножество проверенно рабочих ступеней. Из прежнего набора убраны
// 16, 24, 48, 240, 1024 и 2048: каждая такая ступень означала полноразмерный оригинал
// вместо превью. Добавляя сюда ступень, СНАЧАЛА проверь её curl'ом — «круглое» число
// само по себе ничего не гарантирует (1024 и 2048 как раз сломаны).
