 ...
 */
export const ROUTE_DISTANCE_FORMAT: Readonly<DistanceFormatOptions> = Object.freeze({
  integerFromKm: 1000,
});

/**
 * Канонический форматтер расстояния (#1440): выбирает единицу, а разделители
 * дроби и разрядов берёт из локали интерфейса, иначе русскому пользователю
 * печатается английское «11.4 км» и «2800 км» без разрядов. Новое место, где
 * печатается расстояние, должно звать его, а не собирать строку на месте:
 * #1449 свела сюда последние самодельные `toFixed` — попап места, квесты,
 * карточку квеста и Strava.
