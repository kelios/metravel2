...
const QUEST_SENTENCE_ABBREVIATIONS = new Set([
  'ул', 'пл', 'пр', 'просп', 'пер', 'наб', 'д', 'корп', 'стр', 'кв', 'ст',
  'г', 'гг', 'в', 'вв', 'н', 'э', 'им', 'оз', 'р', 'ок', 'тыс', 'млн', 'млрд',
  // Многобуквенные сокращения с заглавной живут здесь, а не в правиле
  // инициалов: «Св. Иоанна», «Дж. Б. Аллио», «Шв. Игното». Сверка идёт по
  // строчной форме, поэтому «св. Николая» и «Св. Тодора» закрыты одной строкой.
  'св', 'дж', 'шв',
]);
...
/**
 * Инициал — РОВНО одна заглавная буква: «В. Ким», «Ф. К. Богушевич».
 *
 * Две буквы сюда не берутся намеренно: «Ош.» и любое короткое имя собственное в
 * конце фразы неотличимы от сокращения, и правило склеило бы два настоящих
 * предложения («Город называется Ош. Здесь красиво»). Реальные двухбуквенные
 * сокращения каталога перечислены в `QUEST_SENTENCE_ABBREVIATIONS` поимённо.
 */
const QUEST_TRAILING_INITIAL_RE = /(?:^|[^\p{L}\p{N}])(\p{Lu})\.$/u;
