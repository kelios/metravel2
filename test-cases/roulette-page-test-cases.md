...
**Приоритет**: P2  
**Предусловия**: Пользователь авторизован, результаты отображены после "Крутить рулетку"/"Подобрать"

### Шаги:
1. Открыть `/roulette`
2. Запустить рулетку и дождаться карточек случайных путешествий
3. Найти иконку "Избранное" на первой карточке результата
4. Кликнуть на иконку
5. Повторить шаги 2-4 после кнопки "Крутить еще раз"/"Ещё"

### Ожидаемый результат:
- Иконка меняет состояние на выбранное
- A11y label меняется с "Добавить в избранное" на "Удалить из избранного"
- API запрос: `PATCH /api/travels/{id}/mark-as-favorite/`
- В запрос уходит реальный backend `travel.id` результата, а не индекс карточки/slug/HTTP-код
- Toast "Не удалось обновить избранное" не появляется при успешном ответе API
- Путешествие появляется в избранном пользователя после обновления/перехода в раздел избранного

### Проверки:
- [ ] Иконка изменилась
- [ ] A11y label изменился
- [ ] API запрос успешен
- [ ] Нет error toast
- [ ] Избранное синхронизировалось с сервером

---

## TC-ROULETTE-011A: Удаление результата из избранного
**Приоритет**: P2
**Предусловия**: Пользователь авторизован, результат рулетки уже добавлен в избранное

### Шаги:
1. Открыть `/roulette`
2. Запустить рулетку до карточки, которая уже находится в избранном
3. Кликнуть на выбранную иконку "Избранное"

### Ожидаемый результат:
- Иконка возвращается в невыбранное состояние
- A11y label меняется с "Удалить из избранного" на "Добавить в избранное"
