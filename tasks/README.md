...
**Баг или фичу** оформляй по общепринятому issue-шаблону сообщества через skill
`/metravel-issue` (bug: описание → шаги → ожидалось → по факту → окружение →
доказательства → причина/как починить; feature: проблема → решение → альтернативы →
ценность → объём). Шаблоны — в `.claude/skills/metravel-issue/`. Поверх обоих —
обязательный `Task Contract`. `kind=task` (chore/рефактор/контент/тулинг) — по
`000-template.md`, не через issue-шаблон.
