...
и передаются владельцу/бэкендеру; статусы сверяет `backend-status-sync` →
`docs/BACKEND_WORKBOARD.md` до полного переноса трекинга на борд.
...
- См. `docs/TASK_BOARD_MCP.md` (борд/MCP), `docs/CODEX.md` (планирование), `docs/BACKEND_WORKBOARD.md`.
