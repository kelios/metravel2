...
- 2026-06-13 (повторное расследование на проде, бандл `entry-cbf468a3…`): **AC#4 — это и есть основная причина «белого экрана», а не вторичный слой.** Воспроизведено на проде: SPA-переход travel→travel **вешает main-thread рендерера на >150с** (`loadLeafletRuntime` остаётся `pending`), скелетон-оверлей не снимается → белый экран. Прямая F5 работает. Точная цепочка (статика + Network):
  1. Контейнер детали НЕ ремаунтится по travelId (`app/(tabs)/travels/[param].tsx:9` рендерит `<TravelDetailsContainer/>` без `key`). Стейт всех хуков переносится с прошлого травела.
  2. Cache-hit переход (целевой травел уже в React Query кеше, `staleTime:600000` — `useTravelDetails.ts:345`) → `isLoading=false` на ПЕРВОМ рендере, без фазы `true→false`.
  3. Из-за 1+2 гейты отложенной обвязки переносятся уже `true`: `postLcpRuntimeReady`/`lcpLoaded`/`sliderReady`/`heroEnhancersReady`. Ветка сброса в `useTravelDetailsPerformance.ts:148-202` срабатывает ТОЛЬКО при `isLoading=true` — на cache-hit не выполняется. → `deferredChromeReady=true` синхронно (`travelDetailsContainerViewModel.ts:64`).
  4. `canRenderHeavy = useState(Platform.OS === 'web')` (`useTravelDeferredSectionsModel.ts:76`) = `true` на web мгновенно; `<TravelMap>` смонтирована с первого рендера без гейта видимости (`ToggleableMap initiallyOpen` + `keepMounted`, `TravelRouteMapBlock.tsx:117-138`). IntersectionObserver `mapNearViewport` гейтит только GPX-превью, НЕ монтаж карты (`useTravelDetailsMapSectionContentModel.ts:71,74`).
  5. Итог: вся обвязка детали + Leaflet + маркеры/кластеризация строятся синхронно одним коммитом → поток виснет → оверлей висит → белый экран. На F5 `isLoading` проходит `true→false`, гейты сбрасываются и открываются прогрессивно через `requestIdleCallback`.
  - Прежние фиксы оверлея (didNotifyLoadRef reset + visualReadyFallbackMs:500) НЕ помогают: замороженный поток не выполняет ни onLoad, ни fallback-таймер.
  - #419 на каждой SSG-странице — рекаверится клиентским рендером, фоновый шум, НЕ причина белого экрана. Отключение `asyncRoutes.web` уже пробовали — не помогло (подтверждает).
  - **Предлагаемый фикс:** (а) сбрасывать web-гейты (`lcpLoaded/sliderReady/heroEnhancersReady/postLcpRuntimeReady` + `canRenderHeavy`) при смене `travelId` НА ЭТАПЕ РЕНДЕРА (паттерн «adjust state during render»), чтобы SPA-переход вёл себя как свежий монтаж и обвязка раскрывалась прогрессивно через idle, а не одним коммитом; (б) `canRenderHeavy` на web инициализировать НЕ мгновенным `true`, а через `runWhenBrowserIdle`, и/или гейтить сам монтаж `<TravelMap>` на `mapNearViewport` (scroll-into-view), чтобы тяжёлый Leaflet не строился синхронно при навигации. Браузер-верификация — обязательно на проде (локально не воспроизводится из-за нестабильного dev-сервера).
