...
4. **Двинуть статусы (с evidence).**
   ...
   - обязательный gate сейчас недоступен → остановиться, запросить exact unblock и продолжить
     ту же приёмку. `testing` между turns допустим только для повторного замера с exact
     параметром, threshold/trigger и временем.
5. **Багфиксы — отдельным тикетом.** Связанную карточку передай профильному FE-агенту /
   `ticket-flow`; current acceptance не используется как контейнер новой работы.
