...
- Не печатать секреты/токены. Приёмка web-тикетов — на проде после выката задачи
  (`curl -s https://metravel.by/.build-source.json` → `sha` содержит коммит тикета); сборки
  на проде нет → запросить выкат `frontend-deployer` у оркестратора, старую сборку не принимать.
  Dev-стенд и сторы — только по явному запросу.
