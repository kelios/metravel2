...
description: "Статьи-путешествия Юли: из папки фото или правка живой статьи (дневник, впечатления, км/высоты, советы). Триггеры: «отредактируй статью», «путеводитель», «добавь впечатления», «сделай статьи из папки», «добавь фото и распиши»."
---
...
| Дата статьи ≠ дата поездки | `created_at` = день импорта/публикации, а `year`/`month` — про поездку; в каталоге статья стоит не на своём месте | определить дату поездки и передать в бэкенд-карточку (шаг 5a) |
...
- без улиц, номеров домов, кодов дорог и номеров парковок; без дублей в
  пределах статьи — у двух точек с одним именем читатель не поймёт, какая где;
...
Каталог и «Новые маршруты» на главной сортируют `newest` = `-id`
(`travels/travel_common_impl.py:52-54`), а `Travel.created_at` —
`auto_now_add` и в `TravelUpsertSerializer` не объявлен: импортированная
сегодня статья о поездке 2024 года встаёт выше свежих поездок 2026-го, и из
фронта это не чинится (`created_desc` в UI к тому же скрыт). При каждой правке:

1. Определить дату поездки: день — из EXIF кадров или текста статьи, иначе
   год-месяц из `year`/`month` в `GET /api/travels/{id}/`. Пустой `month`
   восстановить по кадрам/тексту и проставить тем же upsert (`payload.month`).
2. Зафиксировать дату в отчёте и дописать в бэкенд-карточку #1995
   (`created_at` через upsert + `newest` по `created_at`); пока она не в
   проде, это единственный путь.
3. Когда API примет поле: в тот же PUT класть
   `payload.created_at = '<YYYY-MM-DD>T12:00:00Z'` и подтверждать re-GET
   байт-в-байт, как `--meta`.
...
Ответ `201` + `{id, thread, user: 1, user_name: "Юля"}`. Автор комментария
берётся из токена — отдельного «редакционного» аккаунта в проекте нет, комментарий
идёт от Юли, тем же токеном, что и правка статьи.

Как писать — это отдельный жанр, не пересказ статьи:

- **2–4 коротких абзаца**, живой голос Юли от первого лица, как в тексте статьи;
...
alt у каждого <img> осмысленный (не «Изображение», не название статьи)
...
дата поездки определена и передана в бэкенд-карточку (шаг 5a); month не пуст
GET /api/travel-comments/tree/?travel_id={id} → комментарий редакции есть, ровно один
```
