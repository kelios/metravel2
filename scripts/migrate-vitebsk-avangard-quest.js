#!/usr/bin/env node
/**
 * Миграция квеста (Витебск (авангард)) на прод.
 * Идемпотентно: переиспользует существующий квест по quest_id, не дублирует шаги.
 *
 * NODE_TLS_REJECT_UNAUTHORIZED=0 node scripts/migrate-vitebsk-avangard-quest.js \
 *   --api-url=https://metravel.by --token=YOUR_TOKEN
 *
 * Dry run (без записи):
 *   node scripts/migrate-vitebsk-avangard-quest.js --dry-run
 */
...
const QUESTS = require('./vitebsk-avangard-quest-data.js');

async function main() {
    console.log(`🚀 Миграция квеста (Витебск (авангард)) → ${API_BASE} (${isDryRun ? 'DRY RUN' : 'LIVE'})\n`);
    ...
    for (const q of QUESTS) {
        ...
        const cityId = 13;
        if (!questDbId) console.log(`  ℹ️ Reusing existing city id=13`);
        ...
        console.log(`\n🎉 Квест "${q.title}" готов!`);
        console.log(`   URL: https://metravel.by/quests/vitebsk-avangard/${q.quest_id}`);
    }
