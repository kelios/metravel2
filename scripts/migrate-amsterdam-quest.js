#!/usr/bin/env node
/**
 * Миграция квеста (Амстердам) на прод.
 * Идемпотентно: переиспользует существующий квест по quest_id и город по имени,
 * не дублирует шаги.
 *
 * NODE_TLS_REJECT_UNAUTHORIZED=0 node scripts/migrate-amsterdam-quest.js \
 *   --api-url=https://metravel.by --token=YOUR_TOKEN
 *
 * Dry run (без записи):
 *   node scripts/migrate-amsterdam-quest.js --dry-run
 */
...
// Дедуп города: ищем существующий quest-city по имени по всем страницам
async function findCityByName(name) {
    ...
    while (endpoint) {
        ...
        endpoint = page.next_page_url ? page.next_page_url.replace(API_BASE, '') : null;
    }
    return null;
}
...
const QUESTS = require('./amsterdam-quest-data.js');

async function main() {
    console.log(`🚀 Миграция квеста (Амстердам) → ${API_BASE} (${isDryRun ? 'DRY RUN' : 'LIVE'})\n`);
    ...
    for (const q of QUESTS) {
        ...
        // 1. Reuse or create city (only when quest is missing)
        let cityId;
        if (!questDbId) {
            try {
                const existingCity = isDryRun ? null : await findCityByName(q.city.name);
                if (existingCity) {
                    cityId = existingCity.id;
                    console.log(`  ℹ️ City already exists: id=${cityId} (${q.city.name})`);
                } else {
        ...
        console.log(`\n🎉 Квест "${q.title}" готов!`);
        console.log(`   URL: https://metravel.by/quests/amsterdam/${q.quest_id}`);
    }
}
