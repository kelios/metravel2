#!/usr/bin/env node
/**
 * Миграция сложного квеста «Код Минска» на прод.
 * Идемпотентно: переиспользует существующий квест по quest_id, не дублирует шаги.
 *
 * NODE_TLS_REJECT_UNAUTHORIZED=0 node scripts/migrate-minsk-cipher-quest.js \
 *   --api-url=https://metravel.by --token=YOUR_TOKEN
 *
 * Dry run (без записи):
 *   node scripts/migrate-minsk-cipher-quest.js --dry-run
 */
...
const QUESTS = require('./minsk-cipher-quest-data.js');
