#!/usr/bin/env node
/**
 * Миграция квеста «Советская: деловое сити старого Гомеля» на прод.
 * Идемпотентно: переиспользует существующий квест по quest_id, не дублирует шаги.
 *
 * ВНИМАНИЕ: город «Гомель» уже существует в БД (создан квестом gomel-spasova).
 * Эта миграция создаёт город ТОЛЬКО когда квест gomel-soviet ещё не заведён.
 * Если квест отсутствует, а город уже есть — повторный POST /api/quest-cities/
 * может создать дубль «Гомеля». Перед live-запуском проверь список городов
 * (GET /api/quest-cities/) и при необходимости подставь существующий city id.
 *
 * NODE_TLS_REJECT_UNAUTHORIZED=0 node scripts/migrate-gomel-soviet-quest.js \
 *   --api-url=https://metravel.by --token=YOUR_TOKEN
 *
 * Dry run (без записи):
 *   node scripts/migrate-gomel-soviet-quest.js --dry-run
 */
...
const QUESTS = require('./gomel-soviet-quest-data.js');

async function main() {
    console.log(`🚀 Миграция квеста «Советская: деловое сити старого Гомеля» → ${API_BASE} (${isDryRun ? 'DRY RUN' : 'LIVE'})\n`);
    ...
    for (const q of QUESTS) {
        ...
        //    NB: Гомель уже есть в БД (city от gomel-spasova). Чтобы не плодить
        //    дубль, пытаемся найти существующий город по имени, и только если
        ...
        console.log(`\n🎉 Квест "${q.title}" готов!`);
        console.log(`   URL: https://metravel.by/quests/gomel/${q.quest_id}`);
    }
