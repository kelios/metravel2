#!/usr/bin/env node
/**
 * Миграция квеста «Спасова слобода: тайна старой веры над Сожем» (Гомель) на прод.
 * Идемпотентно: переиспользует существующий квест по quest_id, не дублирует шаги.
 *
 * NODE_TLS_REJECT_UNAUTHORIZED=0 node scripts/migrate-gomel-spasova-quest.js \
 *   --api-url=https://metravel.by --token=YOUR_TOKEN
 *
 * Dry run (без записи):
 *   node scripts/migrate-gomel-spasova-quest.js --dry-run
 */
...
const QUESTS = require('./gomel-spasova-quest-data.js');

async function main() {
    console.log(`🚀 Миграция квеста «Спасова слобода» (Гомель) → ${API_BASE} (${isDryRun ? 'DRY RUN' : 'LIVE'})\n`);
