#!/usr/bin/env node
/**
 * Миграция квеста «Республика одного слова — Libertas» (Дубровник) на прод.
 * Идемпотентно: переиспользует существующий квест по quest_id, не дублирует шаги.
 *
 * NODE_TLS_REJECT_UNAUTHORIZED=0 node scripts/migrate-dubrovnik-quest.js \
 *   --api-url=https://metravel.by --token=YOUR_TOKEN
 *
 * Dry run (без записи):
 *   node scripts/migrate-dubrovnik-quest.js --dry-run
 */
...
const QUESTS = require('./dubrovnik-quest-data.js');

async function main() {
    console.log(`🚀 Миграция квеста (Дубровник) → ${API_BASE} (${isDryRun ? 'DRY RUN' : 'LIVE'})\n`);
    ...
    for (const q of QUESTS) {
        ...
        console.log(`\n🎉 Квест "${q.title}" готов!`);
        console.log(`   URL: https://metravel.by/quests/dubrovnik/${q.quest_id}`);
    }
