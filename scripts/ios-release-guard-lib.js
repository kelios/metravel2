...
function validateIosRelease(root = process.cwd(), options = {}) {
  ...
  // Android's manifest/colors.xml are generated by `expo prebuild` from this
  // same plugin config and are untracked (.gitignore); they don't exist on a
  // clean clone. Icon/color/channel parity with Android is therefore checked
  // against the single tracked source (this plugin config, the shared icon
  // asset checked below), not against generated android/ output.
  if (notificationPlugins.length !== 1 ||
    ...
    fail(
      ...
      'the official notifications plugin must keep production APNs, foreground-only delivery, and the tracked icon/color/channel configuration shared with Android'
    );
  }
  ...
  const projectPairs = [
    ...
    ['IOS_BUILD_NUMBER_XCODE', /CURRENT_PROJECT_VERSION = 10;/g, /CURRENT_PROJECT_VERSION = /g, 2],
