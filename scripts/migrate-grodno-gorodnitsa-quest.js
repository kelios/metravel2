#!/usr/bin/env node
/**
 * Миграция квеста (Гродно (Городница)) на прод.
 * Идемпотентно: переиспользует существующий квест по quest_id, не дублирует шаги.
 *
 * NODE_TLS_REJECT_UNAUTHORIZED=0 node scripts/migrate-grodno-gorodnitsa-quest.js \
 *   --api-url=https://metravel.by --token=YOUR_TOKEN
 *
 * Dry run (без записи):
 *   node scripts/migrate-grodno-gorodnitsa-quest.js --dry-run
 */
...
const QUESTS = require('./grodno-gorodnitsa-quest-data.js');

async function main() {
    console.log(`🚀 Миграция квеста (Гродно (Городница)) → ${API_BASE} (${isDryRun ? 'DRY RUN' : 'LIVE'})\n`);
    ...
    for (const q of QUESTS) {
        ...
        // 1. Existing city (backend does NOT dedupe by name — reuse known id)
        const cityId = 11;
        if (!questDbId) console.log(`  ℹ️ Reusing existing city id=11`);
        ...
        console.log(`\n🎉 Квест "${q.title}" готов!`);
        console.log(`   URL: https://metravel.by/quests/grodno-gorodnitsa/${q.quest_id}`);
    }
