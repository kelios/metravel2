#!/usr/bin/env node
/**
 * Миграция квеста «Варшава: Сирена, Базилишек и Золотая утка» на прод.
 * Идемпотентно: переиспользует существующий квест по quest_id, не дублирует шаги.
 *
 * NODE_TLS_REJECT_UNAUTHORIZED=0 node scripts/migrate-warsaw-quest.js \
 *   --api-url=https://metravel.by --token=YOUR_TOKEN
 *
 * Dry run (без записи):
 *   node scripts/migrate-warsaw-quest.js --dry-run
 */
...
async function main() {
    console.log(`🚀 Миграция квеста «Варшава» → ${API_BASE} (${isDryRun ? 'DRY RUN' : 'LIVE'})\n`);
    ...
    for (const q of QUESTS) {
        ...
        console.log(`\n🎉 Квест "${q.title}" готов!`);
        console.log(`   URL: https://metravel.by/quests/warsaw/${q.quest_id}`);
    }
