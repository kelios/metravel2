...
function verifyQuestCityHtml(html, expectedCanonical, childHtml = '') {
  ...
  // снова обёртка единственного квеста (#1569). Индексируемость решает порог, а
  // не заметки, но порог без заметок не взять: обзор, практика и лид у всех
  // городов — один шаблон с подставленным названием (150–200 слов, ~0% своей
  // лексики), так что требование walk-блока здесь — следствие порога, и тест
  // «город без заметок не берёт порог #1930» в generate-seo-pages.test.ts
  // пиннит его: отрастит шаблон столько собственного текста, что порог возьмёт
  // и страница без заметок, — требование снимается осознанно, а не красной
  // сборкой. Тег с меткой Helmet Helmet же снимет при гидрации (#1929).
