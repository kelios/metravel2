...
// components/home/homeHeroContent.ts). #1541 перевёл набор на ландшафт 3:2,
// поэтому поля почти нет — но цвет обязан совпадать с React-hero, иначе на
// гидрации меняется фон кадра.
const HOME_HERO_FILL = '#536659';
 ...
 *
 * The search shell has the OPPOSITE failure mode (#1406): `app-hydrated` fires
 * ~1 s BEFORE the first catalog cards exist (prod 2026-08-11, same profile:
 * hydrated 5116 ms, first real card 6194 ms), so hydration alone must not tear
 * the shell down — the user would stare at the bare SearchPageSkeleton grid.
 * On `.ssg-search-shell` the app-hydrated path is deferred to the 20s backstop
 * (same shape as travel), and the primary signal is `data-first-screen-ready`,
 * which ListTravelBase sets only when the catalog reached a terminal state:
 * real cards, an honest empty state, or an error.
 *
