 ...
 *
 * Известный недобор, замер 19.09.2026 по 175 локальным data-файлам: формула
 * «опиши в нескольких словах» стоит у 62 из 315 свободных шагов, в ASK_RE не
 * входит (хотя MULTI_RE её намеренно НЕ считает счётной) и потому в scope не
 * попадает. Расширение ASK_RE этой формулой даёт по корпусу 20 кандидатов —
 * среди них `luxembourg-melusina / 9-corniche`, где подсказка прямо просит
 * «просто запиши: река, крыши, скалы» при пороге 6. Каждый требует
 * редакторского вердикта (порог вниз или строка в allow-файл), поэтому
 * расширять критерий в одиночку нельзя: гейт покраснеет на чужих файлах.
 * До этого «0 находок» читается как «новых по покрытой формуле нет».
 *
...
/**
 * Оба словаря спрашиваются только нормализованным словом
 * (`IMPERATIVES.has(normalizeAnswer(word))`), поэтому и храниться обязаны
 * нормализованными: `normalizeRuntime` схлопывает «э»→«е», «ё»→«е», «і»→«и» и
 * снимает дефис, и запись «это» в исходном написании недостижима — ввод даёт
 * «ето». Так в первой редакции молча умерли четыре служебных слова
 * («это», «этот», «эта», «эти»), и «этот лес» из `sasino-stilo / 5-bukowy-las`
 * становился «вариантом ответа» вместо обломка указательного местоимения.
 * Прогон через `normalizeAnswer` держит словарь живым и для будущих слов с «ё».
 */
const normalizeWords = (words) => new Set(words.map(normalizeAnswer))
 ...
 */
const IMPERATIVES = normalizeWords([
...
const STOP_WORDS = normalizeWords([
