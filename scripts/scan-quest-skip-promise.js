 ...
 * выключат.
 */
// `\\b` в JS определён через ASCII `\\w` и перед кириллицей границы не даёт —
// такой фильтр не срабатывал никогда (гейт #1720). Граница слова задана явно:
// начало строки или пробел/пунктуация перед отрицанием.
const NOT_A_BUTTON_RE = /(?:^|[\s(«"„,—–-])(?:не|легко|сложно|трудно|нельзя|невозможно)\s+(?:было\s+)?$/iu
