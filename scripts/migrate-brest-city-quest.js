#!/usr/bin/env node
/**
 * Миграция квеста (Брест (город)) на прод.
 * Идемпотентно: переиспользует существующий квест по quest_id, не дублирует шаги.
 *
 * NODE_TLS_REJECT_UNAUTHORIZED=0 node scripts/migrate-brest-city-quest.js \
 *   --api-url=https://metravel.by --token=YOUR_TOKEN
 *
 * Dry run (без записи):
 *   node scripts/migrate-brest-city-quest.js --dry-run
 */
...
const QUESTS = require('./brest-city-quest-data.js');

async function main() {
    console.log(`🚀 Миграция квеста (Брест (город)) → ${API_BASE} (${isDryRun ? 'DRY RUN' : 'LIVE'})\n`);
    ...
    for (const q of QUESTS) {
        ...
        const cityId = 12;
        if (!questDbId) console.log(`  ℹ️ Reusing existing city id=12`);
        ...
        console.log(`\n🎉 Квест "${q.title}" готов!`);
        console.log(`   URL: https://metravel.by/quests/brest-city/${q.quest_id}`);
    }
