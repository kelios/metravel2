#!/usr/bin/env node
/**
 * Миграция квеста «Город между двух озёр» (Свитязь) на прод.
 * Идемпотентно: переиспользует существующий квест по quest_id, не дублирует шаги.
 *
 * NODE_TLS_REJECT_UNAUTHORIZED=0 node scripts/migrate-svityaz-quest.js \
 *   --api-url=https://metravel.by --token=YOUR_TOKEN
 *
 * Dry run (без записи):
 *   node scripts/migrate-svityaz-quest.js --dry-run
 */
...
const QUESTS = require('./svityaz-quest-data.js');

async function main() {
    console.log(`🚀 Миграция квеста (Свитязь) → ${API_BASE} (${isDryRun ? 'DRY RUN' : 'LIVE'})\n`);
