#!/usr/bin/env node
/**
 * Миграция квеста «Город между двух озёр» (Туров) на прод.
 * Идемпотентно: переиспользует существующий квест по quest_id, не дублирует шаги.
 *
 * NODE_TLS_REJECT_UNAUTHORIZED=0 node scripts/migrate-turov-quest.js \
 *   --api-url=https://metravel.by --token=YOUR_TOKEN
 *
 * Dry run (без записи):
 *   node scripts/migrate-turov-quest.js --dry-run
 */
...
const QUESTS = require('./turov-quest-data.js');

async function main() {
    console.log(`🚀 Миграция квеста (Туров) → ${API_BASE} (${isDryRun ? 'DRY RUN' : 'LIVE'})\n`);
