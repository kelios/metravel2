 ...
 * srcSet, поэтому в `<img>` уходит адрес из API как есть. На запрос без `?w=`
 * бэкенд отвечает МАСТЕРОМ с `no-store`, а такой ответ не кэширует ни nginx, ни
 * браузер: замер прода 2026-08-03 на `/places` — 12 из 12 запросов
