 ...
 * поэтому у состоявшегося прогона весь stdout целиком парсится `JSON.parse`.
 * Всё человекочитаемое в этом режиме молчит, а диагностика источников идёт в
 * stderr. Аварийный выход (проверять нечего — ни одной цели) до отчёта не
 * доходит вовсе: stdout пуст, причина в stderr, код возврата 1 (#1760).
 */
...
function familyOfMediaUrl(rawUrl, site = SITE) {
  return familyOfMediaUrlAt(rawUrl, site)
}
