#!/usr/bin/env node
/**
 * Миграция квеста «Столица Полесья» (Пинск) на прод.
 * Идемпотентно: переиспользует существующий квест по quest_id, не дублирует шаги.
 *
 * NODE_TLS_REJECT_UNAUTHORIZED=0 node scripts/migrate-pinsk-quest.js \
 *   --api-url=https://metravel.by --token=YOUR_TOKEN
 *
 * Dry run (без записи):
 *   node scripts/migrate-pinsk-quest.js --dry-run
 */
...
const QUESTS = require('./pinsk-quest-data.js');

async function main() {
    console.log(`🚀 Миграция квеста (Пинск) → ${API_BASE} (${isDryRun ? 'DRY RUN' : 'LIVE'})\n`);
    ...
    for (const q of QUESTS) {
        ...
        console.log(`\n🎉 Квест "${q.title}" готов!`);
        console.log(`   URL: https://metravel.by/quests/pinsk/${q.quest_id}`);
    }
