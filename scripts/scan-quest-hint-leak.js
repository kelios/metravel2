...
// Поля шага, в которых ищем ответ. Базовый контур — `hint`: тот, который
// правило 4a требует держать чистым безусловно. `story` шумит закономерно
// (story рассказывает об объекте, об который и задан вопрос) и включается
// только флагом.
//
...
//
// `title` ШАГА (#1908) — по той же схеме, что `location`, и вопреки прежнему
// комментарию про «закономерный шум». Замер 13.09.2026 по проду: контур дал 18
// находок на 1626 шагах, и все 18 при поштучном разборе оказались настоящими
// утечками — предсказанного случая «шаг «Кривая башня» при вопросе как
// называется башня» в базе не нашлось ни одного. Все 18 переписаны, прогон стал
// нулевым, поэтому поле переведено в умолчание.
//
// Чего этот контур НЕ ловит, хотя класс #1908 шире. Здесь сравнение
// подстрочное, а засчитывает ответ `buildAnswerChecker`, у которого с #1631 есть
// второй проход по словоформам. Поэтому мимо подстроки проходят: словоформа
// ответа («Девочка с совой» при словаре `сова`), заголовок с числовым префиксом
// нумерации при счётном ответе («2. Каменное эхо» при ответе «2») и одно-двух-
// буквенный ответ (порог MIN_VALUE_LENGTH). Эти шесть случаев в #1908 нашёл не
// скан, а прогон реального чекера по словам заголовка; повторить его в CJS
// нельзя без копии `utils/questAnswerMorphology.ts`, а копия правил — ровно тот
// риск расхождения, ради которого нормализация вынесена в один модуль
// (`scripts/lib/questAnswerNormalize.js`). Поэтому морфологическая половина
// класса остаётся на вычитке, как и семантическая.
const DEFAULT_FIELDS = ['hint', 'location', 'title']
