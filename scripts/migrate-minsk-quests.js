#!/usr/bin/env node
/**
 * Миграция трёх новых квестов по Минску (Лошица, Тракторный, Дворики) на прод.
 * Идемпотентно: переиспользует существующий квест по quest_id, не дублирует шаги.
 *
 * NODE_TLS_REJECT_UNAUTHORIZED=0 node scripts/migrate-minsk-quests.js \
 *   --api-url=https://metravel.by --token=YOUR_TOKEN
 *
 * Dry run (без записи):
 *   node scripts/migrate-minsk-quests.js --dry-run
 */
...
// Найти id уже существующего города по имени (постранично), чтобы не плодить дубли «Минск».
...
const QUESTS = require('./minsk-quests-data.js');

async function main() {
    console.log(`🚀 Миграция квестов «Минск» → ${API_BASE} (${isDryRun ? 'DRY RUN' : 'LIVE'})\n`);
    ...
    for (const q of QUESTS) {
        ...
        console.log(`\n🎉 Квест "${q.title}" готов!`);
        console.log(`   URL: https://metravel.by/quests/minsk/${q.quest_id}`);
    }
