#!/usr/bin/env node
/**
 * Миграция квеста «Город между двух озёр» (волна-3 финал) на прод.
 * Идемпотентно: переиспользует существующий квест по quest_id, не дублирует шаги.
 *
 * NODE_TLS_REJECT_UNAUTHORIZED=0 node scripts/migrate-wave3-final-quest.js \
 *   --api-url=https://metravel.by --token=YOUR_TOKEN
 *
 * Dry run (без записи):
 *   node scripts/migrate-wave3-final-quest.js --dry-run
 */
...
const QUESTS = require('./wave3-final-quest-data.js');

async function main() {
    console.log(`🚀 Миграция квеста (волна-3 финал) → ${API_BASE} (${isDryRun ? 'DRY RUN' : 'LIVE'})\n`);
