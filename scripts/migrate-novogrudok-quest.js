#!/usr/bin/env node
/**
 * Миграция квеста «Первая корона» (Новогрудок) на прод.
 * Идемпотентно: переиспользует существующий квест по quest_id, не дублирует шаги.
 *
 * NODE_TLS_REJECT_UNAUTHORIZED=0 node scripts/migrate-novogrudok-quest.js \
 *   --api-url=https://metravel.by --token=YOUR_TOKEN
 *
 * Dry run (без записи):
 *   node scripts/migrate-novogrudok-quest.js --dry-run
 */
...
const QUESTS = require('./novogrudok-quest-data.js');

async function main() {
    console.log(`🚀 Миграция квеста (Новогрудок) → ${API_BASE} (${isDryRun ? 'DRY RUN' : 'LIVE'})\n`);
    ...
    for (const q of QUESTS) {
        ...
        console.log(`\n🎉 Квест "${q.title}" готов!`);
        console.log(`   URL: https://metravel.by/quests/novogrudok/${q.quest_id}`);
    }
