#!/usr/bin/env node
/**
 * Миграция квеста «Город между двух озёр» (Гольшаны) на прод.
 * Идемпотентно: переиспользует существующий квест по quest_id, не дублирует шаги.
 *
 * NODE_TLS_REJECT_UNAUTHORIZED=0 node scripts/migrate-golshany-quest.js \
 *   --api-url=https://metravel.by --token=YOUR_TOKEN
 *
 * Dry run (без записи):
 *   node scripts/migrate-golshany-quest.js --dry-run
 */
...
const QUESTS = require('./golshany-quest-data.js');

async function main() {
    console.log(`🚀 Миграция квеста (Гольшаны) → ${API_BASE} (${isDryRun ? 'DRY RUN' : 'LIVE'})\n`);
