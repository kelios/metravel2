...
// ИЗВЕСТНОЕ ОГРАНИЧЕНИЕ (P3 ревью 24.08.2026), сознательно НЕ исправлено здесь.
// `isColorWord` матчит корни подстрокой, а короткие корни `син`/`бел` и `сер `
// (после trim → `сер`) залезают в посторонние слова, которых нет в стоп-листе:
// «синагога», «обелиск», «обсерватория», «консерватория», «сервиз», «серия».
// Шаг с таким словом и глаголом «найди/назови/сколько» попадает в scope как
// `task-color-word` (в замороженном снапшоте таких 12: 110, 103, 129, 269, 417,
// 419, 447, 466, 548, 705, 796, 1349). Это over-inclusion, а не пропуск: все 12
// в аудите помечены «исключено с причиной» (группа C), ни один не среди 60
// разобранных, ни один вердикт не искажён. Для гейта направление тоже
// безопасное — автора попросят обосновать НЕсуществующий цвет (ложная тревога,
// снимается стоп-листом + rebaseline), а реальный дрейф поверхности при этом не
// теряется. Почему не правим сейчас: и добавление слов в стоп-лист, и переход на
// `\b`-границы убирают эти 12 из scope, то есть меняют норматив 60+47+40 и рвут
// доказанный паритет с замороженным `.quest-audit/color-1431/scan-color-steps.js`
// — а именно на этом нормативе держится Done gate #1431. Правка возможна только
// осознанной переморозкой аудита: изменить оба скрипта синхронно, пересобрать
// baseline, пересчитать outcomes/scope-decisions/контракт и заново свести
// сходимость. Это отдельная работа, а не часть устранения FAIL приёмки.
...
function classifyStep(step) {
  ...
  const markers = [
    // И цвет, и облицовочный материал эмитятся БЕЗУСЛОВНО, по каждому значению
    // словаря, а не под порогом `dictIs*`. Ключ находки строится из markers, и
    // если облицовка попала в словарь меньшинством (scope через task-asks-material,
    // напр. `['три яруса','из кирпича','высокая']`), под-пороговый гейт выкинул бы
    // её из ключа — и смена эталона «кирпич»→«плитка» прошла бы мимо baseline,
    // ровно тот молчаливый дрейф поверхностного ответа, ради которого скан
    // написан (смешанная кладка Крево из 4f: облицовка часто меньшинство).
    ...dictColors.map((value) => ({ kind: 'dict-color', value: String(value) })),
    ...dictSurface.map((value) => ({ kind: 'dict-material-surface', value: String(value) })),
...
const USAGE = `Скан поверхностного ответа — правило 4f

Usage:
  node scripts/scan-quest-surface-answer.js [--quest-id <id>] [--source <file>] [--baseline <file>] [--update-baseline] [--api-url <url>] [--json]
...
const CLI_SPEC = {
  name: 'scan-quest-surface-answer',
  usage: USAGE,
  selection: 'quests',
  flags: {
...
async function main() {
  ...
  requireNoBatchFailures(findings.length, {
    ...
    message: `шагов без вердикта: ${findings.length}`,
  })
}
