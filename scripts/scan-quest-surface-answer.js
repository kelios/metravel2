...
const COLOR_ROOTS = [
  'красн', 'червон', 'чырвон', 'алый', 'алая', 'алое',
...
function classifyStep(step) {
  ...
  return { structural, reason, markers, branch: dictIsColor || taskAsksColor || countingWithColor ? 'color' : 'material' }
}
...
function updateBaseline(rootDir) {
  ...
  writeBaseline(path.join(rootDir, BASELINE_PATH), {
    ...
    note: 'Шаги с «поверхностным» ответом (правило 4f), разобранные сверкой #1431: у каждого есть '
      + 'вердикт с источником и датой в отчёте задачи. Гейт check:fast падает только на том, что '
      + 'добавила правка, — новый цветовой или облицовочный ответ обязан получить датированное фото '
      + 'по порогу свежести 4f. Конструктивный материал (камень, сруб, бетон, металл, стекло, а также '
      + 'материал самого произведения — зеркала мозаики, прутья плетёной фигуры) сюда не попадает '
      + 'вовсе: он вне scope правила. Снимается по файлам данных в рабочем дереве, поэтому обновлять '
      + 'его надо на дереве без чужих незавершённых правок. Обновлять: npm run quest:scan-surface-answer:baseline',
    known,
  })
...
async function main() {
  ...
  if (args.json) {
    ...
    if (knownFindings.length) {
      console.log(`\nУчтено baseline (разобрано сверкой #1431): ${knownFindings.length}`)
    }
