#!/usr/bin/env node
/**
 * Миграция трёх районных квестов по Кракову (Казимеж, Подгуже, Нова-Хута) на прод.
 * Идемпотентно: переиспользует существующий квест по quest_id, не дублирует шаги.
 *
 * NODE_TLS_REJECT_UNAUTHORIZED=0 node scripts/migrate-krakow-district-quests.js \
 *   --api-url=https://metravel.by --token=YOUR_TOKEN
 *
 * Dry run (без записи):
 *   node scripts/migrate-krakow-district-quests.js --dry-run
 */
...
// Найти id уже существующего города по имени (постранично), чтобы не плодить дубли «Краков».
async function resolveExistingCityId(name) {
    if (isDryRun) return null;
    try {
        let url = '/api/quests/cities/';
        for (let guard = 0; guard < 50 && url; guard++) {
            ...
            const next = !Array.isArray(page) && page.next ? page.next : null;
            url = next ? next.replace(API_BASE, '') : null;
        }
    } catch { /* список недоступен — создадим город ниже */ }
    return null;
}

const QUESTS = require('./krakow-district-quests-data.js');

async function main() {
    console.log(`🚀 Миграция квестов «Краков» → ${API_BASE} (${isDryRun ? 'DRY RUN' : 'LIVE'})\n`);
    ...
    for (const q of QUESTS) {
        ...
        // 1. Resolve city: reuse existing «Минск» by name, else create.
        //    Все минские квесты (включая minsk-cmok) должны висеть на одном городе.
        let cityId;
        if (!questDbId) {
            cityId = await resolveExistingCityId(q.city.name);
            if (cityId) {
                console.log(`  ℹ️ City reused: id=${cityId} (${q.city.name})`);
            } else {
                try {
        ...
        console.log(`\n🎉 Квест "${q.title}" готов!`);
        console.log(`   URL: https://metravel.by/quests/1/${q.quest_id}`);
    }
