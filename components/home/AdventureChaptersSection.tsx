...
function ChapterCard({ item, index, styles, colors }: ChapterCardProps) {
  ...
  const title =
    (typeof item.name === 'string' ? item.name.trim() : '') || 'Без названия'
  ...
  const travelUrl = useMemo(
    () =>
      resolveTravelUrl({
        ...
        url:
          typeof (item as any).url === 'string' ? (item as any).url : undefined,
      } as any),
    [item],
  )
  ...
  const handlePress = useCallback(() => {
    if (!travelUrl) return
    sendAnalyticsEvent('AdventureChapters_CardClick', {
      travelId: item.id,
      index,
    })
...
function AdventureChaptersSection() {
  ...
  const travelsList = useMemo(() => {
    ...
    if (Array.isArray(travelData)) {
      ...
      (travelData as any)?.data &&
      Array.isArray((travelData as any).data)
    ) {
      arr = (travelData as any).data
    } else if (
      (travelData as any)?.results &&
      Array.isArray((travelData as any).results)
    ) {
        ...
        {travelsList.length === 0 ? (
          <View style={styles.emptyState}>
            ...
            <Text style={styles.emptyTitle}>
              Главы путешествий скоро появятся
            </Text>
            <Text style={styles.emptySubtitle}>
              Пока показываем только основные маршруты. Полная подборка вернётся
            ...
            {rows.map((row, rowIdx) => {
              const paddedRow =
                numColumns > 1
                  ? row.concat(
                      Array.from(
                        { length: Math.max(0, numColumns - row.length) },
                        () => null as unknown as Travel,
                      ),
                    )
                  : row
