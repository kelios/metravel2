/**
 * HomeAdaptive - пример адаптивной главной страницы
 * Демонстрирует использование новой responsive системы
 */
  ...
  return (
    <ResponsiveContainer padding>
      <ResponsiveStack direction="vertical" gap={24}>
        <SkeletonLoader 
          width={isSmallPhone ? 200 : isPhone ? 250 : 300} 
          height={isSmallPhone ? 28 : isPhone ? 32 : 40} 
          borderRadius={8} 
        />
        <ResponsiveStack direction="responsive" gap={20} wrap>
          {Array.from({ length: 3 }).map((_, i) => (
            <SkeletonLoader 
              key={i} 
              width={isSmallPhone ? '100%' : isPhone ? '45%' : '30%'} 
              height={280} 
              borderRadius={12} 
            />
          ))}
        ...
        <ResponsiveContainer padding>
          <ResponsiveStack direction="vertical" gap={isSmallPhone ? 16 : 24}>
            <SkeletonLoader 
              ...
              height={isSmallPhone ? 28 : isPhone ? 32 : 48} 
              borderRadius={8} 
            />
            <SkeletonLoader 
              width={isPhone ? '80%' : 400} 
              height={isSmallPhone ? 16 : isPhone ? 18 : 20} 
              borderRadius={4} 
            />
            <ResponsiveStack 
