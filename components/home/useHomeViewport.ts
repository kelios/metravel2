...
type HomeViewportOptions = {
   ...
   * mobile одним кадром).
   *
   * `false` (по умолчанию) — консьюмер участвует в гидратации
   * (`HomePageSkeleton` есть в SSR HTML), поэтому до собственного commit он
   * обязан отдавать тот же 0×0, что и статический экспорт, иначе React
   * получает hydration mismatch (#418).
   */
  clientOnly?: boolean
}
