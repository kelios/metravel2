...
interface Props {
   ...
   * Проп читает только владельческая ветка: у не-владельца компонент выходит
   * раньше (`if (!trip.isOwner)`) со своей одноколоночной раскладкой, поэтому
   * гость на телефоне получает `mapFirst` от экрана, но не эту ветку.
   */
