...
export const createRoutePanelStyles = (colors: ThemedColors) =>
  StyleSheet.create({
    ...
    // ── Раскладка ──────────────────────────────────────────────────────────
    // Не-мобильная раскладка `stack` (ширина ≥768px, планшет портрет и десктоп):
    // панель слева, карта справа — `workspace` всегда применяется вместе с
    // `workspaceSplit`, одноколоночного пути здесь нет.
