...
export const buildTravelSectionLinks = (
  ...
  // Ссылка «Экскурсии» держится того же предиката, что и сама ExcursionsSection:
  // секция не рисуется, когда Belkraj-виджет ничего не отдаст, и пункт навигации,
  // ведущий в никуда, появляться не должен.
  const hasExcursions = canRenderBelkrajWidget(travel.travelAddress, travel.countryCode)
  pushIf((platform === "web" || platform === "android") && hasExcursions, links, { key: "excursions", icon: "compass", label: i18nT('travel:components.travel.sectionLinks.ekskursii_60b416ff') })
