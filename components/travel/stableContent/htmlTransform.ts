...
// 720 → апскейл ×1.9-2.5, картинки визуально мыльные.
//
// #1113: верхней ступенью desktop-лестницы было 1024 — с обоснованием «выше прокси всё
// равно вернёт 1024». Обоснование неверное: 1024 вообще НЕ входит в whitelist ширин
// прокси, и на такой запрос он не ресайзит и не пережимает, а отдаёт исходный файл
// целиком — вместе с `q=78`, который при этом не применяется. Замер прода 2026-07-28
// на `-detail_hd.jpg` (исходник 1024×576):
//   ?w=800&q=78&fit=contain  →  53 104 B (реальный ресайз + сжатие)
//   ?w=1024&q=78&fit=contain → 132 344 B — оригинал, `q` проигнорирован
//   ?w=1280&q=78&fit=contain → 118 046 B — апскейл, резкости не добавляет
// На статье с 37 фото десктопный кандидат 1024w давал ~8 МБ вместо ~2 МБ. Потолок
// опущен до 800 — последней ступени, которую прокси действительно обрабатывает.
const RESPONSIVE_WIDTHS_MOBILE = [320, 480, 640, 800]
const RESPONSIVE_WIDTHS_DESKTOP = [480, 640, 800]
