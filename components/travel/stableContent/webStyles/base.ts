...
/* #1208/#1213: на web у слота ровно один растр. Поля кадра закрывает заливка рамки,
   а не размытая копия фотографии: прежний ::before с background-image тянул тот же
   файл вторым сетевым запросом (замер прода 2026-08-03: +29 681 B и +174 748 B на
   фото). Блюр-слой остаётся только на native, в ImageCardMedia.

   #1233: но и НЕЙТРАЛЬНАЯ заливка — не то, что задумано. У contain-фото поля
   должен закрывать доминантный цвет кадра (правило ImageCardMedia, решение
   владельца 2026-08-02), иначе портрет висит в пустой светлой рамке. Цвет ставит
   useWebEffects из УЖЕ загруженной картинки, поэтому второго запроса по-прежнему
   нет. Пока цвет не посчитан (или canvas затейнен чужим origin) — прежний нейтрал. */
