...
export const createHeaderStyles = (colors: QuestColors, isMobile: boolean, _screenW: number) => ({
    ...
    // #1669: счётчик заданий вернулся в ряд действий. Он уходил оттуда, пока в
    // ряду стояло семь контролов и его 77px выдавливали первую иконку за край;
    // после переезда редких действий в «Ещё» их четыре, и место освободилось.
    // Выигрыш — строка текста (16px + отступ), которую занимала полоса
    // прогресса: без счётчика она схлопывается до 3px-волосины.
    headerActionCounter: {
        fontSize: 12,
        lineHeight: 16,
        color: colors.textMuted,
        // Счётчик не имеет собственной «желаемой» ширины и потому НИКОГДА не
        // роняет кнопку на вторую строку: он занимает то, что осталось от
        // четырёх 44dp-кнопок, и режется эллипсисом (`numberOfLines={1}`).
        //
        // `flexShrink: 1` тут недостаточно — во flexbox перенос считается ДО
        // сжатия, поэтому при `flexWrap: 'wrap'` длинный счётчик уносил «Ещё»
        // вниз, даже будучи сжимаемым. Замер приёмки #1669 на 320px:
        // «Завдання: 12 / 12» (98px) оставляло 2px запаса, «Завдання: 12 / 24»
        // (100px) уже переносило. Кнопкам ужиматься некуда — 44dp это минимум
        // тач-таргета (#1274), поэтому уступать обязан текст.
        flexBasis: 0,
        flexGrow: 1,
        flexShrink: 1,
        minWidth: 0,
    ...
    headerActionRowMobile: {
        ...
        // #1669 увёл редкие действия в лист «Ещё», и в ряду осталось четыре
        // контрола (шрифт ±, офлайн-загрузка, «Ещё»): арифметика больше не
        // требует именно 3px. Возврат к `SPACING.xs` — видимая правка, её
        // принимают глазами на стадии testing, а не этим комментарием.
        gap: 3,
