...
export default function SubscriptionsTabContent({
  ...
  const filteredAuthors = useMemo(() => {
    ...
    return authors.filter((author) => getFullName(author.profile).toLowerCase().includes(q));
  }, [authors, search, getFullName]);

  const filteredSubscribers = useMemo(() => {
    if (!search.trim()) return subscribers;
    const q = search.trim().toLowerCase();
    return subscribers.filter((profile) => getFullName(profile).toLowerCase().includes(q));
    ...
    <View style={[styles.searchContainer, { backgroundColor: colors.backgroundSecondary, borderColor: colors.borderLight }]}>
      ...
      {search.length > 0 ? (
        <Pressable
          ...
          accessibilityLabel={i18nT('shared:components.subscriptions.SubscriptionsTabContent.ochistit_poisk_8fa83cac')}
          style={[styles.clearSearch, globalFocusStyles.focusable]}
        >
  ...
  const renderSubscribersTab = () => {
    ...
    return (
      <>
        ...
        <View style={styles.header}>
          <Text style={styles.subtitle}>
            {formatSubscribersCount(subscribers.length)}
          </Text>
        </View>
