...
type CompletionState = Extract<FacebookAuthResult, { status: 'email_completion_required' }> & {
    expiresAt: number;
};
...
export default function FacebookAuthFlow({
    ...
    const submitCredential = async (
        ...
        setMessage('');
        if (!credential.emailPermissionGranted && !allowCompletionWithoutPermission) {
    ...
    const startCompletion = async () => {
        ...
        setMessage('');
        try {
            const result = await startFacebookEmailCompletion(completion.completionHandle, email.trim());
    ...
    const confirmCompletion = async () => {
        ...
        try {
            ...
            handleResult(result);
            if (result.status === 'error' && result.errorCode === 'facebook_completion_invalid') {
                resetCompletion();
                setMessage(result.message);
            }
        } finally {
        ...
        return (
            <View style={styles.panel} testID="facebook-permission-panel">
                <View style={styles.headingRow}>
                    ...
                    <Text style={styles.title}>{i18nT('authStatic:facebook.emailPermissionTitle')}</Text>
                </View>
    ...
    if (completion) {
        const reasonText = completion.reasonCode === 'facebook_email_permission_missing'
        ...
        return (
            <View style={styles.panel} testID="facebook-email-completion-panel">
                <View style={styles.headingRow}>
                    <Feather name="mail" size={20} color={colors.info} />
                    <Text style={styles.title}>{i18nT('authStatic:facebook.completeEmailTitle')}</Text>
                </View>
