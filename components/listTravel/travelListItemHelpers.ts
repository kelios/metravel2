...
/**
 * Границы соотношения сторон медиа-слота карточки маршрута (#1487).
 *
 * Слот кадрируется `contain` (инвариант `docs/RULES.md` → «Images and
 * placeholders», исключений по поверхностям нет), поэтому плоское поле
 * `dominant_color` появляется ровно там, где пропорции слота расходятся с
 * пропорциями обложки, и лечится геометрией слота, а не переходом на `cover`.
 *
 * Замер прод-выдачи 2026-08-23 по всем 360 опубликованным маршрутам
 * (`/api/travels/`): 80.0% обложек квадратные 1:1, 10.3% — 4:3, 5.6% — 16:9,
 * 3.6% — 3:4, по одной 3:2 и 9:16. Слоты при этом были ландшафтные — 386×270
 * в каталоге (1.43), 635×316 и 448×292 на главной (2.01 и 1.53), — из-за чего
 * квадратная обложка отдавала под поля до 27% ширины карточки.
 *
 * Поэтому слот берёт пропорции самой обложки, а константы ниже ограничивают
 * только крайности: 16:9 — самый широкий кадр карточки, 5:8 — самый высокий.
 * Внутри диапазона поле равно нулю; за его пределами доля поля с одной стороны
 * остаётся ≤10% для всех соотношений от 0.53 до 2.22, то есть покрывает всю
 * наблюдаемую выдачу с запасом.
 */
export const CARD_MEDIA_SLOT_MIN_RATIO = 0.625
export const CARD_MEDIA_SLOT_MAX_RATIO = 16 / 9

/**
 * Пропорции медиа-слота под конкретную обложку. `null` — пропорции обложки
 * неизвестны (старые payload'ы без `width`/`height`), тогда вызывающий код
 * обязан оставить прежнюю фиксированную высоту слота.
 */
export const resolveCardMediaSlotRatio = (
  aspectRatio: number | null | undefined,
): number | null => {
  if (aspectRatio == null || !Number.isFinite(aspectRatio) || aspectRatio <= 0) return null
  return Math.min(CARD_MEDIA_SLOT_MAX_RATIO, Math.max(CARD_MEDIA_SLOT_MIN_RATIO, aspectRatio))
}
