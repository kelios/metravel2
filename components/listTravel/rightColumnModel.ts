 ...
 * числом. Замер дева 2026-08-23: desktop 1280 — 307 (16:9), 379 (4:3), 476
 * (1:1), 605 (3:4); mobile 390 — 207/276/368/491 медиа плюс ~100 контента.
 * Взвешенная по прод-выдаче (80% обложек 1:1) строка ≈ 455 на обеих ширинах,
 * то есть значения ниже теперь занижены. Поднимать их нельзя в одиночку:
 * инвариант «одна строка позади, две впереди» тянет за собой `drawDistance`,
 * а тот измерен по байтам обложек (#1263/#1400) и требует собственного
 * прогона `e2e/prod-media-smoke.spec.ts`, а не пересчёта на бумаге.
 */
