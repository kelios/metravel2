 ...
 */
/*
 * #1487: с адаптивной геометрией обложки высота строки перестала быть одним
 * числом — она следует пропорциям обложки. Замер прода 2026-08-23 по реальной
 * выдаче `/search` (карточка = медиа + 87 px контента, шаг ряда = высота + gap):
 *   desktop 1280, 3 колонки, высота ряда = максимум из трёх карточек —
 *     310 (16:9), 351 (3:2), 384 (4:3), 483 (1:1), 615 (3:4), 721 (9:16);
 *     замеренные на проде ряды: 310, 615, 384, 483, 615.
 *   mobile 390, одна колонка — 294 (16:9), 363 (4:3), 455 (1:1), 578 (3:4).
 *
 * Заявленное число = МОДА, а не среднее: по распределению прод-обложек
 * (288 из 360 квадратные) ряд из трёх квадратов покрывает 88.7% рядов десктопа,
 * а на мобильном квадрат — это ровно 80% карточек. Хвост (3:4 и 9:16) выше
 * заявленного: 11.3% рядов десктопа получают меньше полной строки позади.
 * Закрывать хвост значило бы поднять `drawDistance` до ~1025 и смонтировать
 * ещё ряд ради 11% случаев — цена в байтах ниже этого не оправдывает.
 */
