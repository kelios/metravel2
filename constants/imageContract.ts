 ...
 *
 * `master` и `derivatives` — не взаимоисключающие множества: профиль может хранить
 * производную РОВНО в ширину своего мастера (`articleBody/1920`). Право спросить
 * ширину даёт наличие производной, а не её несовпадение с мастером: backend
 * `resolve_variant` перебирает производные раньше мастера, поэтому такая ширина
 * отдаётся `stored-derivative` с `immutable`, а не мастером с `no-store`.
 */
export const IMAGE_STORAGE_POLICY_V1 = {
  ...
  articleBody: {
    ...
    derivatives: [
      ...
      { width: 1600, quality: IMAGE_QUALITY.print },
      /**
       * Единственная ступень, СОВПАДАЮЩАЯ с шириной мастера своего профиля, — и это
       * не описка, а закрытие #1215 на стороне backend: `ARTICLE_BODY_PROFILE` в
       * `metravel_shared/image_storage_policy.py` объявляет `content_1920` (q85)
       * обычной stored-производной рядом с `master_width=1920`. `resolve_variant`
       * перебирает производные раньше мастера, поэтому `?w=1920` отдаётся
       * производной, а не мастером.
       *
       * Проба прода 2026-08-10, `travel-description-image/f759c7ba….JPG.webp`:
       * `w=1600` → 200 `stored-derivative` `immutable` (110 578 B), `w=1920` → то же
       * самое, `w=1921` → **400** — то есть fail-closed чтение работает, и 200 на
       * 1920 означает именно объявленную производную.
       *
       * Пока этой строки не было, зеркало обещало МЕНЬШЕ, чем есть: пост-деплой гейт
       * считал легальную ступень 1920 из манифеста нарушением #1260 и валил каждый
       * деплой (#1373). Обратный дрейф того же зеркала — сам #1260.
       */
      { width: 1920, quality: IMAGE_QUALITY.print },
    ],
  },
...
export const IMAGE_WIDTHS = {
  ...
  /**
   * Потолок — 1920, последняя ПРОИЗВОДНАЯ профиля `articleBody`.
   *
   * Ступень 1920 здесь уже стояла и была снята 2026-08-03: тогда это была ширина
   * МАСТЕРА, а не производной. Пока backend умел динамический resize, он такой
   * запрос обслуживал; после включения `MEDIA_IMAGE_DERIVATIVE_READ_ENABLED` чтение
   * стало fail-closed и мастер через `?w=` начал отвечать 400 — замер прода
   * 2026-08-03, `media-resize/legacy/3508/…-detail_hd.jpg`: `w=1600` → 200
   * (232 720 B, stored-derivative), `w=1920` → **400** (47 B, no-store). На desktop
   * @DPR2 браузер выбирал из srcset именно 1920, и 22 из 34 запросов тела статьи
   * падали. Год спустя тем же местом дважды не наступаем: ступень вернулась только
   * потому, что backend завёл `content_1920` РЯДОМ с мастером (#1215), и её
   * легальность подтверждена пробой, а не рассуждением — см. `articleBody`.
   *
   * Слот 920 CSS на 1920vw @DPR2 просит 1840: до возвращения ступени он закрывался
   * 1600 с апскейлом ×1.15, теперь — 1:1. Ступень 1600 остаётся: слот 720 CSS @DPR2
   * просит 1440, и 1920 для него был бы перелётом ×1.33.
   */
  articleBodyDesktop: [480, 640, 800, 960, 1600, 1920],
 ...
 *
 * До появления ступени 1920 такие URL не клэмпились вовсе, и это было безопасно
 * только потому, что набор `articleBodyDesktop` сам обрывался на 1600 — свойство
 * набора, а не роута. Проба 2026-08-10 по `legacy/682/conversions/…webp`:
 * `w=1600` → 200 `stored-derivative`, `w=1920` → **400**. Поэтому вместе с
 * возвращением ступени 1920 (#1373) неопознанный адрес клэмпится явно.
 *
