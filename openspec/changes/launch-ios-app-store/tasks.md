...
## 4. iPhone product compatibility
...
- [ ] 4.5 Run the launch-critical iPhone simulator matrix for guest/auth flows, discovery/search, travel/article/quest details, maps, profile/settings/account deletion, five locales, offline/slow/error states, and cold restart; file concrete defects instead of broad duplicate tickets.
...
- [ ] 6.1 After explicit production-build authorization, create the signed iPhone candidate with the selected canonical release path and record source revision, bundle/version/build, signing/entitlements/privacy validation, archive inspection, and App Store upload processing without exposing secrets.
...
- [ ] 6.3 Run the full physical-iPhone TestFlight matrix on the exact candidate: fresh install/update, guest/auth/Apple login, session restart, links, map/location, HEIC/gallery, share/export, notifications, permissions, five locales, accessibility, offline recovery, account deletion visibility, crashes, hangs, logs, and production network targets.
...
- [ ] 7.2 Capture truthful localized iPhone screenshots from the accepted candidate for required device sizes and verify that store claims, screenshots, app icon, and permission explanations match runtime behavior.
