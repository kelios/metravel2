...
3. **Nonce генерируется на клиенте на каждую попытку** (`expo-crypto` `getRandomValues`) и отправляется серверу вместе с токеном; сервер сравнивает его с claim `nonce` **как есть**. Открытый вопрос закрыт по заголовкам SDK: `FBSDKCoreKit` создаёт claims через `initWithEncodedClaims:nonce:` с *expected nonce* из конфигурации входа, то есть Meta кладёт в claim ровно ту строку, что передана в `logInWithPermissions`, и сама её не хеширует. Хеширование на клиенте (шаблон Firebase) нужно лишь тому, чей сервер ждёт `sha256(rawNonce)`; наш сервер сравнивает сырое значение.
