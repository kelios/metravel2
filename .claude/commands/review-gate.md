...
`review → testing`»). Дальше той же цепочкой идут выкат на прод и приёмка на проде до `done`
(«Выкат и приёмка на проде — часть перехода `testing → done`»). Отпечаток вердикта считается по содержимому изменённых файлов
