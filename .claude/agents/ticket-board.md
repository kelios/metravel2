...
Карточку в `in_progress`/`review`/`testing` ведёт ОДНА сессия. Просьба «переведи #N в
`in_progress`» по карточке, которая УЖЕ в работе, — сигнал возможного столкновения, а не
рутина: сперва скажи вызвавшему, что карточку, судя по статусу, кто-то ведёт, и что перед
перехватом нужен `ListAgents` + `SendMessage` живой сессии. Хук `.claude/hooks/task-claim.mjs`
такой перехват блокирует и называет владельца; получив его отказ, НЕ обходи его через
`TASK_CLAIM_BYPASS` и не пробуй другой путь — верни отказ вызвавшему вместе с именем сессии.
Claim снимается сам при `done`/`wont_do`/`todo`/`backlog` и протухает через 45 минут молчания
сессии; вручную — `node .claude/hooks/task-claim.mjs release --task <id>`.
