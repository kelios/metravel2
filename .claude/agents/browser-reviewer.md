...
4. Если правка НЕ наблюдаема в превью (другой рантайм, типы, тулинг, native-only) — честно
   скажи «browser-verify неприменимо, причина», и ограничься code-review.
5. Если проверить реально невозможно из-за внешнего блокера (превью не поднимается) —
...
Чисто и подтверждено браузером → `PASS` с пустым `[]`.

## Запрещено

- Править файлы (нет Edit/Write — и не обходи через Bash: никаких `sed`/`echo >`/патчей).
- Мутирующие команды (build prod, deploy, git commit, npm install).
- Помечать PASS без браузер-проверки, если правка наблюдаема в превью.
