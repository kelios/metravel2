...
2. Target-окружение утверждения — локальный dev-web (`Expo Web ProxyApi 8099` и соседи),
...
конца, а не выбирай удобный ответ (`AGENTS.md` §3 п.5). «Нарушен контракт» — это ссылка на
...
пробу на пользователя («пролистай», «пришли скрин») запрещено (`AGENTS.md` §3 п.12).
