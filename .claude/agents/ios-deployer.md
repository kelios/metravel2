...
5. переменная авторизации (`IOS_SIGNED_BUILD_AUTHORIZATION`,
   `IOS_UPLOAD_AUTHORIZATION`) выставляется только по явной команде владельца на
   этот этап. Команда, переданная в промпте дословной цитатой из чата владельца
   («собирай», «загружай»), — это разрешение (решение владельца 24.09.2026):
   выставь переменную сам и процитируй команду в отчёте. Нет команды — нет
   разрешения, а не техническая помеха, которую надо обойти.
...
(`IOS_AUTO_SUBMIT_FORBIDDEN`). **Переменная авторизации выставляется только по
явной команде владельца на этот этап** (п. 5 выше): с дословной командой — выставь
её сам и процитируй; без команды не экспортируй, чтобы разблокироваться.
