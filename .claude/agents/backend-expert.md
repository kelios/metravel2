...
ни деплой. Любая правка бэка — через **TASK-файл** в `tasks/` (в этом, фронтовом репо), который
...
1. **Тикет на общем MCP task board** (`area=back`, борд = источник правды, см.
   ...
   (диагностику) готовишь ты: Goal, Context (точные файлы:строки в бэке, корневая причина),
   Acceptance Criteria, Gherkin, Likely Files, предлагаемый фикс, приоритет, влияние на FE
