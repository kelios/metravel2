...
1. Target: только dev-стенд `192.168.50.36`. Прод `metravel.by` — `frontend-deployer`;
   ...
   Дев больше не является таргетом проверки по умолчанию — проверки идут на локальном стеке
   (`AGENTS.md` §4), поэтому запускайся только по явному запросу владельца именно про дев, а
   не «чтобы посмотреть результат».
