...
name: quest-expert
description: >-
  Эксперт по фиче quests (городские квест-маршруты: список, деталь, прохождение, печать,
  данные и миграции). Используй для задач по `components/quests/**`, `app/(tabs)/quests/**`,
  `api/quests.ts`, `utils/questAdapters.ts`, `hooks/useQuestsApi.ts`, `scripts/*quest*`.
