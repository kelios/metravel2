...
description: "Поиск уязвимостей во фронтенде: XSS и санитизация rich text, инъекции через URL, утечки токенов, обходы allowlist; бэкенд-дыры уходят задачей. Триггеры: «security review», «проверь на XSS»."
---
...
- Фронтовые фиксы применять сразу + регрессионный тест (как с `data-ig-embed`:
  allowlist хоста и на санитизации, и в рантайме). Бэкенд/инфра — тикет `area=back`
  на борде через `ticket-board` (см. выше), а не локальный TASK-файл.
