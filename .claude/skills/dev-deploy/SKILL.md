---
name: dev-deploy
description: "Деплой web-фронтенда на дев-сервер через build-dev.sh с пост-деплой проверкой; делегирует агенту dev-deployer. Триггеры: «задеплой на дев», «обнови дев», «почему дев отдаёт 404/502»."
---
...
**Дев больше не является таргетом проверки по умолчанию.** Проверки идут на локальном
стеке (`AGENTS.md` §4, `docs/WORKFLOW_OPERATIONS.md` → «3.0 Локальный стек»), поэтому этот
скилл запускается только по явному запросу владельца именно про дев — «посмотреть результат»
поводом не является. Стенд к тому же регулярно лежит и поднимает его владелец.
