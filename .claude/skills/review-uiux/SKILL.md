...
description: >-
  ...
  с дизайн-системой; находки исправляются. Для того же экрана в приложении — `android-qa-sweep`
  (Android) и `ios-qa-sweep`/`ios-designer` (iPhone): web-ревью не доказывает native-поверхность.
  Триггеры: «UI/UX ревью», «проверь юзабилити», «проверь интерфейс страницы».
---
...
3. **Интерактивная проверка**: клики по основным действиям (поиск, фильтр, карточка,
   пагинация/инфинит-скролл) — computer/fill, после каждого — snapshot:
