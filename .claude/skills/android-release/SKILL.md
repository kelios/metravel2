...
3. Smoke-прогон **по документированному списку**, а не по памяти: P1-минимум
   `AND-USB-01..06`, `08`, `10`, `13..18`, `21..31` из `docs/MANUAL_TEST_CASES.md`
   плюс предметные кейсы затронутых поверхностей (`TR-DET-02` галерея,
   `TR-DET-03` описание). Импровизированный обход по табам этот список НЕ
   заменяет. Краши ловить в `adb logcat`.
   - **Каждую поверхность проверять минимум на ДВУХ разных данных**: короткая и
     длинная статья, пустой и заполненный список, залогинен и гость. Одна
     выборка не отличает «работает» от «работает только сверху»: 03.09.2026 фото
     в описании проверили на одной статье, там они попали в стартовую полосу
     viewport-гейта и загрузились — а на длинной не грузились вовсе, и сборка
     уехала в прод (`66e548ef2`).
   - **Ноль сетевых попыток в logcat = компонент не смонтирован.** Это отдельный
     класс, а не «картинка не загрузилась»: URL и CDN проверять бессмысленно,
     искать надо гейт видимости/условный рендер. Симптом — пустая рамка-заглушка
     при живом URL (`curl` отдаёт 200).
   - Если в аудите Фазы 1 нашлись дефекты в медиа//URL — расширить выборку по
     этой зоне, а не сузить.
