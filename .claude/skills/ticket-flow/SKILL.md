...
## ЖЕЛЕЗНОЕ ПРАВИЛО: карточку в работе ведёт ОДНА сессия

`status=in_progress` — это НЕ «работу бросили, доделывай». Это «кто-то работает прямо
сейчас». Прежде чем взять такую карточку (в том числе когда пользователь дал ссылку на
неё и сказал «доделывай»):

1. `ListAgents` — есть ли на машине другие живые сессии.
2. Есть — `SendMessage` той, что может вести карточку: что она держит, что уже применено
   наружу (прод-контент, деплой, общие файлы), кто доводит до конца.
3. `git status --short` по файлам, которые собираешься менять. Чужие несохранённые правки
   в них = карточку уже ведут, даже если claim'а нет.

Только после этого — `status=in_progress` на себя. Молчаливый параллельный прогон
13.09.2026 стоил переписанного прод-контента: две сессии вели #1923 и с разницей в
пятнадцать секунд применили РАЗНЫЙ текст одного шага квеста; последний писатель выиграл
молча, по `GET` этого не видно.

Механически это держит хук `.claude/hooks/task-claim.mjs`: чужой живой claim блокирует
переход в `in_progress`, а открытие чужой карточки в работе даёт предупреждение. Хук —
страховка, а не замена разговору: он знает только про сессии на этой машине.
Посмотреть и снять claim вручную: `node .claude/hooks/task-claim.mjs show|release --task <id>`.
...
2. **Выбор/заведение тикета.** Карточка уже в `in_progress`/`review`/`testing` — сначала
   проверка занятости из «ЖЕЛЕЗНОГО ПРАВИЛА» выше (`ListAgents` → `SendMessage` →
   `git status`), и только потом работа. Без этого не начинай даже чтение кода под правку.
