...
Разрешение — явная команда владельца на конкретный этап (в чате или
переменной). Решение владельца 24.09.2026: получив такую команду в чате
(«собирай», «загружай», «отправляй на ревью»), агент сам выставляет переменную
этого этапа и цитирует команду дословно в отчёте и карточке. Без команды
переменную не выставлять: её отсутствие — это отсутствие разрешения, а не
техническая помеха. Команда на один этап не разрешает следующий.
`--auto-submit` запрещён гейтом (`IOS_AUTO_SUBMIT_FORBIDDEN`).
...
1. **Готовность.** `main`, чистое дерево, известный source revision.
   ...
   - **Квота плана — 15 iOS-сборок и 15 Android за расчётный период.** Дашборд
     показывает их одним агрегатом `Total builds N / 30`, и по нему легко решить,
     что у iOS есть все 30, — это неверно. Считать надо по iOS-половине.
     Замер 03.09.2026: `Total builds 2 / 30`, обе сборки iOS, значит из
     iOS-лимита израсходовано 2 из 15.
     ...
     - **Android свои 15 не использует вообще** — он собирается локальным Gradle
       (`Android builds 0`). Перелить их в iOS нельзя: лимиты платформенные.
