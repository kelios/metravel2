...
description: "Статьи-путешествия из папок с фото: SEO-текст, точки на карте, обложка, галерея; и обогащение опубликованных статей. Триггеры: «сделай статьи из папки <год>», «добавь фото и распиши статью»."
---
...
`--meta` НЕ работает и отвергается до записи: `TravelUpsertSerializer` поле
`meta_description` не объявляет, поэтому DRF срезает его на валидации и до
статьи значение не доезжает — при HTTP 200 на PUT. Раньше скрипт на этом месте
объявлял порчу текста и откатывал вместе с метой всё заново собранное описание
(#1716). Прочитать поле тоже нечем: `GET /api/travels/<id>/` его не возвращает.
И запись, и чтение ждут бэкенд-задачу #1737; до неё мету через API не выставить
никак, а тело статьи правится тем же вызовом без `--meta`.

`seo-edit` меняет ТОЛЬКО `description`. Название/slug он не
