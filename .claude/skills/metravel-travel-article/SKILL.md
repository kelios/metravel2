...
7. **Вставь фото в текст описания** (не только галерея!):
   `python scripts/metravel_publish.py descimg <id>` — берёт фото из галереи статьи
   и расставляет их между разделами текста (идемпотентно). В описании НЕ должно
   быть «только текст» — всегда 2–4 инлайн-фото.
